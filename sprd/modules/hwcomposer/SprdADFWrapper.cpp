...
bool SprdADFWrapper::Init() {
  ...
  /* getDisplayConfigs for KaiOS */
  {
    uint32_t configs[MAX_NUM_CONFIGS] = {0};
    size_t numConfigs = MAX_NUM_CONFIGS;
    adf_getDisplayConfigs(mHelper, 0, configs, &numConfigs);
  }
...
int SprdADFWrapper::Blank(int DisplayType, bool enabled) {
  ...
  ALOGD_IF_SLOW(100, "Excessive delay in adf_blank()");
...
#if 1
    ret = attachOverlayEngineToInterface(ctx, &overlayEngineId);
    if (ret == -EALREADY)
    {
      ALOGE("attachOverlayEngine engine maybe busy i: %d", i);
      ret = -1;
      goto EXT3;
    } else if (ret)
    {
...
#if 0 //for temp debug
  if (tracker->releaseFenceFd > 1000) {
	  String8 check_name("SprdADFWrapper:fence fd leak check");
	  int check_ret = AdfFenceWait(check_name, tracker->releaseFenceFd);
	  if (check_ret == 0) {
		  ALOGI_IF(mDebugFlag, "wait fence fd:%d success.tracker->releaseFenceFd = %d;", check_ret,tracker->releaseFenceFd);
	  } else {
		  ALOGI_IF(mDebugFlag, "wait fence fd:%d failed!tracker->releaseFenceFd = %d;", check_ret,tracker->releaseFenceFd);
	  }
  }
#endif
  ALOGI_IF(mDebugFlag,
...
void SprdADFWrapper::implementCustomConfig(
  ...
  ALOGI_IF(mDebugFlag,
           ...
           "%x, transform: %d, scale: %d, compression: %d",
