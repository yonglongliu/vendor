...
#define SPRD_VPP_GET_IOVA \
  _IOWR(SPRD_VPP_IOCTL_MAGIC, 7, struct vsp_iommu_map_data)
#define SPRD_VPP_FREE_IOVA \
  _IOW(SPRD_VPP_IOCTL_MAGIC, 8, struct vsp_iommu_map_data)
...
#define VPP_WRITE_REG(reg_addr, value, pstring) \
  (*(volatile uint32 *)(reg_addr + vo->s_vpp_Vaddr_base) = (value))
#define VPP_READ_REG(reg_addr, pstring) \
  (*(volatile uint32 *)(reg_addr + vo->s_vpp_Vaddr_base))
#define VPP_READ_REG_POLL(reg_addr, msk_data, exp_value, time, pstring)   \
  vpp_read_reg_poll((VPPObject *)vo, reg_addr, msk_data, exp_value, time, \
                    pstring)
