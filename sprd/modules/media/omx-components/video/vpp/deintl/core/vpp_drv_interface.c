...
int32 vpp_open_dev(VPPObject *vo) {
  ...
  if (-1 == vo->s_vpp_fd) {
    if ((vo->s_vpp_fd = open(SPRD_VPP_DRIVER, O_RDWR)) < 0) {
      ...
      vo->s_vpp_Vaddr_base =
          (unsigned long)mmap(NULL, SPRD_VPP_MAP_SIZE, PROT_READ | PROT_WRITE,
                              MAP_SHARED, vo->s_vpp_fd, 0);
    }
  }

  SCI_TRACE_LOW("vpp_open_dev s_vpp_fd = %d, s_vpp_Vaddr_base = 0x%lx\n",
...
int32 vpp_close_dev(VPPObject *vo) {
  if (vo->s_vpp_fd > 0) {
    if (munmap((void *)vo->s_vpp_Vaddr_base, SPRD_VPP_MAP_SIZE)) {
      SCI_TRACE_LOW("%s, %d, failed", __FUNCTION__, __LINE__);
      return -1;
    }
...
int VPP_Get_IOVA(VPPObject *vo, int fd, unsigned long *iova, size_t *size) {
  int ret = 0;
  VSP_IOMMU_MAP_DATA_T map_data;

  map_data.fd = fd;
  ret = ioctl(vo->s_vpp_fd, SPRD_VPP_GET_IOVA, &map_data);

  if (ret < 0) {
    SCI_TRACE_LOW("vpp_get_iova failed ret %d \n", ret);
  } else {
    *iova = map_data.iova;
    *size = map_data.size;
  }
...
int VPP_Free_IOVA(VPPObject *vo, unsigned long iova, size_t size) {
  int ret = 0;
  ...
  ret = ioctl(vo->s_vpp_fd, SPRD_VPP_FREE_IOVA, &unmap_data);

  if (ret < 0) {
    SCI_TRACE_LOW("vpp_free_iova failed ret %d \n", ret);
  }

  return ret;
}

int VPP_Get_IOMMU_Status(VPPObject *vo) {
  int ret = 0;

  ret = ioctl(vo->s_vpp_fd, SPRD_VPP_GET_IOMMU_STATUS);
...
int32 vpp_allocate_dma_memory(VPPObject *vo, VPP_DRV_BUFFER_T *vb) {
  ...
  vb->phys_addr = (unsigned int)vdb.phys_addr;
  vb->base = (unsigned long)vdb.base;

  SCI_TRACE_LOW(
      "vpp_allocate_dma_memory, physaddr=0x%x, virtaddr=0x%x, size=%d\n",
...
void vpp_free_dma_memory(VPPObject *vo, VPP_DRV_BUFFER_T *vb) {
  ...
  SCI_TRACE_LOW("base: 0x%lx, phys_addr: 0x%x, size: 0x%x, virt_addr: 0x%lx\n",
...
int32 vpp_read_reg_poll(VPPObject *vo, uint32 reg_addr, uint32 msk_data,
                        uint32 exp_value, uint32 time, char *pstring) {
  uint32 tmp, vsp_time_out_cnt = 0;

  tmp =
      (*((volatile uint32 *)(reg_addr + ((VPPObject *)vo)->s_vpp_Vaddr_base))) &
      msk_data;
  while ((tmp != exp_value) && (vsp_time_out_cnt < time)) {
    tmp = (*((volatile uint32 *)(reg_addr +
                                 ((VPPObject *)vo)->s_vpp_Vaddr_base))) &
          msk_data;
    vsp_time_out_cnt++;
  }
