/******************************************************************************
 ** File Name:    vpp_deint_api.h                                                   *
 ** Author: *
 ** DATE:         6/2/2016                                                   *
 ** Copyright:    2016 Spreadtrum, Incorporated. All Rights Reserved.         *
 ** Description:  define data structures for vpp deinterlace                      *
 *****************************************************************************/
...
int vpp_deint_get_iova(VPPObject *vo, int fd, unsigned long *iova,
  ...
  ret = VPP_Get_IOVA(vo, fd, iova, size);
...
int vpp_deint_free_iova(VPPObject *vo, unsigned long iova, size_t size) {
  ...
  ret = VPP_Free_IOVA(vo, iova, size);
...
int32 vpp_deint_process(VPPObject *vo, unsigned long src_frame,
  ...
  SCI_TRACE_LOW(
      "vpp_deint_process: src_frame = 0x%lx, ref_frame = 0x%lx, dst_frame = "
      "0x%lx, frame_no = %d\n",
...
#else
  {
    ...
    VPP_WRITE_REG(VPP_INT_CLR, V_BIT_1,
                  "VSP_INT_CLR, clear all prossible interrupt");
  }
#endif
