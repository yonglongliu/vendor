...
FMHalThread::FMHalThread(uint32_t  readUnitSize,int card, int device,struct pcm_config * cfg):Thread(false),
pcmHandle(NULL),readSize(readUnitSize),mCard(card),mDevice(device),exit_var(0)
{
    readBuf=(uint8_t *)malloc(readSize);
    if(!readBuf) {
        ALOGE("FMHalThread malloc failed");
    }
    if(cfg) {
        memcpy(&pcm_cfg,cfg,sizeof(struct pcm_config));
    }
    recordbypass=false;
    isactivited=false;
}
...
void FMHalThread:: pause(bool paused)
{
    AutoMutex lock(&mLock);
    recordbypass=paused;
    if((true==paused)&&(isactivited==true)){
        AutoMutex pcmlock(&mpcmLock);
        if(pcmHandle != NULL){
            pcm_close(pcmHandle);
            pcmHandle = NULL;
        }
...
bool FMHalThread:: is_activited(void)
{
    AutoMutex lock(&mLock);
     return isactivited;
}
 ...
 bool        FMHalThread:: threadLoop()
{
    ...
    while (!exitPending()) {
            ...
            if(true==recordbypass){
                ...
                isactivited=false;
                mLock.unlock();
                usleep(readSize*1000000/(2*pcm_cfg.channels)/pcm_cfg.rate);
                ...
                if(card < 0){
                    ...
                    pcmHandle = pcm_open(card, 0, PCM_IN , &pcm_cfg);
                    if (!pcm_is_ready(pcmHandle)) {
                        ALOGE("FM thread  cannot open pcm_in driver: %s", pcm_get_error(pcmHandle));
                        pcm_close(pcmHandle);
                        pcmHandle = NULL;
                        mpcmLock.unlock();
                        usleep(500000);
...
FMHalSource::FMHalSource(int samplerate,int channel):ply_len(0),ply_buf(NULL),mTrack(0),state(IDLE),is_ready(false)
{
       ...
       cfg.format =  PCM_FORMAT_S16_LE;
        ...
        AutoMutex lock(&mLock);
        if((!pcmHandle)&& (FMThread == NULL))  {
            card=get_snd_card_number((const char *)FM_CARD_NAME);
            if(card < 0){
                ALOGE("SmartPA:For Record,can not find the fm card");
                return;
            }
            ...
            mUnitBytes= pcm_frames_to_bytes(pcmHandle, mPeriodSize);
        }
        else{
            if(  (mChannel != channel) ||
                  (mSamplerate !=  samplerate)) {
        ...
        if(pcmHandle || (FMThread != NULL)){
            ...
            mTrack = new FMTrack(mPeriodCount*4,mUnitBytes);
        ...
        if(FMThread == NULL) {
            ...
            FMThread = new FMHalThread(mUnitBytes,card,0,&cfg);
        }
        openCount++;
}
...
void FMHalSource:: pause(bool paused)
{
    AutoMutex lock(&mLock_l);
    FMThread->pause(paused);
}

bool FMHalSource:: is_activited(void)
{
    AutoMutex lock(&mLock_l);
    return FMThread->is_activited();
}
   ...
   //ALOGD("peter: read in bytes %d, buffered bytes is %d",bytes,mTrack->GetDataCount());
    ...
    while(left){
        if(!ply_len){
           ...
           //ALOGE("peter: ply_len:%d,%x",ply_len,ply_buf);
        }
        if(ply_len&&ply_buf){
            copy_count= ply_len>left?left:ply_len;
            //ALOGE("peter: copy_count:%d,cur_ptr:%x,ply_buf:%x",copy_count,cur_ptr,ply_buf);
...
void fm_pcm_pause(FMPcmHandler pcm,bool paused)
{
    ...
    ALOGV("fm_pcm_pause in");
    if(pcmtmp->state == 1)
    {
        pcmtmp->mFmhalSource->pause(paused);
    }
}
...
bool fm_pcm_isactivited(FMPcmHandler pcm)
{
    ...
    ALOGV("fm_pcm_isactivited in");
    if(pcmtmp->state == 1)
    {
        return pcmtmp->mFmhalSource->is_activited();
    }
    return false;
}
}
