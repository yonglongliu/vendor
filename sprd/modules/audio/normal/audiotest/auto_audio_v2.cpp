...
int audPlayerOpen( int outdev, int sampleRate, int stereo, int sync )
{
    ...
    AudioSystem::setStreamVolumeIndex(streamType,15,audDevice);
...
int audRcrderOpen( int indev, int sampleRate )
{
...

    {
        ...
        if(AUD_INDEV_BACK_MIC==indev){
             i=10;// AuxMic is muted 800ms with audio hal,
        }else{
            i=4;
        }

        ALOGI("audRcrderOpen begin clear");
        while(i--){
            audRcrderRecord(tmp, AUD_RCRD_FRAME_BUF);
        }
        ALOGI("audRcrderOpen clear end");
    }
