...
void set_audio_mode_num(int mode){
    s_audio_mode_num = mode;
    ALOGI("%s, current mode num:%d", __func__, s_audio_mode_num);
}
...
static int do_parse(AUDIO_TOTAL_T *audio_params_ptr, unsigned int params_size)
{
    ...
    if(s_audio_mode_num == 0) {
        s_audio_mode_num = adev_get_audiomodenum4eng();
        ALOGI("%s, the old mode num(0), current mode num(%d)", __func__, s_audio_mode_num);
    }
    if (s_audio_mode_num*sizeof(AUDIO_TOTAL_T) != params_size) {
        ALOGE("Error: params_size = %d, total size = %d", params_size, s_audio_mode_num*sizeof(AUDIO_TOTAL_T));
        return -1;
    }
    ...
    fw_header->num_profile = s_audio_mode_num*3; //TODO
    fw_header->num_da = s_audio_mode_num; //TODO
    fw_header->num_profile = s_audio_mode_num; //TODO
    fw_header->num_profile = s_audio_mode_num; //TODO
    ...
    for (i=0; i<s_audio_mode_num; i++) {
        ...
        fseek(fd_dest_paras,sizeof(struct vbc_fw_header)+i*sizeof(struct vbc_ad_eq_profile)+s_audio_mode_num*sizeof(struct vbc_da_eq_profile), SEEK_SET);
        ...
        fseek(fd_dest_paras,sizeof(struct vbc_fw_header)+i*sizeof(struct vbc_ad_eq_profile)+s_audio_mode_num*sizeof(struct vbc_da_eq_profile)+s_audio_mode_num*sizeof(struct vbc_ad_eq_profile), SEEK_SET);
...
int create_vb_effect_params(void)
{
    ...
    //close fd
    if (aud_params_ptr) {
        ret = do_parse(aud_params_ptr, s_audio_mode_num*sizeof(AUDIO_TOTAL_T));
    }
...
int vb_effect_profile_apply(int mode)
{    int ret = 0;
    ALOGI("cur_mode(%d)", mode);

    if (s_ctl_da_eq_profile_select && (mode < s_audio_mode_num) && (mode >=0)) {
        ret = mixer_ctl_set_value(s_ctl_da_eq_profile_select, 0, mode);
        return 0;
    }
