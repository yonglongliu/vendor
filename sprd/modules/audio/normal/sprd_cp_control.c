...
static void *mixer_to_cp_thread(void *args){
    ...
    while(rt_res->init){
        ...
        pcm = pcm_open(card, 0, PCM_OUT, &rt_res->config);
        #else
        ...
        if(rt_res->fd<0)
        {
            rt_res->fd = open(rt_res->root_path, O_RDONLY);
            if(rt_res->fd<0)
                rt_res->fd = open(rt_res->local_path, O_RDONLY);
            if(rt_res->fd<0) {
               ALOGE("YAYE %s unable to open file %s!!!", __func__, rt_res->root_path);
            }
        }
        while(rt_res->enable){
            pthread_mutex_unlock(&rt_res->lock);
        ...
        next:
            ...
            memset(rt_res->root_path, 0, sizeof(rt_res->root_path));
            memset(rt_res->local_path, 0, sizeof(rt_res->local_path));
...
int set_cp_mixer_type(struct mixer *cp_mixer, int mixer_type){
    int ret=0;
    struct mixer_ctl *ctl1;

    ALOGD("YAYE %s mixer is %p", __func__, cp_mixer);
    ctl1 = mixer_get_ctl_by_name(cp_mixer,"PCM MIXER Route");
    mixer_ctl_set_value(ctl1, 0, mixer_type);//mixer_type 0: dl 1:ul 2:ul&&dl

    ALOGD("YAYE %s ctl1:%p mixer_type:%d", __func__, ctl1, mixer_type);

    return 0;
}

int cp_mixer_enable(struct cp_control_res * record_tone_info, struct mixer *cp_mixer, int card, char* file_name){
    int ret=0;
    ...
    if(!file_name) {
        sprintf(rt_res->root_path, AUDIO_MIXER_TO_CP_PCM_FILE_ROOT);
        sprintf(rt_res->local_path, AUDIO_MIXER_TO_CP_PCM_FILE);
    }else {
#ifdef FLAG_VENDOR_ETC
        sprintf(rt_res->root_path,"vendor/etc/%s",file_name);
#else
        sprintf(rt_res->root_path,"/etc/%s",file_name);
#endif
        sprintf(rt_res->local_path ,"/data/local/media/%s",file_name);
    }
