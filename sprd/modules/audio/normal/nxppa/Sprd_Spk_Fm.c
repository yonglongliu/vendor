...
int NxpTfa_Fm_Start(fm_handle pFmRes)
{
    ...
    int ret = 0;
    int status;
    T_NXPPA_RES *nxp_fm_res = (T_NXPPA_RES *)pFmRes;

    if (!nxp_fm_res) {
        ALOGE("sprdfm: NxpTfa_Fm_Start, nxp_fm_res is NULL!");
        return -1;
    }
...
{
    int rc = 0;
    T_NXPPA_RES * nxp_fm_res = (T_NXPPA_RES *)handle;

    if(NULL == handle) {
        ALOGE("Sprdfm: NxpTfa_Fm_Stop ,fm not open");
        return 0;
    }
    ...
    if(1 != nxp_fm_res->is_start) {
        pthread_mutex_unlock(&nxp_fm_res->FmLock);
        ALOGE("Sprdfm: NxpTfa_Fm_Stop, not start yet.");
        return -1;
    }
...
int NxpTfa_Fm_Pause( fm_handle handle,bool ispaused)
{
    T_NXPPA_RES * nxp_fm_res = (T_NXPPA_RES *)handle;

    if(NULL == handle) {
        ALOGE("Sprdfm: NxpTfa_Fm_Pause ,fm not open");
        return 0;
    }

    pthread_mutex_lock(&nxp_fm_res->FmLock);
    if(nxp_fm_res->HalSource_handle) {
        fm_pcm_pause(nxp_fm_res->HalSource_handle,ispaused);
    }
    pthread_mutex_unlock(&nxp_fm_res->FmLock);

    ALOGI("Sprdfm: NxpTfa_Fm_Pause out");

    return 0;
}

bool NxpTfa_Fm_IsActivited( fm_handle handle)
{
    bool ispaused=false;
    T_NXPPA_RES * nxp_fm_res = (T_NXPPA_RES *)handle;

    if(NULL == handle) {
        ALOGI("Sprdfm: NxpTfa_Fm_IsActivited ,fm not open");
        return ispaused;
    }

    pthread_mutex_lock(&nxp_fm_res->FmLock);
    if(nxp_fm_res->HalSource_handle) {
        ispaused=fm_pcm_isactivited(nxp_fm_res->HalSource_handle);
    }
    pthread_mutex_unlock(&nxp_fm_res->FmLock);
...
fm_handle NxpTfa_Fm_Open(void)
{
    ...
    if (!nxp_fm_res) {
        ALOGE("Sprdfm:NxpTfa_Fm_Open, malloc failed!Code is '%s'", strerror(errno));
        return NULL;
    }
    ...
    if (ret) {
        ALOGE("Sprdfm:sem_init nxp_fm_res->sem_stop failed! Code is %s", strerror(errno));
        free(nxp_fm_res);
        return NULL;
    }
...
int NxpTfa_Fm_Close(fm_handle handle)
{
    ...
    if(!handle) {
        ALOGE("Sprdfm: NxpTfa_Fm_Close ,handle is NULL!");
        return -1;
    }
