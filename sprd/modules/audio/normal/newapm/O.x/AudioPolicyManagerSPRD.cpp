                                             ...
                                             audio_session_t session)
{
    ...
    if((!isFmMute) && (stream==AUDIO_STREAM_ENFORCED_AUDIBLE || stream==AUDIO_STREAM_RING ||
            stream==AUDIO_STREAM_ALARM || stream == AUDIO_STREAM_NOTIFICATION)){
        if((outputDesc->mRefCount[AUDIO_STREAM_ENFORCED_AUDIBLE]==0) || (outputDesc->mRefCount[AUDIO_STREAM_RING]==0) ||
                (outputDesc->mRefCount[AUDIO_STREAM_ALARM]==0) || (outputDesc->mRefCount[AUDIO_STREAM_NOTIFICATION]==0)) {
                                            ...
                                            audio_session_t session)
{
    ...
    int ret = stopSource(outputDesc, stream, forceDeviceUpdate);
