...
static int vb_voice_fake_pipe = -1;
static int vb_voip_fake_pipe = -1;
...
extern int pcm_set_samplerate(struct pcm *pcm, unsigned int flags,
                       struct pcm_config *config, unsigned short samplerate);
extern int i2s_pin_mux_sel(struct tiny_audio_device *adev, int type);
extern int adev_record_tone_stop(struct tiny_audio_device *adev);
extern int audio_offload_resume(struct tiny_audio_device *adev);
...
static int32_t GetAudio_mode_number_from_device(struct tiny_audio_device * adev, int outdev,int indev)
{
    int32_t lmode;
    if(((outdev & AUDIO_DEVICE_OUT_WIRED_HEADSET) && (outdev & AUDIO_DEVICE_OUT_SPEAKER))
        ...
        lmode = AUDIO_MODE_HEADSET;  //headset
    }else{
        ALOGW("%s device(0x%x) is not support, set default:handsfree \n",__func__,outdev);
        lmode = AUDIO_MODE_HANDSFREE;
    }

    if(adev->pSmartAmpMgr->isEnable && (adev->call_start == false)
        && (adev->fm_open == false) && (adev->pcm_fm_dl == false)){
        if(lmode == AUDIO_MODE_HANDSFREE){
            lmode = AUDIO_MODE_ISCHANDSFREE;//ISChandsfree
        } else if(lmode == AUDIO_MODE_HEADFREE){
            lmode = AUDIO_MODE_ISCHEADFREE;//ISCheadfree
        }
        ALOGI("%s, smartAmpMEnable:%d, current mode:%d", __func__, adev->pSmartAmpMgr->isEnable,lmode);
    }
...
static int  GetAudio_outpga_nv(struct tiny_audio_device *adev, AUDIO_TOTAL_T *aud_params_ptr, pga_gain_nv_t *pga_gain_nv, uint32_t vol_level)
{
    ...
    ptArmModeStruct = (AUDIO_NV_ARM_MODE_STRUCT_T *)(&(aud_params_ptr->audio_nv_arm_mode_info.tAudioNvArmModeStruct));
    pga_gain_nv->dac_pga_gain_l = aud_params_ptr->audio_nv_arm_mode_info.tAudioNvArmModeStruct.app_config_info_set.app_config_info[0].arm_volume[dac_vol];
    ...
    } else if (vol_level < FM_VOLUME_MAX) {
        AUDIO_NV_ARM_APP_SET_T *info_set =
            &aud_params_ptr->audio_nv_arm_mode_info.tAudioNvArmModeStruct.app_config_info_set;
        unsigned int *volume =
            info_set->app_config_info[AUDIO_NV_INFO_SET_FM_LINEIN_VOL].arm_volume;
        /* If set "HPL Playback Volume"/"HPR Playback Volume" to 0, then the
         * following call of codec_lowpower_open(adev, 0) will cause pop noise.
         * So change the volume to 1 here. set_linein_fm_mute() will
         * make the fm quiet.
         * NOTE: there is a precondition for "(volume[0] & 0xf)":
         * "xxx Playback Volume" must be stored in bit[0~3] of volume[].
         */
        if (vol_level == 0 && (volume[0] & 0xf) == 0 &&
            (adev->out_devices & (AUDIO_DEVICE_OUT_WIRED_HEADPHONE |
            AUDIO_DEVICE_OUT_WIRED_HEADSET)))
            volume[0] |= 1;
        pga_gain_nv->fm_pga_gain_l = volume[vol_level];
        pga_gain_nv->fm_pga_gain_r = pga_gain_nv->fm_pga_gain_l;
    }
...
static int  GetAudio_inpga_nv(struct tiny_audio_device *adev, AUDIO_TOTAL_T *aud_params_ptr, pga_gain_nv_t *pga_gain_nv, uint32_t vol_level)
{
    ...
    if (adev->fm_open && adev->fm_record) {
        if (adev->fm_type != AUD_FM_TYPE_LINEIN_3GAINS) {
            ...
            unsigned int *volume =
                info_set->app_config_info[AUDIO_NV_INFO_SET_FM_LINEIN_VOL].arm_volume;
            pga_gain_nv->adc_pga_gain_l &= (1 << 15); //Keep Vdg Switch
            pga_gain_nv->adc_pga_gain_l |=
                (volume[vol_level] >> FM_LINEIN_VOL_DG_SHIFT) & FM_LINEIN_VOL_DG_MASK;
        }
...
static int GetAudio_PaConfig_by_devices(struct tiny_audio_device *adev, pga_gain_nv_t *pga_gain_nv,int out_dev,int in_dev)
{
    ...
    char * dev_name = NULL;
    lmode = GetAudio_mode_number_from_device(adev, out_dev, in_dev);
...
static int GetAudio_gain_by_devices(struct tiny_audio_device *adev, pga_gain_nv_t *pga_gain_nv, uint32_t vol_level,int out_dev,int in_dev)
{
    ...
    AUDIO_TOTAL_T * aud_params_ptr = NULL;
    char * dev_name = NULL;
    outmode = GetAudio_mode_number_from_device(adev, out_dev, in_dev);
    ...
    if (adev->fm_open &&
        (adev->fm_type == AUD_FM_TYPE_LINEIN_3GAINS || adev->fm_record == true)) {
...
static int SetAudio_PaConfig_by_devices(struct tiny_audio_device *adev, pga_gain_nv_t *pga_gain_nv)
{
    ...
    mode = GetAudio_mode_number_from_device(adev, pga_gain_nv->out_devices,pga_gain_nv->in_devices);
    if((adev->fm_open) && (mode != 1)) {
...
static int SetAudio_gain_by_devices(struct tiny_audio_device *adev, pga_gain_nv_t *pga_gain_nv)
{
    ...
    if((adev->fm_open) && (mode != 1)) {
        ...
        if(((pga_gain_nv->in_devices & AUDIO_DEVICE_IN_BUILTIN_MIC) ||
            (pga_gain_nv->in_devices & AUDIO_DEVICE_IN_BACK_MIC) ||
            (pga_gain_nv->in_devices & AUDIO_DEVICE_IN_WIRED_HEADSET)) &&
           adev->fm_record){
...
static void SetCall_ModePara(struct tiny_audio_device *adev,paras_mode_gain_t *mode_gain_paras)
{
    ...
    uint32_t pre_out_outdevice = 0x0;
    ...
    if(switch_hp_mic == 1){
        if(headset_no_mic()){
            ...
            if(access("/sys/class/switch/h2w/state", R_OK)!= 0){
                ALOGI("SetCall_ModePara /sys/class/switch/h2w/state is not exist out_devices:%d",adev->out_devices);
                if(adev->out_devices & AUDIO_DEVICE_OUT_WIRED_HEADPHONE){
                    MY_TRACE("%s 3-pole headset, need to open main-mic instead of hp-mic ",__func__);
                    switch_mic0 = 1;    /*open main-mic*/
                    switch_hp_mic = 0;
                }else{
                    MY_TRACE("%s 4-pole headset, need to open hp-mic ",__func__);
                    switch_mic0 = 0;
                    switch_hp_mic = 1;
                }
            }
    ...
    pre_out_outdevice = s_call_dl_devices;
    s_call_dl_devices = 0;
    s_call_ul_devices = 0;
    if(switch_earpice){
    ...
    ALOGI("voice:pre_out_outdevice:0x%x, now out_device:0x%x", pre_out_outdevice, s_call_dl_devices);
	/*CP send mute command to DSP, but DSP get command at least 50ms later;
	  and one none-mute package data is 20ms, so real mute will be done at least 70ms later actually.
	  we should wait here every time */
    //if((pre_out_outdevice != 0) && (s_call_dl_devices == 0))
        usleep(1000*100);
    ...
    for (i=0; i<(sizeof(switch_table)/sizeof(unsigned short));i++) {
        if(switch_table[i]){
            cur_setting = get_route_setting(adev, switch_device[i], 1);
            cur_depth = get_route_depth(adev, switch_device[i], 1);
            if (adev->mixer && cur_setting) {
                ALOGD("ON cur_setting name is %s  i=%d", cur_setting->ctl_name, i);
                ret=set_route_by_array_v2(adev->mixer,cur_setting, cur_depth, ctl_on,ctl_on_count,1);
                if(ret>0){
                    ctl_on_count = ret;
                }
            }
...
#endif
            cur_setting = get_route_setting(adev, switch_device[i], 0);
            cur_depth = get_route_depth(adev, switch_device[i], 0);
            if (adev->mixer && cur_setting) {
                ALOGD("OFF cur_setting name is %s  i=%d", cur_setting->ctl_name, i);
                set_route_by_array_v2(adev->mixer,cur_setting, cur_depth, ctl_on,ctl_on_count,0);
            }
        }
    }
#else
...
#endif /* endif ENABLE_DEVICES_CTL_ON */
    if((NULL!=adev->cp)&&(1==adev->cp->mic_default_channel)){
        ALOGD("do not close_adc_channel");
    }else{
        if (!(switch_mic0 && switch_mic1)) {
    ...
    /*
       ...
       */
    if(!adev->call_connected || (android_pre_device == 0)&& (s_call_dl_devices|s_call_ul_devices)){
        usleep(1000*100);
    }
    if (adev->vbc_access)
        mixer_ctl_set_value(adev->vbc_access, 0, 0);
    ALOGW("%s successfully, device: earpice(%s), headphone(%s), speaker(%s), Main_Mic(%s), Back_Mic(%s), hp_mic(%s), mode_indxe: %#x"
            ,__func__,switch_earpice ? "Open":"Close",switch_headset ? "Open":"Close",switch_speaker ? "Open":"Close",
            switch_mic0 ? "Open":"Close",switch_mic1 ? "Open":"Close",switch_hp_mic ? "Open":"Close", mode_gain_paras->mode_index);
}
...
int SetParas_Samplerate_Incall(int fd_pipe,struct tiny_audio_device *adev, unsigned int rate)
{
    int ret = 0;
    ...
    if(adev->pcm_modem_dl){
        ret = pcm_set_samplerate(adev->pcm_modem_dl, PCM_OUT, &pcm_config_vx, rate);
        if(ret < 0){
            pthread_mutex_unlock(&adev->vbc_dlulock);
            return ret;
        }
        ret = pcm_set_samplerate(adev->pcm_modem_ul, PCM_IN, &pcm_config_vrec_vx, rate);
        if(ret < 0){
            pthread_mutex_unlock(&adev->vbc_dlulock);
            return ret;
        }
 ...
 void set_virt_output_switch(struct tiny_audio_device *adev, bool on)
{
    struct mixer_ctl *ctl;

	ctl = mixer_get_ctl_by_name(adev->mixer, PRIVATE_VIRT_OUTPUT_SWITCH);
	if (ctl)
		mixer_ctl_set_value(ctl, 0, !!on);
}

static void  codec_da_shutdown_enable_check(struct tiny_audio_device *adev)
{
    if(s_call_dl_devices) {
        set_virt_output_switch(adev,true);
    }
    else {
        set_virt_output_switch(adev,false);
    }
}
...
void vbc_ctrl_init(struct tiny_audio_device *adev)
{
    ...
    if(!result)
    {
        ...
        if(st_vbc_ctrl_thread_para)
        {
            ...
            st_vbc_ctrl_thread_para->isExit = false;
        }
    }
...
int vbc_ctrl_open(struct tiny_audio_device *adev)
{
    ...
    if(access("data/local/media/vb_voice_fake_pipe", R_OK) != 0){
        if (mkfifo("data/local/media/vb_voice_fake_pipe",S_IFIFO|0666) <0) {
            if (errno != EEXIST) {
        ...
        if(chmod("data/local/media/vb_voice_fake_pipe", S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH) != 0) {
            LOG_E("%s Cannot set RW to \"%s\": %s", __FUNCTION__,"vb_voice_fake_pipe", strerror(errno));
        }
    }

    if (vb_voice_fake_pipe < 0) {
        vb_voice_fake_pipe = open("data/local/media/vb_voice_fake_pipe", O_RDWR);
    }
...
int vbc_ctrl_close()
{
    ...
    while(i<s_vbc_pipe_count)
    {
        st_vbc_ctrl_thread_para[i].isExit = true;
        thread_id = s_vbc_ctrl_thread_id[i];
        w_size = write(vb_voice_fake_pipe, fake, sizeof(fake));
        if (w_size > 0) {
    ...
    close(vb_voice_fake_pipe);
    vb_voice_fake_pipe = -1;
    free(s_vbc_ctrl_thread_id);
    s_vbc_ctrl_thread_id = NULL;
    free(st_vbc_ctrl_thread_para);
    st_vbc_ctrl_thread_para = NULL;
    s_vbc_pipe_count = 0;
...
int vbc_ctrl_voip_close(struct voip_res *res)
{
    int w_size = 0;
    char fake[5] = "fake";
    ...
    if (!res)
        return 0;
    res->isExit = true;
    w_size = write(vb_voip_fake_pipe, fake, sizeof(fake));
    if (w_size  > 0) {
        pthread_join(res->thread_id, NULL);
    }

    close(vb_voip_fake_pipe);
    vb_voip_fake_pipe = -1;
    return 0;
}
...
int vbc_ctrl_voip_open(struct voip_res *res)
{
   ...
   if(!res->enable){
    ...
    if(access("data/local/media/vb_voip_fake_pipe", R_OK) != 0){
        if (mkfifo("data/local/media/vb_voip_fake_pipe",S_IFIFO|0666) <0) {
            if (errno != EEXIST) {
                LOG_E("%s create audio fifo error %s\n",__FUNCTION__,strerror(errno));
                return -1;
            }
        ...
        if(chmod("data/local/media/vb_voip_fake_pipe", S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH) != 0) {
            LOG_E("%s Cannot set RW to \"%s\": %s", __FUNCTION__,"vb_voip_fake_pipe", strerror(errno));
        }
    }

    if (vb_voip_fake_pipe < 0) {
        vb_voip_fake_pipe = open("data/local/media/vb_voip_fake_pipe", O_RDWR);
    }
...
static int vbc_call_end_process(struct tiny_audio_device *adev,int is_timeout)
{
    ...
    if(is_timeout) {
        if (adev->voip_state != 0)
            mixer_ctl_set_value(adev->private_ctl.vbc_switch,
            0, VBC_ARM_CHANNELID);  //switch vbc to arm
        adev->call_prestop = 0;
        adev->call_start = 0;
...
#ifdef NXP_SMART_PA
static void vbc_ctrl_nxp_tfa_open(struct tiny_audio_device *adev)
{
    ALOGE("peter_voice: in, devices: %#x", s_call_dl_devices);
    ALOGE("peter_voice: 2");
    if(!adev->callHandle) {
        i2s_pin_mux_sel_new(adev, EXT_CODEC, EXT_CODEC_CP);
        ALOGE("peter_voice: 4");
        set_ext_codec(adev, SCENE_CALL);
        adev->callHandle = NxpTfa_Open(&NxpTfa9890_Call_Info, 0, 1);
        ALOGE("peter_voice: 5");
    }
    ALOGE("peter_voice: 3");
}

static void vbc_ctrl_nxp_tfa_close(struct tiny_audio_device *adev)
{
    if(adev->callHandle)
    {
...
RESTART:
    ...
    maxfd = (para->vbpipe_fd >  vb_voip_fake_pipe) ? para->vbpipe_fd + 1 : vb_voip_fake_pipe + 1;
    if((fcntl(para->vbpipe_fd,F_SETFL,O_NONBLOCK))<0)
    {
        ALOGD("voip1:vbpipe_name(%s) vbpipe_fd(%d) fcntl error.", para->vbpipe, para->vbpipe_fd);
    }

    if((fcntl(vb_voip_fake_pipe,F_SETFL,O_NONBLOCK))<0)
    {
        ALOGD("voip1:vbpipe_name(%s) vb_voip_fake_pipe(%d) fcntl error.", para->vbpipe, vb_voip_fake_pipe);
    }
    ...
    while(!res->isExit)
    {
        ...
        FD_SET(vb_voip_fake_pipe,&fds_read);
        ...
        vbc_empty_pipe(vb_voip_fake_pipe);
        cur_timeout = NULL;
        continue;
        }
        if(FD_ISSET(para->vbpipe_fd,&fds_read) <= 0) {
            if (FD_ISSET(vb_voip_fake_pipe,&fds_read)) {
                vbc_empty_pipe(vb_voip_fake_pipe);
                cur_timeout = NULL;
                continue;
            } else {
    ...
    ALOGD("voip1: Get CMD(%d) from cp(pipe:%s, pipe_fd:%d,paras_size:%d devices:0x%x mode:%d",
        ...
        switch (read_common_head.cmd_type)
        {
            case VBC_CMD_HAL_OPEN:
            {
                ...
                if (NULL != adev->i2s_btcall_info->i2s_ctl_info) {
                    i2s_ctl_t * i2s_ctl_info = adev->i2s_btcall_info->i2s_ctl_info + adev->cp->vbc_ctrl_pipe_info->cpu_index;
                    i2s_ctl = ((i2s_ctl_info->is_switch << 8) | (i2s_ctl_info->i2s_index << 0) );
                } else {
                    i2s_ctl = 0;
                }
#ifdef NXP_SMART_PA
                /* Tell modem that there is an externel codec. */
                if (adev->smart_pa_exist)
                    i2s_ctl |= (1 << 10);
#endif
#endif
...
#ifdef NXP_SMART_PA
                /*
                * When receive VBC_CMD_RSP_CLOSE, i2s clock has been switched off.
                * However, in some specail case, there is no VBC_CMD_HAL_CLOSE cmd.
                * So add close operation here, too.
                */
                if (adev->smart_pa_exist)
                    vbc_ctrl_nxp_tfa_close(adev);
#endif
            ...
            case VBC_CMD_SET_MODE:
            {
                ...
                codec_da_shutdown_enable_check(adev);
#ifdef NXP_SMART_PA
                if(adev->smart_pa_exist && !(s_call_dl_devices & AUDIO_DEVICE_OUT_SPEAKER))
                    vbc_ctrl_nxp_tfa_close(adev);
#endif
        ...
        case VBC_CMD_SET_SAMPLERATE:
            ...
            set_samplerate_t device_set_samplerate = {0};

            ret = GetParas_Samplerate_Incall(para->vbpipe_fd,&device_set_samplerate);
            if (ret <= 0)
                goto set_rate_responce;

            ALOGI("device_set_samplerate_>samplerate is  %d ",device_set_samplerate.samplerate);

#ifdef NXP_SMART_PA
            if (adev->smart_pa_exist) {
                if (!(s_call_dl_devices & AUDIO_DEVICE_OUT_SPEAKER) &&
                    adev->internal_codec_rate == device_set_samplerate.samplerate) {
                    ALOGI("Set the same rate: %d ", device_set_samplerate.samplerate);
                    goto set_rate_responce;
                }
                adev->internal_codec_rate = device_set_samplerate.samplerate;
            }
            ...
            ret = SetParas_Samplerate_Incall(para->vbpipe_fd,adev,device_set_samplerate.samplerate);
            if(ret < 0){
...
void *vbc_ctrl_thread_routine(void *arg)
{
    ...
    write_common_head.cmd_type = VBC_CMD_NONE;
    write_common_head.paras_size = 0;
    ALOGI("voice:vbc_ctrl_thread_routine in pipe_name:%s.", para->vbpipe);
...
RESTART:
    ...
    if (para->vbpipe_fd < 0) {
        para->vbpipe_fd = open(para->vbpipe, O_RDWR);//open("/dev/vbpipe6", O_RDWR);
        if (para->vbpipe_fd < 0) {
            ALOGI("vbc_ctrl_thread_routine open fail, pipe_name:%s, %d.", para->vbpipe, errno);
            sleep(1);
            goto RESTART;
        } else {
    ...
    maxfd = (para->vbpipe_fd > vb_voice_fake_pipe) ? para->vbpipe_fd + 1 : vb_voice_fake_pipe + 1;
    if((fcntl(para->vbpipe_fd,F_SETFL,O_NONBLOCK))<0)
    {
        ALOGD("voice:vbpipe_name(%s) vbpipe_fd(%d) fcntl error.", para->vbpipe, para->vbpipe_fd);
    }

    if((fcntl(vb_voice_fake_pipe,F_SETFL,O_NONBLOCK))<0)
    {
        ALOGD("voice:vbpipe_name(%s) vb_voice_fake_pipe(%d) fcntl error.", para->vbpipe, vb_voice_fake_pipe);
    }
    ...
    while(!para->isExit)
    {
        ...
        FD_ZERO(&fds_read);
        FD_SET(para->vbpipe_fd,&fds_read);
        FD_SET(vb_voice_fake_pipe,&fds_read);
        result = select(maxfd,&fds_read,NULL,NULL,cur_timeout);
        if(result < 0) {
	    ...
	    vbc_call_end_process(adev,true);
	    vbc_empty_pipe(para->vbpipe_fd);
        vbc_empty_pipe(vb_voice_fake_pipe);
	    cur_timeout = NULL;
	    continue;
        }
        if(FD_ISSET(para->vbpipe_fd,&fds_read) <= 0) {
            if (FD_ISSET(vb_voice_fake_pipe,&fds_read)) {
                vbc_empty_pipe(vb_voice_fake_pipe);
                cur_timeout = NULL;
                continue;
            } else {
        ...
        /* read parameters common head of the packet.*/
        ret = ReadParas_Head(para->vbpipe_fd, &read_common_head);
        ALOGW("voice:VBC_CMD_HAL_get_cmd try vbc_lock, pipe_name:%s, ret:%d.", para->vbpipe, ret);
        if(ret < 0) {
	...
	case VBC_CMD_HAL_OPEN:
        ...
        if (adev->smart_pa_exist) {
            i2s_ctl |= (1 << 10);
            adev->internal_codec_rate = pcm_config_vx.rate;
        }
#endif
		cur_timeout = &timeout;
		...
		if(adev->out_devices & AUDIO_DEVICE_OUT_ALL_SCO) {
	        ...
	        if(adev->hfp_start){
	            if(adev->i2s_btcall_info->i2s_ctl_info) {
                        FILE *i2s_handle;
                        char * i2s_ctrl_value="1";
                        i2s_handle = fopen("/proc/pin_switch/iis0_sys_sel/cp1_iis0","wb");
                        if(i2s_handle  != NULL) {
                            fputs(i2s_ctrl_value,i2s_handle);
                            fclose(i2s_handle);
                        }
                    } else {
                        i2s_ctl = 0x100;
                        i2s_pin_mux_sel(adev,PUBCP);
                    }
		}
	...
	case VBC_CMD_RSP_CLOSE:
		...
		if(adev->hfp_start) {
		    if(adev->i2s_btcall_info->i2s_ctl_info) {
		        FILE *i2s_handle;
		        char * i2s_ctrl_value="1";
		        i2s_handle = fopen("/proc/pin_switch/iis0_sys_sel/ap_iis0","wb");
		        if(i2s_handle  != NULL) {
		            fputs(i2s_ctrl_value,i2s_handle);
		            fclose(i2s_handle);
		        }
	...
	case VBC_CMD_SET_MODE:
		...
		if(adev->routeDev & AUDIO_DEVICE_OUT_ALL_SCO) {
		    if(adev->cp_type == CP_TG)
			    i2s_pin_mux_sel(adev,1);
            else if(adev->cp_type == CP_W)
			    i2s_pin_mux_sel(adev,0);
	...
	case VBC_CMD_SET_SAMPLERATE:
		...
		{
		    ALOGE("pcm dl stop unsucessfully");
		}
		if(( NULL != adev->pcm_modem_ul ) && (0 != pcm_stop(adev->pcm_modem_ul)))
		{
		    ALOGE("pcm ul stop unsucessfully");
		}
		pthread_mutex_unlock(&adev->vbc_dlulock);
...
static void do_select_devices_l(struct tiny_audio_device *adev,int devices_out_l)
{
    ...
    /* update EQ profile*/
    if(adev->eq_available){
        int mode = GetAudio_mode_number_from_device(adev, devices_out_l, adev->in_devices);
...
reconnect:
    ...
    maxfd = fd > adev->modem_monitor_mgr.fake_fd ? fd + 1 : adev->modem_monitor_mgr.fake_fd + 1;

    while(!adev->modem_monitor_mgr.is_entry_exit)
    {

        FD_ZERO(&fds_read);
        FD_SET(fd,&fds_read);
        FD_SET(adev->modem_monitor_mgr.fake_fd,&fds_read);
        int result = 0;

        result = select(maxfd,&fds_read,NULL,NULL,cur_timeout);
        if(result < 0) {
            ALOGE("vbc_ctl_modem_monitor_routine: select error %d",errno);
            cur_timeout = NULL;
            continue;
        }
        else if(!result) {
            ALOGE("vbc_ctl_modem_monitor_routine: select timeout");
            cur_timeout = NULL;
            continue;
        }

        if(FD_ISSET(fd,&fds_read) <= 0) {
            if (FD_ISSET(adev->modem_monitor_mgr.fake_fd,&fds_read)) {
                numRead = read(adev->modem_monitor_mgr.fake_fd, &buf, sizeof(buf));
                cur_timeout = NULL;
                continue;
            } else {
                ALOGE("voip1: select ok but no fd is set");
...
int vb_ctl_modem_monitor_close(void * arg)
{
    char fake[5] = {0};
    int w_size = 0;
    struct tiny_audio_device *adev = (struct tiny_audio_device *)arg ;
    ALOGD("vb_ctl_modem_monitor_close in");
    if( !adev) {
        ALOGD("vb_ctl_modem_monitor_close:error adev is null");
        return -1;
    }
    adev->modem_monitor_mgr.is_entry_exit = true;
    w_size = write(adev->modem_monitor_mgr.fake_fd, fake, sizeof(fake));
    if (w_size > 0) {
        pthread_join(adev->modem_monitor_mgr.thread_id, (void **) NULL);
    }
    close(adev->modem_monitor_mgr.fake_fd);
    adev->modem_monitor_mgr.fake_fd = -1;
    return 0;
}
...
int vb_ctl_modem_monitor_open(void * arg)
{
    ...
    memset(&adev->modem_monitor_mgr, 0 , sizeof(vb_ctl_modem_monitor_mgr));
    adev->modem_monitor_mgr.is_entry_exit = false;
    adev->modem_monitor_mgr.fake_fd = -1;

    if(access("data/local/media/vb_monitor_fake_pipe", R_OK) != 0){
        if (mkfifo("data/local/media/vb_monitor_fake_pipe",S_IFIFO|0666) <0) {
            if (errno != EEXIST) {
                LOG_E("%s create audio fifo error %s\n",__FUNCTION__,strerror(errno));
                return -1;
            }
        ...
        if(chmod("data/local/media/vb_monitor_fake_pipe", S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH) != 0) {
            LOG_E("%s Cannot set RW to \"%s\": %s", __FUNCTION__,"vb_monitor_fake_pipe", strerror(errno));
        }
    }

    do {
        fd = open("data/local/media/vb_monitor_fake_pipe", O_RDWR);
        if (fd < 0) {
            LOG_E("%s open vb monitor fifo error %s\n",__FUNCTION__,strerror(errno));
        }
    } while (fd < 0);

    adev->modem_monitor_mgr.fake_fd = fd;

    rc = pthread_create(&adev->modem_monitor_mgr.thread_id, NULL,
