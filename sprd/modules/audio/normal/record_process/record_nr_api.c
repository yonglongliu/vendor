...
//Record Nr Init
record_nr_handle AudioRecordNr_Init(int16 *pNvPara, CallBack read_pcm_data, void *data_read_handle,int request_channel)
{
    ...
    ALOGD("Audio Nr Nv Info: 0x%x   0x%x    0x%x   %d    %d   %d   %d   %d", pNvInfo[0], pNvInfo[1],
...
static void AudioRecordNr_ProcEx(record_nr_handle nr_hande, void * buffer, size_t bytes, int channel)
{
    ...
    do {
        ...
        bytes -= read_bytes;
        ALOGV("AudioRecordNr_ProcEx byres = %d, read_bytes = %d",bytes,read_bytes);
        if(channel == 1)//one channel,left buffer same as right buffer
        {
            ...
            AUDPROC_ProcessDpEx(buffLeft, buffRight, read_bytes/2, temp_buf, temp_buf + (read_bytes/2), &dest_count);
            ALOGV("AudioRecordNr_ProcEx 1  read_byres  = %d, dest_count = %d",read_bytes,dest_count);
            for ( i = 0;  i < read_bytes/2; i++)
            {
        ...
        else//channel == 2
        {
            ...
            AUDPROC_ProcessDpEx( buffLeft, buffRight, read_bytes/4, temp_buf, temp_buf +( read_bytes/4), &dest_count);
            ALOGV("AudioRecordNr_ProcEx 2 read_byres  = %d, dest_count = %d",read_bytes,dest_count);
            for ( i = 0;  i < read_bytes/4; i++)
            {
...
int AudioRecordNr_Proc(record_nr_handle nr_hande, void* buffer, size_t bytes)
{
    ...
    {
        memcpy((char *)buffer, (char *)record_nr_res->stage_buffer, record_nr_res->stage_size);
        ...
        while(1)
        {
            ...
            {
                memcpy((char *)buffer +transfered_size, (char *)record_nr_res->read_buffer, unit);
                transfered_size += unit;
            }
            else
            {
                memcpy((char *)buffer +transfered_size, (char *)record_nr_res->read_buffer, bytes-transfered_size);
                //storage the data left
                new_storage_size = transfered_size + unit-bytes;//update the storage size
                memcpy((char *)record_nr_res->stage_buffer, (char *)record_nr_res->read_buffer+(unit - new_storage_size), new_storage_size);
    ...
    else
    {
        ...
        memcpy((char *)record_nr_res->stage_buffer, (char *)record_nr_res->stage_buffer+bytes, record_nr_res->stage_size);
    }
    return ret;
err:
    return -1;
