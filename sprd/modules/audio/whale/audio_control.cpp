...
static uint8_t get_record_param(int input_source, audio_devices_t in_devices);
...
{
    int ret=0;
    LOG_I("device_access_enable usecase %#x agdsp_sleep_status %d", actl->usecase, actl->agdsp_ctl->agdsp_sleep_status);
    if(false == actl->agdsp_ctl->agdsp_sleep_status) {
        ret = dsp_sleep_ctrl(actl->agdsp_ctl,true);
        if (0 == ret) {
            actl->access_set ++;
        }
        LOG_I("device_access_enable dsp_sleep_ctrl_l true  ret = %d actl->access_set=%d", ret, actl->access_set);
    }else{
        ret = 0;
    }
    return ret;
...
static int device_access_restore(struct audio_control *actl)
{
    ...
    LOG_E("device_access_restore agdsp_sleep_status %d", actl->agdsp_ctl->agdsp_sleep_status);
    if(true == actl->agdsp_ctl->agdsp_sleep_status) {
        if(actl->access_set == 1) {
            ret = dsp_sleep_ctrl(actl->agdsp_ctl,false);
            if (ret)
                LOG_E("%s dsp_sleep_ctrl_l false failed", __func__);
            actl->access_set = 0;
            LOG_E("device_access_restore dsp_sleep_ctrl_l false  ret = %d  actl->access_set=%d", ret, actl->access_set);
       } else {
            if(actl->access_set > 1)
                actl->access_set--;
        }
...
static int usecase_to_stream_type(int usecase){
    ...
    switch(usecase){
        ...
        case UC_VOIP:
            ...
            break;
        case UC_BT_VOIP :
            stream_type = AUDIO_HW_APP_VOIP_BT;
            break;
...
static int get_stream_type_with_pcm_dev(int pcm_devices){
    ...
    switch(pcm_devices){
        ...
        case AUD_PCM_DEEP_BUFFER:
            ...
            break;
        case AUD_PCM_OFF_LOAD:
            audio_app_type = AUDIO_HW_APP_OFFLOAD;
            break;
        case AUD_PCM_MODEM_UL:
        ...
        case AUD_PCM_DSP_LOOP:
            ...
            break;
        case AUD_PCM_BT_RECORD:
            audio_app_type = AUDIO_HW_APP_BT_RECORD;
            break;
        default:
...
static int get_usecase_with_mdg(struct audio_control *actl,struct mixer_ctrl_t * mdg){
    ...
    if(&mute->dsp_da0_mdg_mute == mdg){
        pcm_devices = (1<<VBC_PCM_DEV_2)|(1<<VBC_PCM_DEV_5);
    }

    if(&mute->dsp_da1_mdg_mute == mdg){
        pcm_devices = (1<<VBC_PCM_DEV_1)|(1<<VBC_PCM_DEV_7)|(1<<VBC_PCM_DEV_9);
    }

    if(&mute->audio_mdg_mute == mdg){
        pcm_devices = (1<<VBC_PCM_DEV_0);
    }
...
int set_mdg_mute(struct audio_control *actl,int usecase,bool on){
    ...
    if(on==false){
        ...
        if(false == is_usecase_unlock(actl, mdg_usecase&(~usecase))){
            LOG_I("set_mdg_mute, usecase:%x, on:%d",usecase, on);
            ret = _set_mdg_mute(mdg_ctl,on);
        }
    }
...
int enable_dspsleep_for_fm(struct audio_control *actl)
{
    int ret = -1;
    pthread_mutex_lock(&actl->lock);
    if(actl->usecase == UC_FM) {
         LOG_I("enable_dspsleep_for_fm");
        ret = dsp_sleep_ctrl(actl->agdsp_ctl,false);
    }
    pthread_mutex_unlock(&actl->lock);
    return ret;
...
int select_devices_new(struct audio_control *actl, AUDIO_HW_APP_T audio_app_type, audio_devices_t device, bool is_in, bool update_param, bool sync)
{
    ...
    LOG_I("select_devices_new devices 0x%x, is in %d app type:%d sync is %d", device, is_in,audio_app_type, sync);
...
int dev_ctl_get_out_pcm_config(struct audio_control *dev_ctl,int app_type, int * dev, struct pcm_config *config)
{
    ...
    if(config == NULL){
     ...
     switch(app_type){
        case AUDIO_HW_APP_OFFLOAD:
             pcm_devices=dev_ctl->pcm_handle.playback_devices[AUD_PCM_OFF_LOAD];
             memcpy(config, &dev_ctl->pcm_handle.play[AUD_PCM_OFF_LOAD], sizeof(struct pcm_config));
             LOG_I("dev_ctl_get_out_pcm_config UC_OFFLOAD:%p %d %d",config,config->period_count,config->period_size)
             break;
        case AUDIO_HW_APP_CALL:
...
void set_ctl_source(struct audio_control *control, audio_source_t source){
    if(source > AUDIO_SOURCE_MAX || control== NULL){
        ALOGE("the input source is error!");
        return;
    }
...
static uint8_t get_record_param(int input_source, audio_devices_t in_devices)
{
    ...
    if(!(in_devices&AUDIO_DEVICE_BIT_IN))
        return -1;

    if(AUDIO_SOURCE_UNPROCESSED == input_source && (AUDIO_DEVICE_IN_BUILTIN_MIC == in_devices
        || AUDIO_DEVICE_IN_BACK_MIC == in_devices)){
        strcat((char *)name ,"Music\\Handsfree\\extRecord");
    } else {
...
{
    uint8_t param_id =get_record_param(audio_param->input_source, in_devices);
...
int select_audio_param_unlock(struct audio_control *dev_ctl,struct audio_param_res  *param_res){
    ...
    if(is_voice_active(param_res->usecase)) {
        ...
        if ((is_call_active(param_res->usecase)) && (UC_MM_RECORD&param_res->usecase)){
            param_id = get_record_param(dev_ctl->audio_param->input_source, param_res->in_devices);
            if(param_id>=PROFILE_MODE_MAX){
        ...
        if(is_fm_active(param_res->usecase)){
            uint8_t music_param_id=PROFILE_MODE_MAX;
            ...
            if(PROFILE_MODE_MUSIC_Handsfree_FM==param_id){
                music_param_id=PROFILE_MODE_MUSIC_Handsfree_Playback;
            }else if(PROFILE_MODE_MUSIC_Headset_FM==param_id){
                music_param_id=PROFILE_MODE_MUSIC_Headset_Playback;
            }

            if(music_param_id<PROFILE_MODE_MAX){
                offset=(uint8_t)get_audio_param_offset(dev_ctl->audio_param,SND_AUDIO_PARAM_DSP_VBC_PROFILE_DSP,music_param_id);

                if(AUDIO_PARAM_INVALID_8BIT_OFFSET!=offset){
                    ret = set_vbc_param(dev_ctl, offset, music_param_id,  dsp_case);
                }
            } else {
                LOG_W("get_fm_param failed!!!");
                return -1;
            ...
            if(is_playback_active(param_res->usecase) ) {
                param_id = get_playback_param(param_res->out_devices);
                if(param_id>=PROFILE_MODE_MAX){
                    LOG_W("get_playback_param failed %d", __LINE__);
                    return -1;
                }
                set_play_dg_param(dev_ctl, param_id, 0);
            }
