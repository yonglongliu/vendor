...
static size_t out_get_buffer_size(const struct audio_stream *stream)
{
    ...
    if((AUDIO_HW_APP_VOIP == out->audio_app_type)
       ||(AUDIO_HW_APP_VOIP_BT == out->audio_app_type)){
...
static uint32_t out_get_latency(const struct audio_stream_out *stream)
{
    ...
    ALOGI("latency=%d period_size=%d,period_count=%d,rate=%d",temp_latency,out->config->period_size,out->config->period_count,out->config->rate);
...
static int start_input_stream(struct tiny_stream_in *in)
{
    ...
    if(is_usecase_unlock(control, UC_MM_RECORD)){
        set_ctl_source(control, in->source);
    }
    ...
    if (in->requested_rate != in->config->rate && (in->audio_app_type == AUDIO_HW_APP_NORMAL_RECORD || in->audio_app_type == AUDIO_HW_APP_FM_RECORD)){
...
static int in_set_parameters(struct audio_stream *stream, const char *kvpairs)
{
    ...
    if (ret >= 0) {
        ...
        if(in->source != val){
            in->source = val;
            pthread_mutex_unlock(&in->lock);
            ALOGI("in_set_parameters, the current source(%d) has changed!", val);
            do_input_standby(in);
        }else{
            ALOGV("in_set_parameters, the current source(%d) no changed!", val);
            pthread_mutex_unlock(&in->lock);
        }
        //TODO
    }
