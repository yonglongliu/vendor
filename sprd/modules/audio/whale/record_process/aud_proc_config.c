...
#define EQ_PARA_SET_S2_SET_TIMES 20
        ...
        int32_t sample_rate)
    {
        ...
        SCI_TRACE_LOW("aud_proc_config.c,[AUDPROC_Seteq]-bass-:fo:%d  df:%d  boost:%d  gain:%d",
                      eq_param_ptr->bass_fo, eq_param_ptr->bass_df, eq_param_ptr->bass_boostdB,
                      eq_param_ptr->bass_gaindB );
        ...
        SCI_TRACE_LOW("aud_proc_config.c,[AUDPROC_Seteq]-treble-:fo:%d  df:%d  boost:%d  gain:%d",
                      eq_param_ptr->treble_fo, eq_param_ptr->treble_df, eq_param_ptr->treble_boostdB,
                      eq_param_ptr->treble_gaindB );
        ...
        int32_t sample_rate)
    {
        ...
        if(REC_FILTER_LCFPARA_BUTTERWORTH == lcf_para_set.eLcfParaType) {
            ...
            SCI_TRACE_LOW("aud_proc_config.c,[AUDPROC_Initlcf] lcf type is %d.Not support!",
                          lcf_para_set.eLcfParaType);
        ...
        int32_t sample_rate)
    {
        ...
        if(AGC_input_Gain > 1024) {
            gd_Attack  = (AGC_input_Gain * 1024 - 1024 * 1024) / (Fs * AGC_attack /
                         1000); //scaled by 1024*1024 int32_t
            gd_Release = (AGC_input_Gain * 1024 - 1024 * 1024) / (Fs * AGC_release /
                         1000); //scaled by 1024*1024 int32_t

        } else {
            AGC_SW_m     =  FALSE; //AGC sw off automatic

            gd_Attack  = (AGC_input_Gain * 1024) / (Fs * AGC_attack /
                                                    1000); //scaled by 1024*1024 int32_t
            gd_Release = (AGC_input_Gain * 1024) / (Fs * AGC_release /
                                                    1000); //scaled by 1024*1024 int32_t
        }
    ...
    BOOLEAN AUDPROC_SetAgcDg( int16_t digtal_gain)
    {
        ...
        SCI_TRACE_LOW("aud_proc_config.c,[AUDPROC_SetAgcDg] digtal_gain:%d",
                      digtal_gain);
        ...
        int32_t       Fs)
    {
        ...
        RECORD_EQ_SW     = ((recordeq_param_ptr->RECORDEQ_sw_switch) &
                            (1 << 15)) ? TRUE : FALSE;//bit 15
        RECORD_EQ_STEREO = ((recordeq_param_ptr->RECORDEQ_sw_switch) &
                            (1 << 14)) ? TRUE : FALSE;//bit 14
        ...
        if(RECORD_EQ_SW) {
            for(i = 0; i < RECORDEQ_MAX_BAND; i++) { //i :band index
                ...
                RECORD_EQ_BAND_SW[i]  = ((recordeq_param_ptr->RECORDEQ_sw_switch) & (1 <<
                                         (8 + i))) ? TRUE : FALSE; //bit (8+i);
        ...
        int32_t       Fs)
    {
        ...
        if(DP_input_gain > 1024) {
            gd_Attack_Compressor  = ((DP_input_gain * 1024 - 1024 * 1024) /
                                     (Fs * COMPRESSOR_attack / 1000)); //%scaled by 1024*1024 int32_t
            gd_Release_Compressor = ((DP_input_gain * 1024 - 1024 * 1024) /
                                     (Fs * COMPRESSOR_release / 1000)); //%scaled by 1024*1024 int32_t

            gd_Attack_Expander  = ((DP_input_gain * 1024 - 1024 * 1024) /
                                   (Fs * EXPANDER_attack / 1000)); //%scaled by 1024*1024 int32_t
            gd_Release_Expander = ((DP_input_gain * 1024 - 1024 * 1024) /
                                   (Fs * EXPANDER_release / 1000)); //%scaled by 1024*1024 int32_t
        } else {
            gd_Attack_Compressor  = ((DP_input_gain * 1024) / (Fs * COMPRESSOR_attack /
                                     1000)); //%scaled by 1024*1024 int32_t
            gd_Release_Compressor = ((DP_input_gain * 1024) / (Fs * COMPRESSOR_release /
                                     1000)); //%scaled by 1024*1024 int32_t

            gd_Attack_Expander  = ((DP_input_gain * 1024) / (Fs * EXPANDER_attack /
                                   1000)); //%scaled by 1024*1024 int32_t
            gd_Release_Expander = ((DP_input_gain * 1024) / (Fs * EXPANDER_release /
                                   1000)); //%scaled by 1024*1024 int32_t
        }
        ...
        for(si = 0; si < uiSrcCount; si++) { //AUDIO POST PROCESS
            ...
            if(DP_SW) {
                ...
                if(DP_in_power > COMPRESSOR_threshold) { //%     tc < in     COMPRESSOR
                    ...
                    dp_out_power = COMPRESSOR_threshold + ((DP_in_power - COMPRESSOR_threshold) *
                                                           COMPRESSOR_ratio >> 15);
                    if(dp_out_power > DP_limit_up)
                    { dp_out_power = DP_limit_up ; }
                    agc_gain_p_dp = DP_input_gain * F32768power10(DP_in_power - dp_out_power) >>
                                    5; //%scaled by 1024*1024
                } else if(DP_in_power >= EXPANDER_threshold) { //%    te <= in <= tc
                    agc_gain_p_dp = DP_input_gain << 10;
                } else {                                  //%    in < te     EXPANDER
                    dp_is_expander = TRUE;
                    dp_out_power = EXPANDER_threshold - ((EXPANDER_threshold - DP_in_power) << 15) /
                                   EXPANDER_ratio;
            ...
            if(RECORD_EQ_SW) { // xout_L xout_R ==>>  out_left & out_right
                for(i = 0; i < RECORDEQ_MAX_BAND; i++) { //i :band index
                    if(RECORD_EQ_BAND_SW[i]) {
                        ...
                        //step1
                        xout_L   =  CUR_RECORD_EQ_L_D1[i] + (xin_L_H * s_cur_recordeq_para[i][0] << 1)
                                    +  ((xin_L_L * s_cur_recordeq_para[i][0] + 8192) >> 14) ;
                        xout_L_H =  xout_L >> 15;
                        xout_L_L = xout_L - (xout_L_H << 15);

                        //step2
                        CUR_RECORD_EQ_L_D1[i] =  CUR_RECORD_EQ_L_D2[i]  + (xin_L_H *
                                                 s_cur_recordeq_para[i][1] << 1) + ((xin_L_L * s_cur_recordeq_para[i][1] + 8192)
                                                         >> 14)  + (xout_L_H * s_cur_recordeq_para[i][3] << 1) + ((
                                                                 xout_L_L * s_cur_recordeq_para[i][3] + 8192) >> 14);

                        //step3
                        CUR_RECORD_EQ_L_D2[i] =               (xin_L_H * s_cur_recordeq_para[i][2] << 1)
                                                              + ((xin_L_L * s_cur_recordeq_para[i][2] + 8192) >> 14)  +
                                                              (xout_L_H * s_cur_recordeq_para[i][4] << 1) + ((xout_L_L *
                                                                      s_cur_recordeq_para[i][4] + 8192) >> 14);
                    }
                }
                ...
                if(RECORD_EQ_STEREO) {
                    for(i = 0; i < RECORDEQ_MAX_BAND; i++) { //i :band index
                        if(RECORD_EQ_BAND_SW[i]) {
                            ...
                            //step1
                            xout_R   =  CUR_RECORD_EQ_R_D1[i] + (xin_R_H * s_cur_recordeq_para[i][0] << 1)
                                        + ((xin_R_L * s_cur_recordeq_para[i][0] + 8192) >> 14) ;
                            xout_R_H = xout_R >> 15;
                            xout_R_L = xout_R - (xout_R_H << 15);

                            //step2
                            CUR_RECORD_EQ_R_D1[i] =  CUR_RECORD_EQ_R_D2[i]  + (xin_R_H *
                                                     s_cur_recordeq_para[i][1] << 1) + ((xin_R_L * s_cur_recordeq_para[i][1] + 8192)
                                                             >> 14)  + (xout_R_H * s_cur_recordeq_para[i][3] << 1) + ((
                                                                     xout_R_L * s_cur_recordeq_para[i][3] + 8192) >> 14);

                            //step3
                            CUR_RECORD_EQ_R_D2[i] =               (xin_R_H * s_cur_recordeq_para[i][2] << 1)
                                                                  + ((xin_R_L * s_cur_recordeq_para[i][2] + 8192) >> 14)  +
                                                                  (xout_R_H * s_cur_recordeq_para[i][4] << 1) + ((xout_R_L *
                                                                          s_cur_recordeq_para[i][4] + 8192) >> 14);
            ...
            if(LCF_SW_dp) { // out_left & out_right ==>>  out_left & out_right
                ...
                xout_L     =  CUR_LCF_L_D1_dp  + (xin_L_H * s_cur_lcf_para_left_dp[0] << 1) + ((
                                  xin_L_L * s_cur_lcf_para_left_dp[0] + 8192) >> 14);
                if(xout_L > ((1 << 30) - 1)) {
                    xout_L = ((1 << 30) - 1);
                } else if(xout_L < -(1 << 30)) {
                    xout_L = -(1 << 30);
                ...
                CUR_LCF_L_D1_dp =  CUR_LCF_L_D2_dp  + (xin_L_H * (-s_cur_lcf_para_left_dp[0]) <<
                                                       2) + (((xin_L_L * (-s_cur_lcf_para_left_dp[0]) << 1) + 8192) >> 14)  -
                                   (xout_L_H * s_cur_lcf_para_left_dp[1] << 1) - ((xout_L_L *
                                           s_cur_lcf_para_left_dp[1] + 8192) >> 14);
                CUR_LCF_L_D2_dp =                  (xin_L_H * s_cur_lcf_para_left_dp[0] << 1)
                                                   + ((xin_L_L * s_cur_lcf_para_left_dp[0] + 8192) >> 14)  -
                                                   (xout_L_H * s_cur_lcf_para_left_dp[2] << 1) - ((xout_L_L *
                                                           s_cur_lcf_para_left_dp[2] + 8192) >> 14);
                ...
                if(RECORD_EQ_STEREO) {
                    ...
                    xout_R     =  CUR_LCF_R_D1_dp  + (xin_R_H * s_cur_lcf_para_right_dp[0] << 1)
                                  + (( xin_R_L * s_cur_lcf_para_right_dp[0] + 8192) >> 14);
                    if(xout_R > ((1 << 30) - 1)) {
                        xout_R = ((1 << 30) - 1);
                    } else if(xout_R < -(1 << 30)) {
                        xout_R = -(1 << 30);
                    ...
                    CUR_LCF_R_D1_dp =  CUR_LCF_R_D2_dp  + (xin_R_H * (-s_cur_lcf_para_right_dp[0])
                                                           << 2) + (((xin_R_L * (-s_cur_lcf_para_right_dp[0]) << 1) + 8192) >> 14)  -
                                       (xout_R_H * s_cur_lcf_para_right_dp[1] << 1) - ((xout_R_L *
                                               s_cur_lcf_para_right_dp[1] + 8192) >> 14);
                    CUR_LCF_R_D2_dp =                  (xin_R_H * s_cur_lcf_para_right_dp[0] << 1)
                                                       + ((xin_R_L * s_cur_lcf_para_right_dp[0] + 8192) >> 14)  -
                                                       (xout_R_H * s_cur_lcf_para_right_dp[2] << 1) - ((xout_R_L *
                                                               s_cur_lcf_para_right_dp[2] + 8192) >> 14);
...
void  AUDPROC_ProcessDp_2(
        ...
        int frame_count=uiSrcCount/channel;
    
        int16_t  power_log = 0,DP_ingain_cdB = 0,DP_in_power = 0;
        int32_t  dp_out_power = 0;
        int32_t  attack_gain = 0,release_gain = 0;
    
        for(si=0;si<frame_count;si++)//AUDIO POST PROCESS
        {
            //get decoder left and right
            if(1==channel){
                sl = psSrcData[si];
                ...
                sl = psSrcData[2*si];// s(si,1);
                sr = psSrcData[2*si+1];// s(si,2);
            }
            ...
            if(1==channel){
                //psDestData[si]   =  out_left;
                psDestData[si]  =  out_right;
            }else{
                sl = psDestData[2*si];// s(si,1);
                sr = psDestData[2*si+1];// s(si,2);
            }
        }
