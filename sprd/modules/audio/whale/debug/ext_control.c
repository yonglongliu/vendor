...
extern int agdsp_timeout_dump_set(void * arg,void * params,int opt);
...
int ext_control_init(struct tiny_audio_device *adev){
    ...
    if(access(MMI_DEFAULT_PCM_FILE, R_OK) != 0){
        ...
        if(chmod(MMI_DEFAULT_PCM_FILE, S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH) != 0) {
            LOG_E("%s Cannot set RW to \"%s\": %s", __FUNCTION__,MMI_DEFAULT_PCM_FILE, strerror(errno));
        }
    }
