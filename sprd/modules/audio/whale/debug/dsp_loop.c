...
static int audio_dsp_loop_start(struct tiny_audio_device *adev,struct dsp_loop_t *loop){
   ...
   // switch_vbc_iis_route(adev->dev_ctl,UC_LOOP,true);
    ...
    select_devices_new(adev->dev_ctl,AUDIO_HW_APP_DSP_LOOP,in_devices,true,false,true);
    select_devices_new(adev->dev_ctl,AUDIO_HW_APP_DSP_LOOP,out_devices,false,false,true);
