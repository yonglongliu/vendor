        ...
        case AGDSP_CMD_ASSERT_EPIPE:
            {
                if(agdsp_ctl->dsp_assert_time_out || agdsp_ctl->dsp_assert){
                    ALOGE("dsp already time out or assert!!!");
                    usleep(2000000);
                    break;
                }
                uint8_t temp[40] = {0};
                ...
                aud_dsp_assert_set(agdsp_ctl->dev, true);
                ALOGE("dsp asserted!!!");
                len_t = sprintf(temp,":%x,%x,%x,%x",msg->parameter0,msg->parameter1,msg->parameter2,msg->parameter3);
                set_usecase(agdsp_ctl->dev_ctl, UC_AGDSP_ASSERT, true);
                dsp_ctrl_dsp_assert_notify(agdsp_ctl,0,agdsp_ctl->reset_modem,temp,len_t);

                ALOGE("peter: force all standby in");
                force_all_standby(agdsp_ctl->dev);
                if(agdsp_ctl->auto_reset_dsp) {
                    agdsp_boot();
                }
                ...
                ALOGE("peter: force all standby out");
            }
            break;
...
int agdsp_timeout_dump_set(void * arg,void * params,int opt){
    struct tiny_audio_device *adev = (struct tiny_audio_device *)arg ;
    ...
    if(agdsp_ctl->fd_dsp_assert_log > 0) {
        ...
        ALOGE(" agdsp_timeout_dump_set ret: %d, opt: %d",ret, opt);
    }
    return 0;
}
...
void * dsp_ctrl_open(struct audio_control *dev_ctl)
{
    ...
    ALOGE(" dsp_ctrl_notify_dspdump notify ret: %d",ret);
...
void dsp_ctrl_close(struct dsp_control_t * dsp_ctl)
{
    ...
    pthread_kill(dsp_ctl->rx.thread_id, SIGKILL);
