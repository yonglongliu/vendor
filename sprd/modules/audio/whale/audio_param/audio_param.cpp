...
static const struct audio_param_mode_t  audio_param_mode_table[] = {
    ...
    { PROFILE_MODE_MUSIC_Handsfree_ExtRecord,        "Music\\Handsfree\\extRecord"  },
...
static int load_sprd_ap_audio_param(void *audio_param,param_group_t ele, param_infor_t  * param_infor,char *path){
   ...
   }else if(PROFILE_MODE_MUSIC_Handsfree_ExtRecord == id){
        process_param=(char *)&(ap_param->record[AUDIO_RECORD_MODE_EXT_HANDSFREE]);
