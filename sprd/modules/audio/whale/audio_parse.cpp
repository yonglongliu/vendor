...
int parse_audio_route(struct audio_control *control)
{
    ...
    private_control = root->FirstChildElement("vbc_pcm_dump");
    if(NULL!=private_control){
        dev = private_control->FirstChildElement();
        if(NULL!=dev){
...
{
    if((NULL==route) || (NULL==route->trans) || (0==route->trans_size)){
        LOG_I("free_transition:route is null");
        return;
    }
