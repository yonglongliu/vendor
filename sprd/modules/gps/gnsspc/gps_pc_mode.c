...
static unsigned int gps_mode = 0;   // cold start : 125     cw cn0: 50002
#define  GREENEYE_I    1
...
int set_gps_mode(unsigned int mode)
{
	...
	switch(mode)
	{
		...
		case 10: // TCXO mode
			...
			ret = 1;
			break;
		case 11: // GPS rf data test
		case 12: // GLONASS rf data test
		case 13: // Beidou rf dta test
...
int get_init_mode(void)
{
	if(first_open == 0)
	{
		...
		D("after dlopen\n");
		property_set("persist.gnss.tool.test", "1");
