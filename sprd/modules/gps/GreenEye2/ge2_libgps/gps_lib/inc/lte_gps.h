...
typedef struct {
    ...
    uint32_t  physCellId; /*range:0~503*/

    uint32_t  rsrp;
    uint32_t  rsrq;
    uint32_t  systemFrameNumber;  /*10Bits, invalid value:0xFFFF*/
    uint32_t  rxTxTimeDiffResult;     /*range:0~4095, invalid value:0xFFFF*/
    uint32_t  timeAlignmentValue;   /*invalid value:0xFFFF*/
    uint32_t  cellTrackingAreaCode;
    uint32_t  is_ECGI_present ;
    uint32_t  MCCMNC0;
    uint32_t  MCCMNC1;
    uint32_t  MCCMNC2;
    uint32_t  refMNCDigits;    // 2 or 3, number of MCC-MNC-Digit
    uint32_t  cellIdentity;  /*28Bits*/
#endif
