...
void *allocation_tracker_notify_free(UNUSED_ATTR uint8_t allocator_id, UNUSED_ATTR void *ptr) {
  if (!allocations || !ptr)
    return ptr;
  ...
  LOG_INFO("%s beginning_canary : %s", __func__, beginning_canary);
  LOG_INFO("%s end_canary : %s", __func__, end_canary);
