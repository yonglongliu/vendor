...
#define SYSFS_DUMPMEM_NODE "/sys/devices/platform/wcn_bt/dumpmem"
...
int userial_vendor_open(tUSERIAL_CFG *p_cfg)
{
    ...
    ret = property_get("ro.build.type", value, "");
    if (ret >= 0 && strstr(value, "userdebug")) {
        ALOGI("%s set usrdebug mode to tty.", __func__);
        int fd = -1, ret;
        char value_usr_debug = 2;
        fd = open(SYSFS_DUMPMEM_NODE, O_WRONLY);
        if (fd < 0) {
          ALOGI("open %s faild.", SYSFS_DUMPMEM_NODE);
        } else {
          ret = write(fd, &value_usr_debug, 1);
          if (ret < 0) {
            ALOGI("write %s faild.", SYSFS_DUMPMEM_NODE);
          }
          close(fd);
        }
    }
