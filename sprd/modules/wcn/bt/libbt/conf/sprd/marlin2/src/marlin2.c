...
static void marlin2_pskey_reload(pskey_config_t *pskey) {
    ...
    ret = property_get("persist.sys.bt.dsp.log", property_buf, "1");
    if (ret >= 0) {
        value = strtoul(property_buf, 0, 0) & 0xFFFFFFFF;
        ALOGI("persist.sys.bt.dsp.log %u -> %u", pskey->share_memo_tx_packet_num_addr, value);
...
static int marlin2_init(void) {
    ...
    ret = property_get("persist.sys.bt.non.ssp", property, "close");
