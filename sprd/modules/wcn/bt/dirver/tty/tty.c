...
static void stty_handler (int event, void *data)
{
	...
	case SBUF_NOTIFY_READ:
		...
		break;
	default:
		pr_debug("Received event is invalid(event=%d)\n", event);
	}
		      ...
		      int count)
{
	...
	/*if(count <= 16) {
		printk("stty write buf data : ");
		while(loop_count_i < count) {
			printk("%02X ", buf[loop_count_i++]);
		}
		printk("\n");
	}
	else {
		printk("stty write buf data : %02X %02X %02X %02X %02X %02X %02X %02X ",
			buf[0],buf[1],buf[2],buf[3],buf[4],buf[5],buf[6],buf[7]);
		printk(".......%02X %02X %02X %02X %02X %02X %02X %02X ",
			buf[count - 8],buf[count - 7],buf[count - 6],buf[count - 5],
			buf[count - 4],buf[count - 3],buf[count - 2],buf[count - 1]);
	}*/
...
//	unsigned char loop_count_i = 0;
	...
	while(left_legnth > 0) {
		...
		pr_debug("stty write bufwrite_length = %d, left_legnth = %d\n",write_length, left_legnth);
