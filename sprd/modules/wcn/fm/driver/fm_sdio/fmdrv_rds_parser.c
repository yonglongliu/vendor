...
/* add for RT not support two types*/
	...
	{
		rds_event_set(&(fmdev->rds_data.event_status),
		RDS_EVENT_LAST_RADIOTEXT + RDS_EVENT_PROGRAMNAME);
	return;
	}
...
void rds_get_ps(unsigned char *buf)
{
	...
	if (valid == fm_false) {
		...
		rds_event_set(&(fmdev->rds_data.event_status),
			RDS_EVENT_PROGRAMNAME + RDS_EVENT_LAST_RADIOTEXT);
		return;
	}
	...
	while (1) {
		...
		case RDS_PS_FINISH:
			rds_event_set(event, RDS_EVENT_PROGRAMNAME + RDS_EVENT_LAST_RADIOTEXT);
