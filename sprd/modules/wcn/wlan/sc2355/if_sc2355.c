...
#define PCIE_TX_NUM 96
	...
	int list_num;
	unsigned char p_buf[128] = {0};

	wl_info("%s:%d tx_count is %d\n", __func__, __LINE__, tx_count);
	...
	if (ret != 0 || head == NULL || tail == NULL) {
		wl_err("%s, %d error!mbuf alloc fail\n", __func__, __LINE__);
		return -1;
	}
...
/*TODO*/
		...
		*msg_ptr = (unsigned long)msg_pos;
		/*TODO, these code is for msg_buf check*/
		if (*(data_ptr + 5) != 0x0b) {
			sprintf(p_buf, "msg%02d:%p/ptr:%p/%02x %02x %02x %02x %02x %02x",
				i, msg_pos, data_ptr,
				*(data_ptr+4), *(data_ptr+5),
				*(data_ptr+6), *(data_ptr+7),
				*(data_ptr+8), *(data_ptr+9));
			wl_err("%s, %d, error! wrong msg:%s\n",
	...
	ret = sprdwcn_bus_push_list(dev->tx_data_port, head, tail, tx_count);
	if (ret != 0) {
		...
		INIT_LIST_HEAD(tx_list_head);
		tx_packets += tx_count;
		wl_err("%s:%d, TX succ, tx count: %d, total tx_num:%lu\n",
		       __func__, __LINE__, tx_count, tx_packets);/*TODO*/
	}
	return ret;
}
...
{
	unsigned char i;
	/*struct sprdwl_peer_entry *tempht;*/
	...
	/*tempht = (struct sprdwl_peer_entry *)hash_table_get
			  (intf->ht, intf->skb_da);
	if (tempht &&
		(intf->peer_entry[tempht->lut_index].ctx_id == vif->ctx_id)) {
		wl_debug("%s, %d, lut_index=%d\n",
				__func__, __LINE__, tempht->lut_index);
		return tempht->lut_index;
	}*/
	for (i = 0; i < MAX_LUT_NUM; i++) {
...
/*TODO*/
	...
	if (skb->ip_summed == CHECKSUM_PARTIAL) {
		...
		wl_debug("%s: offload: offset: %d, protocol: %d\n",
			 __func__, dscr->tcp_udp_header_offset,
			 ip_hdr(skb)->protocol);
	}
...
int sprdwl_suspend_resume_handle(int chn, int mode)
{
	...
	if (mode == 0) {
		if (atomic_read(&tx_msg->tx_list_special_data.ref) > 0 ||
...
void sprdwl_event_sta_lut(struct sprdwl_vif *vif, u8 *data, u16 len)
{
	...
	case ADD_LUT_INDEX:
		...
		/*hash_table_add(intf->ht, intf->peer_entry[i].tx.da,
					   &(intf->peer_entry[i]));*/
		break;
	default:
		break;
		       ...
		       unsigned char cmd_id, unsigned char ctx_id)
{
	...
	rbuf = kzalloc((1 + sizeof(struct host_addba_param)), GFP_KERNEL);
	if (!rbuf) {
	...
	for (i = 0; i < len; i++)
		wl_err("ba_mgmt num: %d : %02x\n", i, data_ptr[i]);
	if (sprdwl_cmd_send_recv(priv, msg, CMD_WAIT_TIMEOUT, rbuf, NULL))
		goto out;
	...
	if (cmd_id == WIFI_CMD_ADDBA_REQ &&
		...
		if (!test_and_clear_bit(tid, &peer_entry->ba_tx_done_map))
			goto out;
		wl_err("%s, %d, tx_addba failed, reason=%d, tid=%d, map=%lu\n",
		       __func__, __LINE__,
		       rbuf[0],
		       tid,
...
static unsigned int hash_33(char *macaddr)
{
	unsigned int hash = 0;
	int i;

	for (i = 0; i < ETH_ALEN; i++)
		hash = (hash << 5) + hash + *macaddr++;
	return hash;
}
...
{
	struct HashTable *ht = kmalloc(sizeof(struct HashTable), GFP_KERNEL);

	if (NULL == ht) {
		wl_info("hash table create failed at %s, %d\n",
				__func__, __LINE__);
		return NULL;
	}
	ht->table = kmalloc(sizeof(struct kv *) * TABLE_SIZE, GFP_KERNEL);
	if (NULL == ht->table) {
		wl_info("hash table create failed at %s, %d\n",
				__func__, __LINE__);
	...
	wl_info("hash table create succ at %s, %d\n",
			__func__, __LINE__);
	return ht;
}
...
void hash_table_delete(struct HashTable *ht)
{
	...
	if (ht) {
		...
		kfree(ht);
		ht = NULL;
		wl_info("hash table delete succ at %s, %d\n",
				__func__, __LINE__);
	}
}

/* insert or update a value indexed by macaddr */
int hash_table_add(struct HashTable *ht, char *macaddr, void *value)
{
	int i = hash_33(macaddr) % TABLE_SIZE;
	struct kv *p = ht->table[i];
	struct kv *prep = p;
	struct kv *kvv;

	while (p) { /* if macaddr is already stroed, update its value */
			if (memcmp(p->macaddr, macaddr, ETH_ALEN) == 0) {
				p->value = value;
				wl_info("hash table update value succ at %s, %d\n",
						__func__, __LINE__);
				break;
			}
			prep = p;
	...
	if (p == NULL) {/* if macaddr has not been stored, then add it */
		kvv = kmalloc(sizeof(struct kv), GFP_KERNEL);
		if (NULL == kvv) {
			wl_info("hash table add value failed at %s, %d\n",
					__func__, __LINE__);
			return -1;
		}
		init_kv(kvv);
		...
		if (prep == NULL)
			...
			prep->next = kvv;
		wl_info("hash value add succ at %s, %d\n",
				__func__, __LINE__);
	}
	return 0;
}

/* get a value indexed by macaddr */
void *hash_table_get(struct HashTable *ht, char *macaddr)
{
	int i = hash_33(macaddr) % TABLE_SIZE;
	struct kv *p = ht->table[i];

	while (p) {
			if (memcmp(macaddr, p->macaddr, ETH_ALEN) == 0) {
				printk_ratelimited("hash value get succ at %s, %d\n",
							__func__, __LINE__);
		   return p->value;
		   }
		   p = p->next;
	}
	printk_ratelimited("hash value get failed at %s, %d, macaddr is:%02x:%02x:%02x:%02x:%02x:%02x\n",
			__func__, __LINE__,	macaddr[0], macaddr[1],
			macaddr[2],	macaddr[3], macaddr[4], macaddr[5]);
	return NULL;
}

/* remove a value indexed by macaddr */
void hash_table_rm(struct HashTable *ht, char *macaddr)
{
	int i = hash_33(macaddr) % TABLE_SIZE;
	struct kv *p = ht->table[i];
	struct kv *prep;
	...
	if (p == NULL) {
		wl_info("hash value is null at %s, %d, macaddr is:%02x:%02x:%02x:%02x:%02x:%02x\n",
				__func__, __LINE__,	macaddr[0], macaddr[1],
				macaddr[2], macaddr[3], macaddr[4], macaddr[5]);
	} else {
		if (memcmp(macaddr, p->macaddr, ETH_ALEN) == 0) {
			ht->table[i] = p->next;
			free_kv(p);
			wl_info("hash value rm succ at %s, %d\n",
					__func__, __LINE__);
		} else {
			while (p) {
				prep = p;
				p = p->next;
				if (!p) {
					wl_info("hash value not found when removing, %s, %d\n",
						__func__, __LINE__);
					break;
				}
				if (memcmp(macaddr, p->macaddr, ETH_ALEN)
					== 0) {
					prep->next = p->next;
					free_kv(p);
					p = NULL;
