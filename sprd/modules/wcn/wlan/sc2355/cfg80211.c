...
static void sprdwl_cancel_scan(struct sprdwl_vif *vif)
{
	...
	wlan_set_assert(vif->priv, vif->ctx_id, SCAN_ERROR);
}
					    ...
					    *request)
{
	...
	if (request->n_ssids > TOTAL_2G_5G_CHANNEL_NUM) {
		wl_err("%s, %d, error! request->n_ssids=%d\n",
		       __func__, __LINE__, request->n_ssids);
		request->n_ssids = TOTAL_2G_5G_CHANNEL_NUM;
	}
				     ...
				     enum nl80211_tdls_operation oper)
{
	...
	return sprdwl_tdls_oper(vif->priv, vif->ctx_id, peer, oper, 1);
}
