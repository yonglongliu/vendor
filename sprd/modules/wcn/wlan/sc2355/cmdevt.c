	...
	if (ret != -1) {
		if (rbuf && rlen && *rlen) {
			...
			*rlen = min(*rlen, plen);
			...
			wl_info("ctx_id:%d cmd_id:%d [%s]rsp received\n",
		...
		if (vif != NULL) {
			...
			if (intf->cp_asserted == 0)
				sprdwl_send_assert_cmd(vif, CMD_RSP_TIMEOUT_ERROR);
		}
	}
	sprdwl_cmd_unlock(cmd);
out:
...
#define SEC2	2
	...
	wl_info("total config len:%ld,sec1 len:%ld, sec2 len:%ld\n",
		sizeof(wifi_data), sizeof(*sec1), sizeof(*sec2));
...
int sprdwl_get_fw_info(struct sprdwl_priv *priv)
{
	...
	if (!ret && r_len) {
		...
		if ((r_len - SEC1_LEN) >= sizeof(struct wiphy_sec2_t)) {
		     ...
		     int oper, u8 need_cp2_rsp)
{
	...
	tdls.peer = peer;
	tdls.oper = oper;
	tdls.need_cp2_rsp = need_cp2_rsp;
		     ...
		     int oper, u8 need_cp2_rsp)
{
	...
	if (need_cp2_rsp)
		...
		return sprdwl_cmd_send_recv_no_wait(priv, msg);
}
...
void sprdwl_send_assert_cmd(struct sprdwl_vif *vif, u8 reason)
{
	struct sprdwl_work *misc_work;
	...
	memcpy(misc_work->data, &reason, 1);
...
* 1:rsp cnt lost
*/
int wlan_set_assert(struct sprdwl_priv *priv, u8 vif_ctx_id, u8 reason)
{
	struct sprdwl_msg_buf *msg;
...
unsigned short sprdwl_rx_rsp_process(struct sprdwl_priv *priv, u8 *msg)
{
	...
	if (0 != handle_flag)
		wlan_set_assert(priv, SPRDWL_MODE_NONE, HANDLE_FLAG_ERROR);
...
unsigned short sprdwl_rx_event_process(struct sprdwl_priv *priv, u8 *msg)
{
	...
	wl_info("[%u]ctx_id %d recv[%s]len: %d\n",
