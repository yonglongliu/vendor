...
static char s_calibration_flashlight_file[] =
    "/data/sensor_%s_flashlight_%s.dat";
static char s_calibration_lsc_file[] = "/data/sensor_%s_lnc_%d_%d_%d_%s.dat";

int32_t ISP_Cali_GetLNCTab(struct sensor_lnc_map_addr *lnc_map_addr, uint32_t x,
                           uint32_t y) {
#if 0
...
int32_t ISP_Cali_SaveLNCTab(struct isp_addr_t lnc_tab, uint32_t size,
...
void ISP_Cali_GetLensTabSize(struct isp_size_t img_size, uint32_t grid,
                             uint32_t *tab_size) {
...
int32_t ISP_Cali_GetLensTabs(struct isp_addr_t img_addr, uint32_t grid,
                             struct isp_size_t img_size, uint32_t *lens_tab,
                             uint32_t x, uint32_t y, uint32_t type) {
    uint32_t rtn = 0;
#if 0
	uint32_t length = 0;
...
int32_t ISP_Cali_RawRGBStat(struct isp_addr_t *img_addr,
    ...
    rtn = ISP_Cali_Get_RawRGB_Stat(img_addr, rect, img_size, img_pttn,
                                   stat_param);

    return rtn;
}

uint32_t ISP_Cali_LensCorrection(struct isp_addr_t *src_data,
...
int32_t ISP_Cali_UnCompressedPacket(struct isp_addr_t src_addr,
                                    struct isp_addr_t dst_addr,
                                    struct isp_size_t img_size,
                                    uint32_t edn_type) {
    int32_t rtn = 0;
    uint32_t indx = 0;
...
int32_t ISP_Cali_BlackLevelCorrection(struct isp_addr_t *in_img_addr,
    ...
    ISP_CALI_LOG("ISP_Cali_BlackLevelCorrection: rect (%d, %d, %d, %d), image "
                 "size: (%d, %d), bayer mode: %d, blc stat:(r:%d, b:%d, gr:%d, "
                 "gb: %d)\n",
                 rect->x, rect->y, rect->width, rect->height, img_size->width,
                 img_size->height, bayer_pttn, stat_param->r_stat,
                 stat_param->b_stat, stat_param->gr_stat, stat_param->gb_stat);

    rtn = ISP_Cali_BLCorrecton(in_img_addr, out_img_addr, rect, img_size,
                               bayer_pttn, stat_param);

    return rtn;
}
static isp_cali_ops_t OMICI = {
    ISP_Cali_GetLensTabSize, ISP_Cali_GetLensTabs, ISP_Cali_LensCorrection,
    ISP_Cali_RawRGBStat,
};