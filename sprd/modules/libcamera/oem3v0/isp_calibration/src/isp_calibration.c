...
uint32_t ISP_Cali_Get_RawRGB_Stat(struct isp_addr_t *img_addr,
    ...
    if ((!img_addr) || (!rect) || (!img_size) || (!stat_param)) {

        ISP_CALI_LOG("ISP_Cali_Get_RawRGB_Stat: params are NULL,addr=0x%p, "
                     "rect=0x%p, size=0x%p, stat=0x%p\n",
...
int32_t _isp_Cali_LNCResAlloc(void *in_ptr, void *cali_ptr, void *gain_ptr) {
    ...
    struct isp_lnc_cali_info *cali_param_ptr =
        (struct isp_lnc_cali_info *)cali_ptr;
    struct isp_lnc_cali_gain *gain_param_ptr =
        (struct isp_lnc_cali_gain *)gain_ptr;
    ...
    if ((0 == in_ptr) || (0 == cali_ptr) || (0 == gain_ptr)) {
        ISP_CALI_LOG("ISP_Cali_LNCResAlloc: input param is null, in =0x%p, "
                     "cali=0x%p, gain=0x%p\n",
    ...
    grid_x = (((width >> 1) - 1) % lnc_grid)
                 ? (((width >> 1) - 1) / lnc_grid + 2)
                 : (((width >> 1) - 1) / lnc_grid + 1);
    grid_y = (((height >> 1) - 1) % lnc_grid)
                 ? (((height >> 1) - 1) / lnc_grid + 2)
                 : (((height >> 1) - 1) / lnc_grid + 1);
...
void _isp_Cali_LNCResFree(struct isp_lnc_cali_info *param_ptr,
                          struct isp_lnc_cali_gain *gain_ptr) {
    if (0 == param_ptr) {
...
int32_t _isp_Cali_LNC_Calc(void *in_ptr, void *rtn_ptr) {
    int32_t rtn = ISP_CALI_RTN_SUCCESS;
    struct isp_lnc_cali_in *in_param_ptr = (struct isp_lnc_cali_in *)in_ptr;
    struct isp_lnc_cali_info *rtn_param_ptr =
        (struct isp_lnc_cali_info *)rtn_ptr;
    ...
    for (y = 0x00; y < map_height; y++) {
        ...
        for (x = 0x00; x < map_width; x++) {
            ...
            if ((0x00 == (0x01 & y)) && (0x00 == (0x01 & x))) {
                rtn_param_ptr->chn_00.stat_ptr[grid_x * stat_y + stat_x] +=
                    *src_img_ptr++;
            } else if ((0x00 == (0x01 & y)) && (0x01 == (0x01 & x))) {
                rtn_param_ptr->chn_01.stat_ptr[grid_x * stat_y + stat_x] +=
                    *src_img_ptr++;
            } else if ((0x01 == (0x01 & y)) && (0x00 == (0x01 & x))) {
                rtn_param_ptr->chn_10.stat_ptr[grid_x * stat_y + stat_x] +=
                    *src_img_ptr++;
            } else if ((0x01 == (0x01 & y)) && (0x01 == (0x01 & x))) {
                rtn_param_ptr->chn_11.stat_ptr[grid_x * stat_y + stat_x] +=
                    *src_img_ptr++;
            }
        }
    }
    ...
    for (x = 0x00; x < grid_num; x++) {
        if (rtn_param_ptr->chn_00.max_stat <
            rtn_param_ptr->chn_00.stat_ptr[x]) {
            rtn_param_ptr->chn_00.max_stat = rtn_param_ptr->chn_00.stat_ptr[x];
        }
        if (rtn_param_ptr->chn_01.max_stat <
            rtn_param_ptr->chn_01.stat_ptr[x]) {
            rtn_param_ptr->chn_01.max_stat = rtn_param_ptr->chn_01.stat_ptr[x];
        }
        if (rtn_param_ptr->chn_10.max_stat <
            rtn_param_ptr->chn_10.stat_ptr[x]) {
            rtn_param_ptr->chn_10.max_stat = rtn_param_ptr->chn_10.stat_ptr[x];
        }
        if (rtn_param_ptr->chn_11.max_stat <
            rtn_param_ptr->chn_11.stat_ptr[x]) {
...
int32_t _isp_Cali_LNC_GainCalc(uint32_t base_gain, uint32_t shf,
                               uint32_t bayer_pattern, uint32_t r_prc,
                               uint32_t g_prc, uint32_t b_prc, void *in_ptr,
                               void *out_ptr) {
    int32_t rtn = ISP_CALI_RTN_SUCCESS;
    struct isp_lnc_cali_info *in_param_ptr = (struct isp_lnc_cali_info *)in_ptr;
    struct isp_lnc_cali_gain *rtn_param_ptr =
        (struct isp_lnc_cali_gain *)out_ptr;
    ...
    for (x = 0x00; x < in_param_ptr->num; x++) {
        map_gain = (in_param_ptr->chn_00.max_stat << shf_bit) /
                   (in_param_ptr->chn_00.stat_ptr[x] + 1);
        map_gain -= std_gain;
        map_gain *= prc_00;
        map_gain >>= shf_bit;
        ...
        map_gain = (in_param_ptr->chn_01.max_stat << shf_bit) /
                   (in_param_ptr->chn_01.stat_ptr[x] + 1);
        map_gain -= std_gain;
        map_gain *= prc_01;
        map_gain >>= shf_bit;
        tmp_val = std_gain + map_gain;
        ...
        map_gain = (in_param_ptr->chn_10.max_stat << shf_bit) /
                   (in_param_ptr->chn_10.stat_ptr[x] + 1);
        map_gain -= std_gain;
        map_gain *= prc_10;
        map_gain >>= shf_bit;
        tmp_val = std_gain + map_gain;
        ...
        map_gain = (in_param_ptr->chn_11.max_stat << shf_bit) /
                   (in_param_ptr->chn_11.stat_ptr[x] + 1);
        map_gain -= std_gain;
        map_gain *= prc_11;
        map_gain >>= shf_bit;
        tmp_val = std_gain + map_gain;
...
int32_t _isp_Cali_LNCTabMerge(struct isp_lnc_cali_gain *gain_ptr,
                              uint32_t grid_num, uint16_t *lnc_tab) {
    uint32_t i = 0;
    uint32_t inx = 0;
...
void ISP_Cali_GetLNCTabSize(struct isp_size_t img_size, uint32_t grid,
                            uint32_t *tab_size) {
...
#if 1
int32_t ISP_Cali_LNCTaleCalc(struct isp_addr_t img_addr, uint32_t bayer_pttn,
                             struct isp_size_t img_size, uint32_t grid,
                             uint16_t *lnc_tab) {
    int32_t rtn = ISP_CALI_RTN_SUCCESS;
    ...
    if ((0 == lnc_tab) || (0 == img_addr.y_addr)) {
        ...
        ISP_CALI_LOG(
            "ISP_Cali_LNCTaleCalc: raw img addr=0x%lx, lnc tab addr=0x%p\n",
    ...
    _isp_Cali_LNCResAlloc((void *)&cali_in_param, (void *)&cali_info_param,
                          (void *)&cali_gain_param);
    _isp_Cali_LNC_Calc((void *)&cali_in_param, (void *)&cali_info_param);
    _isp_Cali_LNC_GainCalc(1024, 10, bayer_pttn, 1024, 1024, 1024,
                           (void *)&cali_info_param, (void *)&cali_gain_param);
...
int32_t ISP_Cali_LNC_GainCalc(uint32_t base_gain, uint32_t shf,
                              uint32_t bayer_pattern, uint32_t r_prc,
                              uint32_t g_prc, uint32_t b_prc, void *in_ptr,
                              void *out_ptr) {
    int32_t rtn = ISP_CALI_RTN_SUCCESS;
    ...
    for (x = 0x00; x < in_param_ptr->num; x++) {
        map_gain = (in_param_ptr->chn_00.max_stat << shf_bit) /
                   in_param_ptr->chn_00.stat_ptr[x];
        map_gain -= std_gain;
        map_gain *= prc_00;
        map_gain >>= shf_bit;
        ...
        map_gain = (in_param_ptr->chn_01.max_stat << shf_bit) /
                   in_param_ptr->chn_01.stat_ptr[x];
        map_gain -= std_gain;
        map_gain *= prc_01;
        map_gain >>= shf_bit;
        ...
        map_gain = (in_param_ptr->chn_10.max_stat << shf_bit) /
                   in_param_ptr->chn_10.stat_ptr[x];
        map_gain -= std_gain;
        map_gain *= prc_10;
        map_gain >>= shf_bit;
        ...
        map_gain = (in_param_ptr->chn_11.max_stat << shf_bit) /
                   in_param_ptr->chn_11.stat_ptr[x];
        map_gain -= std_gain;
        map_gain *= prc_11;
        map_gain >>= shf_bit;
...
int32_t ISP_Cali_LNCTaleCalc(struct isp_addr_t img_addr, uint32_t bayer_pttn,
    ...
    if ((0 == lnc_tab) || (0 == img_addr.y_addr)) {
        ...
        ISP_CALI_LOG(
            "ISP_Cali_LNCTaleCalc: raw img addr=0x%x, lnc tab addr=0x%x\n",
            img_addr.y_addr, lnc_tab);
        return ISP_CALI_RTN_PT_NULL;
    }
    ...
    _isp_Cali_LNC_GainCalc(255, 8, bayer_pttn, 256, 256, 256,
                           (void *)&cali_info_param, (void *)&cali_gain_param);
...
int32_t ISP_Cali_Get_Advanced_LensShading(struct isp_addr_t *lnc_tg,
    ...
    if ((!lnc_tg) || (!lnc_mr) || (!lnc_mg) || (!lnc_final)) {
        // ISP_CALI_LOG("ISP_Cali_Get_Advanced_LensShading: lnc_tg
        // =0x%x,lnc_mr=0x%x, lnc_mg=0x%x, lnc_final\n",
        //	(uint32_t)lnc_tg, (uint32_t)lnc_mr, (uint32_t)lnc_mg,
        //(uint32_t)lnc_final);
        return ISP_CALI_RTN_PT_NULL;
    }

    if ((0 == lnc_tg->y_addr) || (0 == lnc_mr->y_addr) ||
        (0 == lnc_mg->y_addr) || (0 == lnc_final->y_addr)) {

        ISP_CALI_LOG(
            "ISP_Cali_Get_Advanced_LensShading: tg addr=0x%x, mr addr=0x%x,\
...
uint32_t ISP_Cali_LNCCorrection(struct isp_addr_t *src_data,
                                struct isp_addr_t *dst_data,
                                struct isp_size_t img_size, uint8_t grid,
                                uint16_t *lnc_tab) {
    // u/v in Q15
    uint32_t i, j;
    uint32_t ii, jj;
    ...
    uint16_t RelativeX,
        RelativeY; // the relative position of current pixel inside the grid
    ...
    if ((0 == src_data) || (0 == dst_data)) {
        ISP_CALI_LOG("The lens_correction input data pointer is NULL\n");
        return -1;
    }

    if ((0 == src_data->y_addr) || (0 == dst_data->y_addr)) {
    ...
    GridX = ((iWidth / 2 - 1) % LNC_GRID) ? ((iWidth / 2 - 1) / LNC_GRID + 2)
                                          : ((iWidth / 2 - 1) / LNC_GRID + 1);
    GridY = ((iHeight / 2 - 1) % LNC_GRID) ? ((iHeight / 2 - 1) / LNC_GRID + 2)
                                           : ((iHeight / 2 - 1) / LNC_GRID + 1);

    for (row = startRow, SliceY = 0; row < startRow + iSliceHeight;
         row++, SliceY++) {
        for (col = startCol, SliceX = 0; col < startCol + iSliceWidth;
             col++, SliceX++) {
            ...
            if (((iWidth / 2) % LNC_GRID == 1) &&
                ((col / 2) == iWidth / 2 - 1)) {
                GridCurrentX--;
                RelativeX++;
            }

            if (((iHeight / 2) % LNC_GRID == 1) &&
                ((row / 2) == iHeight / 2 - 1)) {
                GridCurrentY--;
                RelativeY++;
            }
            ...
            if ((row & 1) && (col & 1)) {
                cor_ratio = (uint16_t)(
                    (r1c1_ptr[(i * GridX + j) * 4] * temp1 +
                     r1c1_ptr[((i + 1) * GridX + j) * 4] * temp2 +
                     r1c1_ptr[(i * GridX + j + 1) * 4] * temp3 +
                     r1c1_ptr[((i + 1) * GridX + j + 1) * 4] * temp4) >>
                    15);
            }

            if (!(row & 1) && !(col & 1)) {
                cor_ratio = (uint16_t)(
                    (r0c0_ptr[(i * GridX + j) * 4] * temp1 +
                     r0c0_ptr[((i + 1) * GridX + j) * 4] * temp2 +
                     r0c0_ptr[(i * GridX + j + 1) * 4] * temp3 +
                     r0c0_ptr[((i + 1) * GridX + j + 1) * 4] * temp4) >>
                    15);
            }

            if ((row & 1) && !(col & 1)) {
                cor_ratio = (uint16_t)(
                    (r1c0_ptr[(i * GridX + j) * 4] * temp1 +
                     r1c0_ptr[((i + 1) * GridX + j) * 4] * temp2 +
                     r1c0_ptr[(i * GridX + j + 1) * 4] * temp3 +
                     r1c0_ptr[((i + 1) * GridX + j + 1) * 4] * temp4) >>
                    15);
            }

            if (!(row & 1) && (col & 1)) {
                cor_ratio = (uint16_t)(
                    (r0c1_ptr[(i * GridX + j) * 4] * temp1 +
                     r0c1_ptr[((i + 1) * GridX + j) * 4] * temp2 +
                     r0c1_ptr[(i * GridX + j + 1) * 4] * temp3 +
                     r0c1_ptr[((i + 1) * GridX + j + 1) * 4] * temp4) >>
                    15);
            }

            temp1 = (uint16_t)(
                (src_ptr[SliceY * iSliceWidth + SliceX] * (uint32_t)cor_ratio +
                 512) >>
                10);
...
int32_t ISP_Cali_BLCorrecton(struct isp_addr_t *in_img_addr,
                             ...
                             struct isp_rect_t *rect,
                             struct isp_size_t *img_size, uint32_t bayer_pttn,
                             struct isp_bayer_ptn_stat_t *stat_param) {
    uint32_t i, j;
    int32_t tmp;
    uint16_t blc_gr;
    ...
    if ((PNULL == in_img_addr) || (PNULL == out_img_addr) ||
        (PNULL == stat_param) || (PNULL == rect) || (PNULL == img_size)) {

        ISP_CALI_LOG("The ISP_Cali_Blc pointer is NULL, in_ptr: 0x%p, out_ptr: "
                     "0x%p, stat_ptr: 0x%p, rect:0x%p, img_size:0x%p\n",
    ...
    if ((PNULL == in_data_ptr) || (PNULL == out_data_ptr)) {
        ISP_CALI_LOG("The ISP_Cali_Blc data address is NULL, in data addr: "
                     "0x%p, out data addr: 0x%p\n",
