...
//            client->handleCompressedPicture(dataPtr);
			if(data) {
				if(takepic_action[opr_index].pic_cnt == 0 || takepic_action[opr_index].pic_cnt == opts[opr_index].pic_cnt-1) {
//					sleep(20);  //for trace  dump
				}

				if((opts[opr_index].pic_cnt <= 1) || (++takepic_action[opr_index].pic_cnt >= opts[opr_index].pic_cnt)){
...
extern "C" {
	...
	int  start_camera(camera_device_t  **pdev, int *index)
	{
		...
		if(*pdev == NULL) {
			...
			ops_status = HAL_MODULE_INFO_SYM.common.methods->open(&HAL_MODULE_INFO_SYM.common, opts[*index].cameraId, (hw_device_t**)&tmp);
			if(ops_status != 0){
			...
			tmp->ops->set_callbacks(tmp, notify_cb, data_cb, data_cb_timestamp, get_memory, index);
	...
	int  stop_camera(camera_device_t  *pdev)
	{
		...
		if(pdev) {
			CMR_LOGI("pdev->common.close %s %s \n", pdev->common.module->name, pdev->common.module->id);
...
//	while(opts.during == -1){sleep(100);};
	...
	{
		case WK_PREVIEW:
		CMR_LOGI("%d\n WK_PREVIEW during %d s=>", opr_index, opts[opr_index].during);
		ops_status = pdev->ops->start_preview(pdev);
		...
		CMR_LOGI("%d\n WK_PREVIEW during %d s  stopping", opr_index, opts[opr_index].during);
		...
		CMR_LOGI("\n WK_RECVIDEO during %d s  stopping", opts[opr_index].during);
		sleep(1);
		break;
...
exit:
	free(thread);
	running_sensors --;
	CMR_LOGI("sprdcamtest:%d exit,running_sensors = %d", opr_index, running_sensors);
	return (void *)0;
}
...
cmr_int sensor_thread(cmr_handle *thread_handle, int index)
{
	...
	thread = (struct sensor_thread_tag*)malloc(sizeof(struct sensor_thread_tag));
	if (!thread) {
		CMR_LOGE("No mem!");
