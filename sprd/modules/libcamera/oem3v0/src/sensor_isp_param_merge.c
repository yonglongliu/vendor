...
static int sensor_isp_get_param_len(struct sensor_raw_info *sensor_info_ptr,
                                    uint32_t *all_param_len, uint32_t *ae_len,
                                    uint32_t *awb_len, uint32_t *lnc_len,
                                    int mode) {
    int rtn = ISP_SUCCESS;
    ...
    sensor_fix_info =
        (struct sensor_raw_fix_info *)sensor_info_ptr->fix_ptr[mode];

    src_mode_ptr =
        (struct isp_mode_param *)sensor_info_ptr->mode_ptr[mode].addr;
    ...
    CMR_LOGI("sensor_info_ptr->mode_ptr[mode].len =%d",
             sensor_info_ptr->mode_ptr[mode].len);
    ...
    if (-1 != ae_flag) {
        ...
        for (i = 0; i < AE_FLICKER_NUM; i++) {
            for (j = 0; j < AE_ISO_NUM_NEW; j++) {
                ...
                param_len = param_len + again_len + dgain_len + dummy_len +
                            exposure_len + index_len;
                add_ae_len = add_ae_len + again_len + dgain_len + dummy_len +
                             exposure_len + index_len;
            }
        }
        /*ae_flash table*/
        ...
        for (i = 0; i < AE_SCENE_NUM; i++) {
            for (j = 0; j < AE_FLICKER_NUM; j++) {
                ...
                if (0 == j) {
                    scene_info_len =
                        scene_tab[i * AE_FLICKER_NUM + j].scene_info_len;
                } else {
                    scene_info_len = 0;
                }
                param_len = param_len + again_len + dgain_len + dummy_len +
                            exposure_len + index_len + scene_info_len;
                add_ae_len = add_ae_len + again_len + dgain_len + dummy_len +
                             exposure_len + index_len + scene_info_len;
            }
        }
    ...
    if (-1 != awb_flag) {
        awb_map_weight_ptr =
            (struct sensor_awb_map_weight_param *)&sensor_fix_info->awb;
        awb_weight =
            (struct sensor_awb_weight *)&awb_map_weight_ptr->awb_weight;
        ...
        /*wab_weight*/
        param_len =
            param_len + awb_weight->weight_len + awb_weight->size_param_len;
        add_awb_len =
            add_awb_len + awb_weight->weight_len + awb_weight->size_param_len;
    ...
    if (-1 != lnc_flag) {
        ...
        for (i = 0; i < LNC_MAP_COUNT; i++) {
            param_len =
                param_len + lnc_map[i].lnc_len + lnc_map[i].map_info_len;
            add_lnc_len =
                add_lnc_len + lnc_map[i].lnc_len + lnc_map[i].map_info_len;
            CMR_LOGI("lnc_map[i].lnc_len = %d, lnc_map[i].map_info_len = %d",
                     lnc_map[i].lnc_len, lnc_map[i].map_info_len);
        }
    }
    CMR_LOGI(
        "data_len = %d,AE_data_len = %d,AWB_data_len = %d,LNC_data_len = %d",
...
static int sensor_isp_get_awb_packet(void *awb_ptr,
    ...
    memcpy(awb_alg_common_param,
           (void *)((uint8_t *)awb_ptr + sizeof(struct awb_param_ctrl)),
           sizeof(struct awb_alg_param_common));
    ...
    awb_alg_weight_param =
        (uint8_t *)malloc(awb_weight->weight_len + awb_weight->size_param_len);
    ...
    memset(awb_alg_weight_param, 0x00,
           awb_weight->weight_len + awb_weight->size_param_len);
    memcpy((void *)awb_alg_weight_param, (void *)awb_weight->size,
           awb_weight->size_param_len);
    awb_alg_weight_param_tmp =
        (uint8_t *)awb_alg_weight_param + awb_weight->size_param_len;
    memcpy(awb_alg_weight_param_tmp, (uint8_t *)awb_weight->addr,
           awb_weight->weight_len);
    ...
    rtn = _packet_ctrl_param_v1(&spc_data_ctrl, ctrl_param, ctrl_pack_buf,
                                ctrl_pack_buf_size);
    if (0 != rtn) {
        rtn = AWB_PACKET_ERROR;
        goto EXIT;
    }
    rtn = _packet_alg_param_v1(&spc_data_alg, alg_param_common, &alg_param_map,
                               &alg_param_weight, alg_pack_buf,
                               alg_pack_buf_size);
    if (0 != rtn) {
        rtn = AWB_PACKET_ERROR;
        goto EXIT;
    }
    rtn = _ctrl_alg_param_pack_v1(&spc_data_ctrl_alg, ctrl_pack_buf,
                                  ctrl_pack_buf_size, alg_pack_buf,
                                  alg_pack_buf_size, pack_buf, *pack_buf_size);
    if (0 != rtn) {
        rtn = AWB_PACKET_ERROR;
        goto EXIT;
    }
...
int sensor_isp_param_merge(struct sensor_raw_info *sensor_info_ptr,
    ...
    rtn = sensor_isp_get_param_len(sensor_info_ptr, &param_len, &add_ae_len,
                                   &add_awb_len, &add_lnc_len, mode);
    if (ISP_SUCCESS != rtn) {
    ...
    /*get the total mem of new param that match with the struct of
     * ae_tuning_param */
    param_len =
        param_len +
        (OLD_PARAM_RESERVE_LEN - NEW_PARAM_RESERVE_LEN) * sizeof(uint32_t) +
        PAC_AWB_MALLOC_MEM_LEN;
    ...
    memcpy((void *)param_ptr, (void *)src_mode_ptr,
           sizeof(struct isp_mode_param));
    tmp_ptr = param_ptr + sizeof(struct isp_mode_param);

    /*copy the data except AE/AWB/LNC .Meanwhile ,record the info of
     * AE/AWB/LNC*/
    ...
    for (i = 0; i < src_mode_ptr->block_num; i++) {
        ...
        if (src_header[i].block_id == ISP_BLK_AE_V1) {
            ...
            memcpy((void *)ae_ptr, (void *)((uint8_t *)src_mode_ptr + offset),
                   src_header[i].size);
            ...
            offset = src_header[i].offset;
            memcpy((void *)awb_ptr, (void *)((uint8_t *)src_mode_ptr + offset),
                   src_header[i].size);
            ...
            memcpy((void *)lnc_ptr, (void *)((int8_t *)src_mode_ptr + offset),
                   src_header[i].size);
        } else {
            offset = src_header[i].offset;
            src_header_new[i].offset = (uint32_t)(tmp_ptr - param_ptr);
            memcpy((void *)tmp_ptr, (void *)((uint8_t *)src_mode_ptr + offset),
                   src_header[i].size);
    ...
    if (ae_index >= 0) {
        src_header_new[ae_index].size = add_ae_len + src_header[ae_index].size +
                                        (2314 - 256) * sizeof(uint32_t);
        src_header_new[ae_index].offset = (uint32_t)(tmp_ptr - param_ptr);
        memcpy((void *)tmp_ptr, (void *)ae_ptr,
               sizeof(struct ae_param_tmp_001));
        tmp_ptr = tmp_ptr + sizeof(struct ae_param_tmp_001);
        ae_tmp_ptr = ae_ptr + sizeof(struct ae_param_tmp_001);

        /*get param from fix struct*/
        scene_tab =
            (struct ae_scene_exp_gain_tab *)sensor_fix_info->ae.scene_tab;
        ae_tab = (struct ae_exp_gain_tab *)sensor_fix_info->ae.ae_tab;
        weight_tab = (struct ae_weight_tab *)sensor_fix_info->ae.weight_tab;
        auto_iso_tab =
            (struct ae_auto_iso_tab_v1 *)sensor_fix_info->ae.auto_iso_tab;

        /*copy ae gain table data*/
        for (i = 0; i < AE_FLICKER_NUM; i++) {
            for (j = 0; j < AE_ISO_NUM_NEW; j++) {
                memcpy((void *)tmp_ptr,
                       (void *)ae_tab[i * AE_ISO_NUM_NEW + j].index,
                       ae_tab[i * AE_ISO_NUM_NEW + j].index_len);
                tmp_ptr = tmp_ptr + ae_tab[i * AE_ISO_NUM_NEW + j].index_len;
                memcpy((void *)tmp_ptr,
                       (void *)ae_tab[i * AE_ISO_NUM_NEW + j].exposure,
                       ae_tab[i * AE_ISO_NUM_NEW + j].exposure_len);
                tmp_ptr = tmp_ptr + ae_tab[i * AE_ISO_NUM_NEW + j].exposure_len;
                memcpy((void *)tmp_ptr,
                       (void *)ae_tab[i * AE_ISO_NUM_NEW + j].dummy,
                       ae_tab[i * AE_ISO_NUM_NEW + j].dummy_len);
                tmp_ptr = tmp_ptr + ae_tab[i * AE_ISO_NUM_NEW + j].dummy_len;
                memcpy((void *)tmp_ptr,
                       (void *)ae_tab[i * AE_ISO_NUM_NEW + j].again,
                       ae_tab[i * AE_ISO_NUM_NEW + j].again_len);
                tmp_ptr = tmp_ptr + ae_tab[i * AE_ISO_NUM_NEW + j].again_len;
                memcpy((void *)tmp_ptr,
                       (void *)ae_tab[i * AE_ISO_NUM_NEW + j].dgain,
                       ae_tab[i * AE_ISO_NUM_NEW + j].dgain_len);
        ...
        for (i = 0; i < AE_WEIGHT_TABLE_NUM; i++) {
            memcpy((void *)tmp_ptr, (void *)weight_tab[i].weight_table,
                   weight_tab[i].len);
        ...
        for (i = 0; i < AE_SCENE_NUM; i++) {
            memcpy((void *)tmp_ptr,
                   (void *)scene_tab[i * AE_FLICKER_NUM].scene_info,
                   scene_tab[i * AE_FLICKER_NUM].scene_info_len);
            tmp_ptr = tmp_ptr + scene_tab[i * AE_FLICKER_NUM].scene_info_len;
            for (j = 0; j < AE_FLICKER_NUM; j++) {
                memcpy((void *)tmp_ptr,
                       (void *)scene_tab[i * AE_FLICKER_NUM + j].index,
                       scene_tab[i * AE_FLICKER_NUM + j].index_len);
                tmp_ptr = tmp_ptr + scene_tab[i * AE_FLICKER_NUM + j].index_len;
                memcpy((void *)tmp_ptr,
                       (void *)scene_tab[i * AE_FLICKER_NUM + j].exposure,
                       scene_tab[i * AE_FLICKER_NUM + j].exposure_len);
                tmp_ptr =
                    tmp_ptr + scene_tab[i * AE_FLICKER_NUM + j].exposure_len;
                memcpy((void *)tmp_ptr,
                       (void *)scene_tab[i * AE_FLICKER_NUM + j].dummy,
                       scene_tab[i * AE_FLICKER_NUM + j].dummy_len);
                tmp_ptr = tmp_ptr + scene_tab[i * AE_FLICKER_NUM + j].dummy_len;
                memcpy((void *)tmp_ptr,
                       (void *)scene_tab[i * AE_FLICKER_NUM + j].again,
                       scene_tab[i * AE_FLICKER_NUM + j].again_len);
                tmp_ptr = tmp_ptr + scene_tab[i * AE_FLICKER_NUM + j].again_len;
                memcpy((void *)tmp_ptr,
                       (void *)scene_tab[i * AE_FLICKER_NUM + j].dgain,
                       scene_tab[i * AE_FLICKER_NUM + j].dgain_len);
        ...
        for (i = 0; i < AE_FLICKER_NUM; i++) {
            memcpy((void *)tmp_ptr, (void *)auto_iso_tab[i].auto_iso_tab,
                   auto_iso_tab[i].len);
            tmp_ptr = tmp_ptr + auto_iso_tab[i].len;
        }

        memcpy((void *)tmp_ptr, (void *)ae_tmp_ptr,
               sizeof(struct ae_param_tmp_002));
        tmp_ptr = tmp_ptr + sizeof(struct ae_param_tmp_002);
        ae_tmp_ptr = ae_tmp_ptr + sizeof(struct ae_param_tmp_002);

        /*for reserved*/
        memcpy((void *)tmp_ptr, (void *)ae_tmp_ptr,
               NEW_PARAM_RESERVE_LEN * sizeof(uint32_t));
    ...
    if (lnc_index >= 0) {
        ...
        src_header_new[lnc_index].size = add_lnc_len +
                                         src_header[lnc_index].size +
                                         2 * LNC_MAP_COUNT * sizeof(uint32_t);
        CMR_LOGI("src_header_new[lnc_index].size = %d",
                 src_header_new[lnc_index].size);
        src_header_new[lnc_index].offset = (uint32_t)(tmp_ptr - param_ptr);
        memcpy((void *)tmp_ptr, (void *)lnc_ptr,
               sizeof(struct sensor_2d_lsc_param_v1));
        tmp_ptr = tmp_ptr + sizeof(struct sensor_2d_lsc_param_v1);
        map = sensor_fix_info->lnc.map;
        for (i = 0; i < LNC_MAP_COUNT; i++) {
            uint32_t lnc_offset = 0;
            memcpy((void *)tmp_ptr, (void *)map[i].map_info,
                   map[i].map_info_len);
    ...
    if (awb_index >= 0) {
        ...
        pack_buf_size =
            PAC_AWB_MALLOC_MEM_LEN * PAC_AWB_MALLOC_MEM_LEN * sizeof(uint32_t);
        ...
        awb_weight =
            (struct sensor_awb_weight *)&sensor_fix_info->awb.awb_weight;

        /*pack the awb param*/
        rtn = sensor_isp_get_awb_packet((void *)awb_ptr, awb_map, awb_weight,
                                        pack_buf, &pack_buf_size);

        param_ptr_awb_pac =
            (uint8_t *)malloc(src_header_new[awb_index].offset + pack_buf_size);
        ...
        memset(param_ptr_awb_pac, 0x00,
               src_header_new[awb_index].offset + pack_buf_size);
        ...
        src_header_awb_pac =
            (struct isp_block_header *)src_mode_ptr_awb_pac->block_header;

        src_header_awb_pac[awb_index].size = pack_buf_size;
        src_header_awb_pac[awb_index].offset =
            (uint32_t)(tmp_ptr - param_ptr_awb_pac);
...
int sensor_merge_isp_param(struct isp_init_param *init_param_ptr,
    ...
    if (NULL == sensor_raw_info_ptr ||
        NULL == sensor_raw_info_ptr->version_info) {
    ...
    version_info =
        (struct sensor_version_info *)sensor_raw_info_ptr->version_info;
    if (ISP_PARAM_OLD_VERSION < version_info->version_id) {
        for (mode = 0; mode < MAX_MODE_NUM; mode++) {
            rtn = sensor_isp_param_merge(sensor_raw_info_ptr,
                                         &init_param_ptr->mode_ptr[mode], mode);
            if (0 != rtn) {
                CMR_LOGE("isp get param error");
                return rtn;
            }
            CMR_LOGI("data_info.data_ptr = %p,data_info.size = %d",
                     init_param_ptr->mode_ptr[mode].data_ptr,
                     init_param_ptr->mode_ptr[mode].size);
        }
    }
    return rtn;
}
    ...
    if (ISP_PARAM_OLD_VERSION < sensor_version->version_id) {
        for (i = 0; i < MAX_MODE_NUM; i++) {
            if (NULL != sensor_raw_info_ptr->mode_ptr[i].addr) {
                ...
                if (NULL != ae->block.block_info ||
                    NULL != awb->block.block_info ||
                    NULL != lnc->block.block_info) {
                    CMR_LOGV("mode = %d,init_addr = %p,len =%d", i,
                             init_data_info[i].data_ptr,
                             init_data_info[i].size);
                    ...
                    /*for isp tool*/
                    CMR_LOGV("mode = %d,update_addr = %p,len =%d", i,
                             update_data_info[i].data_ptr,
                             update_data_info[i].size);
