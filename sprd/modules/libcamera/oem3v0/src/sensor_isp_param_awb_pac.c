...
*/
static int32_t
_relayout_spc_data(struct spc_data *spcdata, uint32_t relayout_module_index,
                   struct spc_module_data *relayout_module_data) {
    ...
    for (i = 0; i < dst_header->module_num; i++) {
        ...
        tmp_module_header =
            (struct spc_module_header *)((int8_t *)tmp_buf.data +
                                         sizeof(struct spc_header) +
                                         i * sizeof(struct spc_module_header));
        dst_module_header =
            (struct spc_module_header *)((int8_t *)spcdata->data +
                                         sizeof(struct spc_header) +
                                         i * sizeof(struct spc_module_header));

        if (0 == i) {
            dst_module_header->offset =
                dst_header->fixed_size + sizeof(struct spc_header);
        } else {
            dst_module_header_last =
                (struct spc_module_header *)((uint8_t *)spcdata->data +
                                             sizeof(struct spc_header) +
                                             (i - 1) *
                                                 sizeof(
            ...
            dst_module_header->offset =
                dst_module_header_last->offset + dst_module_header_last->size;
        }
        ...
        if (i == relayout_module_index) {
            ...
            if (tmp_module_header->size < relayout_module_data->data_size) {
                spc_data_dst_size =
                    spc_data_tmp_size +
                    (relayout_module_data->data_size - tmp_module_header->size);
            } else {
                spc_data_dst_size =
                    spc_data_tmp_size -
                    (tmp_module_header->size - relayout_module_data->data_size);
            }
            ...
            if (0 < dst_module_header->size)
                memcpy(dst_addr, (int8_t *)relayout_module_data->data,
                       dst_module_header->size);
...
*@ return:
*/
static int32_t _delete_module(struct spc_data *spcdata, uint32_t module_index,
                              struct spc_module_data *delete_module_data) {
    int32_t rtn = SPC_SUCCESS;
    uint32_t i, j;
    ...
    module_header_base =
        (uint8_t *)((uint8_t *)spcdata->data + sizeof(struct spc_header));
    ...
    for (i = 0, j = 0; i < spcheader->module_num; i++) {
        module_header_tmp_addr =
            module_header_tmp_base + i * (sizeof(struct spc_module_header));
        module_header_addr =
            module_header_base + j * (sizeof(struct spc_module_header));

        if (i != module_index) {
            memcpy(module_header_addr, module_header_tmp_addr,
                   sizeof(struct spc_module_header));
            j++;
        }
    }
...
*@ return:
*/
static int32_t spc_init(struct spc_data *spcdata,
                        struct spc_init_param *init_param) {
    int32_t rtn = SPC_SUCCESS;
    struct spc_header *spcheader = NULL;
    uint32_t spc_size = 0;
...
*@ return:
*/
static int32_t spc_write_module(struct spc_data *spcdata,
                                struct spc_write_param *write_param) {
    int32_t rtn = SPC_SUCCESS;
    uint32_t i, j;
    struct spc_header *spcheader = NULL;
    ...
    spc_max_module_num =
        spcheader->fixed_size / (sizeof(struct spc_module_header));
    ...
    for (i = 0; i < w_module_num; i++) { // i : index of w_module
        ...
        for (j = 0; j < spcheader->module_num;
             j++) { // j : index of src spc module data
            spc_module_header_ptr =
                (struct spc_module_header *)((int8_t *)spcheader +
                                             sizeof(struct spc_header) +
                                             j * sizeof(
                                                     struct spc_module_header));
        ...
        if (j >= spcheader->module_num) { // not found same id
            ...
            if ((0 == w_module_data->data_size) ||
                (NULL == w_module_data->data))
                return SPC_WRITE_NULL_DATA;

            if (w_module_data->data_size + spcheader->size >
                spcdata->max_size) {
                rtn = SPC_MEM_NOT_ENOUGH;
                break;
            }
            ...
            spc_module_data_addr =
                (int8_t *)(spcheader) + spc_module_header_ptr->offset;
            memcpy(spc_module_data_addr, (int8_t *)w_module_data->data,
                   w_module_data->data_size);

            spcheader->size +=
                w_module_data->data_size; // spc data size in spc header
            spcheader->module_num += 1;
...
int32_t _packet_ctrl_param_v1(struct spc_data *spc,
                              struct awb_param_ctrl *ctrl_param, void *pack_buf,
...
int32_t _packet_alg_param_v1(struct spc_data *spc,
                             struct awb_alg_param_common *alg_param,
                             struct awb_alg_map *alg_map,
                             struct awb_alg_weight *alg_weight, void *pack_buf,
                             uint32_t pack_buf_size) {
    int32_t rtn = 0;
