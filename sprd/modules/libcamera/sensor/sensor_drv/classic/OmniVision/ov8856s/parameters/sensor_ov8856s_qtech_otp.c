...
//#define OTP_AUTO_LOAD_LSC_ov8856s_qtech

#define RG_TYPICAL_ov8856s_qtech     0x0138
#define BG_TYPICAL_ov8856s_qtech	    0x013E
#define R_TYPICAL_ov8856s_qtech		0x0000
#define G_TYPICAL_ov8856s_qtech		0x0000
#define B_TYPICAL_ov8856s_qtech		0x0000

static uint32_t ov8856s_qtech_read_otp_info(void *param_ptr)
{
	uint32_t rtn = SENSOR_SUCCESS;
	struct otp_info_t *otp_info=(struct otp_info_t *)param_ptr;
	otp_info->rg_ratio_typical=RG_TYPICAL_ov8856s_qtech;
	otp_info->bg_ratio_typical=BG_TYPICAL_ov8856s_qtech;
	otp_info->r_typical=R_TYPICAL_ov8856s_qtech;
	otp_info->g_typical=G_TYPICAL_ov8856s_qtech;
	otp_info->b_typical=B_TYPICAL_ov8856s_qtech;
...
//	{
//	otp_flag = Sensor_ReadReg(i);//0x7010
//	SENSOR_PRINT("ADD=0x%x, data=0x%x ",i,otp_flag);
//	}
	...
	addr = 0;
	if((otp_flag & 0xc0) == 0x40) {
		addr = 0x7024; // base address of VCM Calibration group 1,0x22
	}else if((otp_flag & 0x30) == 0x10) {
		addr = 0x7028; // base address of VCM Calibration group 2,0x7025
	}
	if(addr != 0) {
		otp_info->flag |= 0x20;
	...
	otp_flag = Sensor_ReadReg(0x702c); 
	addr = 0;
	int checksum2=0;
	if((otp_flag & 0xc0) == 0x40) {
...
static uint32_t ov8856s_qtech_update_awb(void *param_ptr)
{
	uint32_t rtn = SENSOR_SUCCESS;
	struct otp_info_t *otp_info=(struct otp_info_t *)param_ptr;
...
#ifndef OTP_AUTO_LOAD_LSC_ov8856s_qtech

static uint32_t ov8856s_qtech_update_lsc(void *param_ptr)
{
	uint32_t rtn = SENSOR_SUCCESS;
	struct otp_info_t *otp_info=(struct otp_info_t *)param_ptr;
	...
	if (otp_info->flag & 0x10) {
		...
		for(i=0;i<240;i++) {
			SENSOR_PRINT("otp_info->lsc_param[%d]=0x%x\n",i,otp_info->lsc_param[i]);
...
#endif
static uint32_t ov8856s_qtech_update_otp(void *param_ptr)
{
	uint32_t rtn = SENSOR_SUCCESS;
	struct otp_info_t *otp_info=(struct otp_info_t *)param_ptr;

	rtn=ov8856s_qtech_update_awb(param_ptr);
	...
	rtn=ov8856s_qtech_update_lsc(param_ptr);
	if(rtn!=SENSOR_SUCCESS)
	{
...
static uint32_t ov8856s_qtech_identify_otp(void *param_ptr)
{
	uint32_t rtn = SENSOR_SUCCESS;

	rtn=ov8856s_qtech_read_otp_info(param_ptr);
