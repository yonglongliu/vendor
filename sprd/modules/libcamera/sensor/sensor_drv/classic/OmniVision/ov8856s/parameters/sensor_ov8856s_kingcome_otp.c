...
//#define OTP_AUTO_LOAD_LSC_ov8856s_kingcome

#define RG_TYPICAL_ov8856s_kingcome     0x0138
#define BG_TYPICAL_ov8856s_kingcome	    0x013E
#define R_TYPICAL_ov8856s_kingcome		0x0000
#define G_TYPICAL_ov8856s_kingcome		0x0000
#define B_TYPICAL_ov8856s_kingcome		0x0000

static uint32_t ov8856s_kingcome_read_otp_info(void *param_ptr)
{
	uint32_t rtn = SENSOR_SUCCESS;
	struct otp_info_t *otp_info=(struct otp_info_t *)param_ptr;
	otp_info->rg_ratio_typical=RG_TYPICAL_ov8856s_kingcome;
	otp_info->bg_ratio_typical=BG_TYPICAL_ov8856s_kingcome;
	otp_info->r_typical=R_TYPICAL_ov8856s_kingcome;
	otp_info->g_typical=G_TYPICAL_ov8856s_kingcome;
	otp_info->b_typical=B_TYPICAL_ov8856s_kingcome;
	...
	// OTP VCM Calibration
	otp_flag = Sensor_ReadReg(0x7021);
	addr = 0;
	if((otp_flag & 0xc0) == 0x40) {
...
static uint32_t ov8856s_kingcome_update_awb(void *param_ptr)
{
	uint32_t rtn = SENSOR_SUCCESS;
	struct otp_info_t *otp_info=(struct otp_info_t *)param_ptr;
...
#ifndef OTP_AUTO_LOAD_LSC_ov8856s_kingcome

static uint32_t ov8856s_kingcome_update_lsc(void *param_ptr)
{
	uint32_t rtn = SENSOR_SUCCESS;
	struct otp_info_t *otp_info=(struct otp_info_t *)param_ptr;
...
#endif
static uint32_t ov8856s_kingcome_update_otp(void *param_ptr)
{
	uint32_t rtn = SENSOR_SUCCESS;
	struct otp_info_t *otp_info=(struct otp_info_t *)param_ptr;

	rtn=ov8856s_kingcome_update_awb(param_ptr);
	...
	rtn=ov8856s_kingcome_update_lsc(param_ptr);
	if(rtn!=SENSOR_SUCCESS)
	{
...
static uint32_t ov8856s_kingcome_identify_otp(void *param_ptr)
{
	uint32_t rtn = SENSOR_SUCCESS;

	rtn=ov8856s_kingcome_read_otp_info(param_ptr);
