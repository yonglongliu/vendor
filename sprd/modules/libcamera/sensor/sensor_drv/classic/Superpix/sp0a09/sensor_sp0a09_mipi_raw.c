...
#define FPS_INFO       s_sp0a09_mode_fps_info
#define STATIC_INFO    s_sp0a09_static_info
#define VIDEO_INFO     s_sp0a09_video_info
#define MODULE_INFO    s_sp0a09_module_info_tab
#define RES_TAB_RAW    s_sp0a09_resolution_tab_raw
#define RES_TRIM_TAB   s_sp0a09_resolution_trim_tab
#define MIPI_RAW_INFO  g_sp0a09_mipi_raw_info
 ...
 *============================================================================*/
static void sp0a09_drv_group_hold_on(cmr_handle handle) {
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void sp0a09_drv_group_hold_off(cmr_handle handle) {
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void sp0a09_drv_write_gain(cmr_handle handle, cmr_u32 gain) {
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_u16 sp0a09_drv_read_frame_length(cmr_handle handle) {
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void sp0a09_drv_write_frame_length(cmr_handle handle,
                                       cmr_u32 frame_len) {
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_u32 sp0a09_drv_read_shutter(cmr_handle handle) {
    cmr_u32 value = 0x00;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void sp0a09_drv_write_shutter(cmr_handle handle, cmr_u32 shutter) {
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 *============================================================================*/
static cmr_int sp0a09_drv_write_exposure_dummy(cmr_handle handle,
                                             cmr_u32 shutter,
                                             cmr_u32 dummy_line,
                                             cmr_u16 size_index) {
    ...
    cur_fr_len = sp0a09_drv_read_frame_length(handle);

    if (shutter < SENSOR_MIN_SHUTTER)
        shutter = SENSOR_MIN_SHUTTER;

    if (dest_fr_len != cur_fr_len)
        sp0a09_drv_write_frame_length(handle, dest_fr_len);
write_sensor_shutter:
    /* write shutter to sensor registers */
    sns_drv_cxt->sensor_ev_info.preview_shutter = shutter;

    sp0a09_drv_write_shutter(handle, shutter);
...
static cmr_int sp0a09_drv_identify(cmr_handle handle, cmr_uint param)
{
	...
	if ((sp0a09_PID_VALUE == pid_value) && (id0 == 1) && (id1 == 1)) {
...
static cmr_int sp0a09_drv_before_snapshot(cmr_handle handle, cmr_uint param)
{
    ...
    cap_shutter = sp0a09_drv_write_exposure_dummy(handle, cap_shutter, 0, 0);
    cap_gain = gain;
    sp0a09_drv_write_gain(handle, cap_gain);
...
static cmr_int sp0a09_drv_write_exposure(cmr_handle handle, cmr_uint param)
{
    ...
    ret_value = sp0a09_drv_write_exposure_dummy(handle, exposure_line, dummy_line,
                                             size_index);
...
static cmr_int sp0a09_drv_write_gain_value(cmr_handle handle, cmr_uint param)
{
    ...
    sp0a09_drv_write_gain(handle, real_gain);

	return ret_value;
}

static struct sensor_reg_tag sp0a09_shutter_reg[] = {
    {0xfd, 0x00}, {0x03, 0x01}, {0x04, 0xc8},{0x01, 0x01},
};

static struct sensor_i2c_reg_tab sp0a09_shutter_tab = {
    .settings = sp0a09_shutter_reg, .size = ARRAY_SIZE(sp0a09_shutter_reg),
};

static struct sensor_reg_tag sp0a09_again_reg[] = {
  {0xfd, 0x00}, {0x24, 0x60}, {0x01, 0x01},
};

static struct sensor_i2c_reg_tab sp0a09_again_tab = {
    .settings = sp0a09_again_reg, .size = ARRAY_SIZE(sp0a09_again_reg),
};

static struct sensor_reg_tag sp0a09_dgain_reg[] = {
 //   {0x5100, 0}, {0x5101, 0}, {0x5102, 0},    {0x5103, 0},
 //   {0x5104, 0}, {0x5105, 0}, {0x3208, 0x11}, {0x3208, 0xA1},
};

struct sensor_i2c_reg_tab sp0a09_dgain_tab = {
    .settings = sp0a09_dgain_reg, .size = ARRAY_SIZE(sp0a09_dgain_reg),
};

static struct sensor_reg_tag sp0a09_frame_length_reg[] = {
 //   {0x380e, 0}, {0x380f, 0},
};
...
static cmr_u16 sp0a09_drv_calc_exposure(cmr_handle handle, cmr_u32 shutter,cmr_u32 dummy_line, 
...
static void sp0a09_drv_calc_gain(float gain, struct sensor_aec_i2c_tag *aec_info) {
    float gain_a = gain;
    float gain_d = 0x400;
...
static cmr_int sp0a09_drv_read_aec_info(cmr_handle handle,
                                           cmr_uint param) {
    cmr_int ret_value = SENSOR_SUCCESS;
...
//        sp0a09_drv_get_default_frame_length(handle, mode);
//    s_current_default_line_time = s_sp0a09_resolution_trim_tab[mode].line_time;
    sns_drv_cxt->line_time_def = sns_drv_cxt->trim_tab_info[mode].line_time;

    sns_drv_cxt->sensor_ev_info.preview_shutter = sp0a09_drv_calc_exposure(
        handle, exposure_line, dummy_line, mode, &sp0a09_aec_info);

    gain = info->gain < SENSOR_BASE_GAIN ? SENSOR_BASE_GAIN : info->gain;
    real_gain = (float)info->gain * SENSOR_BASE_GAIN / ISP_BASE_GAIN * 1.0;
    sp0a09_drv_calc_gain(real_gain, &sp0a09_aec_info);
    return ret_value;
}

cmr_int sp0a09_drv_set_master_FrameSync(cmr_handle handle,
                                           cmr_uint param) {
