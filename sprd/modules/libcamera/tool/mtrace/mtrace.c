...
static int register_log(headinfo *p, int line, const char *file,
                        memstat_t *stat) {
    ...
    for (i = 0; (stat->log.alloc_p_buffersize != 0) &&
                (i < (int)(stat->log.alloc_p_buffersize / sizeof(void *)));
         i++) {
    ...
    for (i = 0; (stat->log.max_index != -1) && (i <= stat->log.max_index);
         i++) {

        if (stat->log.logstr[i] == NULL)
            continue;

        sscanf(stat->log.logstr[i], logfmt, &o_times, &o_size, o_file, &o_line,
               &j);
        if ((o_line == line) && (strcmp(o_file, file) == 0) && o_times > 0) {
            ret = snprintf((char *)stat->log.logstr[i], MAX_ALLOC_FILNAMELEN,
                           logfmt, o_times + 1, size + o_size, file, line, j);
            if (ret < 0) {
    ...
    ret = snprintf((char *)stat->log.logstr[j], MAX_ALLOC_FILNAMELEN, logfmt, 1,
                   size, file, line, o_alloc_id++);
    if (ret < 0) {
        mtrace_log_warn("snprintf error\n");
    }
...
static int unregister_log(headinfo *p, memstat_t *stat) {
    ...
    if (o_times > 1) {
        snprintf((char *)stat->log.logstr[j], MAX_ALLOC_FILNAMELEN, logfmt,
                 --o_times, o_size - p->size, o_file, o_line, i);
        return 0;
    }
...
static int updata_meminfo(char *pmem, int type, int size, memstat_t *stat,
...
*/
void *mtrace_calloc_trace(size_t size, int nunit, const char *file,
                          const char *fun, int line, memstat_t *stat) {
    char *p;
...
void *mtrace_malloc_trace(size_t size, const char *file, const char *fun,
                          int line, memstat_t *stat) {
    char *p;
...
void *mtrace_realloc_trace(void *old, size_t size, const char *file,
                           const char *fun, int line, memstat_t *stat) {
    char *p;
    int type;
...
void mtrace_free_trace(void *pmen, const char *file, const char *fun, int line,
                       memstat_t *stat) {
    if (pmen) {
...
static int checkregister(void *pbuf, size_t num, int line, const char *file,
                         memstat_t *stat) {
    int i;
    int ret = 0;
    ...
    for (i = 0; (stat->log.alloc_p_buffersize != 0) &&
        ...
        if (stat->log.alloc_p[i] != NULL) {
            ...
            tailinfo *ptail =
                (tailinfo *)((char *)p + headinfo_len + ALIGN(p->size, 4));
            ...
            if (ptail->magic != (TAIL_MAGIC_MASK | type)) {
                mtrace_log_error("tail overwite at %p,0x%x,0x%x\n", p,
                                 ptail->magic, ptail->size);
                error++;
            }
            if (error != 0) {
            } else if ((type == MB_TYPE_F) || (type == MB_TYPE_F)) {
                mtrace_log_error("already free %p \n", p);
            } else if (((char *)pbuf <= (char *)p &&
                        ((char *)pbuf + num) > (char *)p) ||
                       ((char *)pbuf < ((char *)p + headinfo_len) &&
                        ((char *)pbuf + num) >= ((char *)p + headinfo_len))) {
                mtrace_log_error("will overwite head %p by %s:%d\n", p, file,
                                 line);
                show_log(p, stat);
                ret = -1;
            } else if (((char *)pbuf <= (char *)ptail &&
                        ((char *)pbuf + num) > (char *)ptail) ||
                       ((char *)pbuf < ((char *)ptail + tailinfo_len) &&
                        ((char *)pbuf + num) >=
                            ((char *)ptail + tailinfo_len))) {
                mtrace_log_error("will overwite tail %p by %s:%d\n", p, file,
                                 line);
                show_log(p, stat);
                ret = -1;
            } else if (((char *)pbuf >= ((char *)p + headinfo_len)) &&
                       ((char *)pbuf + num) <= (char *)ptail) {
...
void *mtrace_memset_trace(void *ptr, int value, size_t num, const char *file,
                          const char *fun, int line, memstat_t *stat) {
...
void *mtrace_memcpy_trace(void *destination, const void *source, size_t num,
                          const char *file, const char *fun, int line,
                          memstat_t *stat) {
    mtrace_log_info("memcpy %p,%x at %s:%s:%d \n", destination, num, file, fun,
                    line);
...
void mtrace_print_alllog(memstat_t *stat) {
    ...
    mtrace_log_info(
        "total:0x%x, max_once:0x%x, max_total:0x%x, max_total_cnt:0x%x\n",
        stat->total, stat->max_once, stat->max_total, stat->max_total_cnt);
    mtrace_log_info(
        "calloc_cnt:0x%x, malloc_cnt:0x%x, realloc_cnt:0x%x, total_cnt:0x%x\n",
        stat->calloc_cnt, stat->malloc_cnt, stat->realloc_cnt, stat->total_cnt);
