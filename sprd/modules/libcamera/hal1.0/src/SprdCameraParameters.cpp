...
/*Parse rectangle from string like "(100,100,200,200, weight)" . if
 * exclude_weight is true: the weight is not write to rect array*/
static int parse_rect(int *rect, int *count, const char *str,
                      bool exclude_weight);
static void coordinate_struct_convert(int *rect_arr, int arr_size);
static int coordinate_convert(int *rect_arr, int arr_size, int angle,
                              int is_mirror,
                              SprdCameraParameters::Size *preview_size,
...
const int SprdCameraParameters::kPreviewSettingCount =
    sizeof(kPreviewSizes) / sizeof(Size);
const int SprdCameraParameters::kFrontCameraConfigCount =
    ARRAY_SIZE(sprd_front_camera_hardware_config);
const int SprdCameraParameters::kBackCameraConfigCount =
    ARRAY_SIZE(sprd_back_camera_hardware_config);
...
const char SprdCameraParameters::KEY_JPEG_THUMBNAIL_QUALITY[] =
    "jpeg-thumbnail-quality";
const char SprdCameraParameters::KEY_JPEG_THUMBNAIL_SIZE[] =
    "jpeg-thumbnail-size";
const char SprdCameraParameters::KEY_JPEG_THUMBNAIL_WIDTH[] =
    "jpeg-thumbnail-width";
const char SprdCameraParameters::KEY_JPEG_THUMBNAIL_HEIGHT[] =
...
const char SprdCameraParameters::KEY_SUPPORTED_BRIGHTNESS[] =
    "brightness-values";
...
const char SprdCameraParameters::KEY_EXPOSURE_COMPENSATION[] =
...
const char SprdCameraParameters::KEY_SUPPORTED_SLOWMOTION[] =
    "slow-motion-values";
const char SprdCameraParameters::KEY_SATURATION[] = "saturation";
const char SprdCameraParameters::KEY_SUPPORTED_SATURATION[] =
    "saturation-values";
...
const char SprdCameraParameters::KEY_SUPPORTED_FLASH_MODE[] =
...
SprdCameraParameters::SprdCameraParameters(const String8 &params)
...
void SprdCameraParameters::setDefault(ConfigType config) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        //		LOGI("SetDefault: key = %s, value = %s", element[i].key,
        // element[i].value);
    }
}

const char *SprdCameraParameters::getDefaultValue(ConfigType config,
    ...
    for (i = 0; i < count; i++) {
        if (0 == strcmp(element[i].key, KEY)) {
            LOGV("got Default: key = %s, value = %s", element[i].key,
                 element[i].value);
            break;
        }
    }
...
/*return rectangle: (x1, y1, x2, y2, weight), the values are on the screen's
 * coordinate*/
...
/*return rectangle: (x1, y1, x2, y2), the values are on the sensor's
 * coordinate*/
void SprdCameraParameters::getFocusAreas(int *area, int *count,
                                         Size *preview_size, Rect *preview_rect,
    ...
    if (area_count > 0) {
        int ret = coordinate_convert(&focus_area[0], area_count, orientation,
                                     mirror, preview_size, preview_rect);
        if (ret) {
            area_count = 0;
    ...
    if (area_count > 0) {
        ...
        for (i = 0; i < area_count; i++) {
            ...
            if ((left != 0) && (right != 0) && (top != 0) && (bottom != 0)) {
                if ((left >= right) || (top >= bottom) || (left < -1000) ||
                    (top < -1000) || (right > 1000) || (bottom > 1000) ||
                    (weight < 1) || (weight > 1000)) {
                    LOGI("chekFocusAreas: left=%d,top=%d,right=%d,bottom=%d, "
                         "weight=%d \n",
...
/*return rectangle: (x1, y1, x2, y2), the values are on the sensor's
 * coordinate*/
void SprdCameraParameters::getMeteringAreas(int *area, int *count,
                                            Size *preview_size,
                                            Rect *preview_rect, int orientation,
                                            bool mirror) {
    ...
    if (area_count > 0) {
        int ret = coordinate_convert(&metering_area[0], area_count, orientation,
                                     mirror, preview_size, preview_rect);
        if (ret) {
            area_count = 0;
        ...
        for (i = 0; i < area_count; i++) {
            ...
            if ((left != 0) && (right != 0) && (top != 0) && (bottom != 0)) {
                if ((left >= right) || (top >= bottom) || (left < -1000) ||
                    (top < -1000) || (right > 1000) || (bottom > 1000) ||
                    (weight < 1) || (weight > 1000)) {
                    LOGI("checkMeteringAreas: "
                         "left=%d,top=%d,right=%d,bottom=%d, weight=%d \n",
                         left, top, right, bottom, weight);
                    return 1;
                }
            }
...
int SprdCameraParameters::getJpegQuality() { return getInt(KEY_JPEG_QUALITY); }
...
const char *SprdCameraParameters::get_Effect() const { return get(KEY_EFFECT); }
...
const char *SprdCameraParameters::get_Zoom() const { return get(KEY_ZOOM); }

void SprdCameraParameters::setZoom(const char *value) { set("zoom", value); }
...
int SprdCameraParameters::getExposureCompensation() {
    ...
    return lookup(exposure_compensation_map, p,
...
const char *SprdCameraParameters::get_Iso() const { return get(KEY_ISO); }

void SprdCameraParameters::setIso(const char *value) { set("iso", value); }
...
void SprdCameraParameters::removeGpsLatitude(void) { remove(KEY_GPS_LATITUDE); }
...
void SprdCameraParameters::removeGpsAltitude(void) { remove(KEY_GPS_ALTITUDE); }
...
void SprdCameraParameters::set_Supported_Auto_Exposure_Values(
    const char *values) {
...
void SprdCameraParameters::setZsl(int value) { set(KEY_ZSL, value); }

void SprdCameraParameters::setCapMode(int value) { set(KEY_CAPMODE, value); }
...
void SprdCameraParameters::updateSupportedPreviewSizes(int width, int height) {
    ...
    const char *pos_1 = vals_p, *pos_2 = vals_p, *vals_temp = vals_p,
               *pos_dst = vals_p;
    ...
    if (strlen(size_new) > strlen(vals_new) ||
        (strlen(size_new) == strlen(vals_new) && strcmp(size_new, vals_new))) {
        ...
        while (pos_1) {
            ...
            pos_2 = pos_1 + 1;
            if (strlen(size_new) > strlen(vals_new) ||
                (strlen(size_new) == strlen(vals_new) &&
                 strcmp(size_new, vals_new))) {
...
static int coordinate_convert(int *rect_arr, int arr_size, int angle,
    ...
    LOGD("coordinate_convert: preview_rect x=%d, y=%d, width=%d, height=%d",
         preview_rect->x, preview_rect->y, preview_rect->width,
         preview_rect->height);
    ...
    for (i = 0; i < arr_size * 2; i++) {
        ...
        LOGD("coordinate_convert rect i=%d x=%d y=%d", i, rect_arr[i * 2],
             rect_arr[i * 2 + 1]);
    }
