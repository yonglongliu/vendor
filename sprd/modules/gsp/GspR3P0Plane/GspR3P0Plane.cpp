...
bool GspR3P0Plane::checkBlending(hwc_layer_1_t *layer) {
  bool result = false;
  ...
  switch (layer->blending) {
    ...
    case HWC_BLENDING_COVERAGE:
      ALOGI_IF(mDebugFlag, "r3p0 support blending flag: %d", layer->blending);
      result = true;
      break;
    default:
...
bool GspR3P0Plane::isVideoLayer(enum gsp_r3p0_osd_layer_format format) {
  ...
  if (format > GSP_R3P0_OSD_FMT_RGB565) result = true;
