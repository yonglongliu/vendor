 ...
 */
#ifndef GSPR5P0PLANE_GSPR5P0PLANEDST_H_
#define GSPR5P0PLANE_GSPR5P0PLANEDST_H_
...
#define R5P0_DST_GSP_MMU_SIZE (16 * 1024 * 1024)

namespace android {

class GspR5P0PlaneDst : public GspR5P0Plane {
 public:
  explicit GspR5P0PlaneDst(bool async, int max_gspmmu_size,
                           int max_gsp_bandwidth);
  virtual ~GspR5P0PlaneDst() {}

  bool adapt(SprdHWLayer **list, int count);

  struct gsp_r5p0_des_layer_user &getConfig();
 ...
 private:
  enum gsp_r5p0_des_layer_format dstFormatConvert(int format);
  ...
  void configFBC(bool outAFBC, int format);
  ...
  struct gsp_r5p0_des_layer_user mConfig;
...
#endif  // GSPR5P0PLANE_GSPR5P0PLANEDST_H_
