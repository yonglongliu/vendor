...
int GspR5P0PlaneArray::adapt(SprdHWLayer **list, int count, uint32_t w,
  ...
  /*
    ...
    while (i < count) {
      ...
      if (privateH->internal_format & GRALLOC_ARM_INTFMT_AFBC_SPLITBLK) {
        ALOGI_IF(mDebugFlag, "GSP find enable  AFBC on layer: %d .", i);
      } else {
        ALOGI_IF(mDebugFlag, "GSP find disable  AFBC on layer: %d .", i);
      }
      i++;
    }
  */
  ...
  ALOGI_IF(mDebugFlag, "gsp r5p0plane adapt success");

  return ret;
}

bool GspR5P0PlaneArray::imagePlaneParcel(SprdHWLayer **list,
                                         int ImgPlaneIndex) {
