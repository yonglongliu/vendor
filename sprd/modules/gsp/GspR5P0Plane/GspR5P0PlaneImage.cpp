...
bool GspR5P0PlaneImage::adapt(SprdHWLayer *layer, int LayerIndex) {
  ...
  if (checkImgLayerFormat(format) == false) return false;
...
void GspR5P0PlaneImage::configPmargbMode(hwc_layer_1_t *layer) {
  struct gsp_r5p0_img_layer_params *params = &mConfig.params;
...
void GspR5P0PlaneImage::configCSCMatrix(uint8_t y2r_mod) {
  struct gsp_r5p0_img_layer_params *params = &mConfig.params;
...
void GspR5P0PlaneImage::configEndian(native_handle_t *handle) {
  ...
  switch (format) {
    ...
    case HAL_PIXEL_FORMAT_YCrCb_420_SP:
      params->endian.uv_word_endn = GSP_R5P0_WORD_ENDN_2;
      break;
...
bool GspR5P0PlaneImage::parcel(SprdHWLayer *layer) {
  if (layer == NULL) {
  ...
  uint8_t y2r_mod = 3;
#if 0
  if (handle->yuv_info == MALI_YUV_BT601_NARROW ||
      handle->yuv_info == MALI_YUV_BT709_NARROW
      /*||private_h->yuv_info == MALI_YUV_BT2020_NARROW*/) {
    y2r_mod = 3;
  } else if (handle->yuv_info == MALI_YUV_BT601_WIDE ||
