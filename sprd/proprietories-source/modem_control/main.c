...
static void print_log_data(char *buf, int cnt) {
  ...
  if (cnt > DATA_BUF_SIZE) cnt = DATA_BUF_SIZE;
...
static void modem_ctrl_ipc_init() {
  ...
  for (i = 0; i < MAX_CLIENT_NUM; i++) client_fd[i] = -1;

  FD_ZERO(&s_rdyFds);
  memset(s_ipc_evt_watch_table, 0, sizeof(s_ipc_evt_watch_table));
}

static int ipc_blocking_write(int fd, const void *buffer, size_t len) {
  ...
  while (writeOffset < len) {
    ...
    if (written >= 0) {
      ...
      MODEM_LOGE("ipc_blocking_write: unexpected error on write errno:%d",
                 errno);
      return -1;
    }
  }
...
static void ipc_watch_fd_remove(int fd) {
  ...
  if ((fd + 1) == s_maxfd) {
    ...
    for (i = 0; i < IPC_EVT_MAX_WATCH; i++) {
      if (s_ipc_evt_watch_table[i].valid && (s_ipc_evt_watch_table[i].fd > n)) {
...
static void *modem_ctrl_ipc_thread(void *par) {
  ...
  // start listen
  listen_fd = socket_local_server(
      MODEM_SOCKET_NAME, 0, /*ANDROID_SOCKET_NAMESPACE_RESERVED,*/ SOCK_STREAM);
  if (listen_fd < 0) {
    MODEM_LOGE(
        "modemctrl_listenaccept_thread: cannot create local socket server\n");
    exit(-1);
  }
...
static int get_modem_assert_information(char *assert_info, int size) {
  ...
  if ((assert_info == NULL) || (size == 0)) return 0;
...
int chk_assert_info_string(const char *message, int size) {
  if (memcmp(message, LTE_MODEM_ASSERT_STR, strlen(LTE_MODEM_ASSERT_STR)) ==
      0) {
    return 1;
  }
...
static void process_modem_state_message(char *message, int size) {
  ...
  MODEM_LOGD(
      "alive=%d reset=%d force_reset=%dflash_less = %d, modem_state = %d \n",
  ...
  switch (modem_state) {
    ...
    case MODEM_STA_ALIVE:
      if (MODEM_INTF_EVENT_ASSERT == evt) {
        ...
        ret = chk_assert_info_string(assert_info_buffer,
                                     strlen(assert_info_buffer));
        if (ret > 0) {
...
static void switchUserToSystem(void)
{
  ...
  if (capset(&header, &data[0]) == -1) {
    MODEM_LOGD("capset failed: %s", strerror(errno));
  }
}

static int modem_ctrl_is_reserve_root(void) {
  int fd = 0, ret = 0;
  int mode = 0;
  ...
  if ((ret = read(fd, cmdline, sizeof(cmdline) - 1)) > 0) {
    ...
    MODEM_LOGD("%s:check calibration!\n", __FUNCTION__);
    if (strstr(cmdline, "calibration="))
      mode = 1; // is calimode

    MODEM_LOGD("%s:check factorytest!\n", __FUNCTION__);
    if (strstr(cmdline, "=factorytest"))
      mode = 2; // is factorytest
  }
...
int main(int argc, char *argv[]) {
  ...
  ret = modem_ctrl_is_reserve_root();
  MODEM_LOGD("%s: mode = %d!", __FUNCTION__, ret);
  ...
  property_get(MODEM_RADIO_TYPE, g_modem, "not_find");

  MODEM_LOGD("%s: modem type is %s", __FUNCTION__, g_modem);

  if (strcmp("not_find", g_modem) == 0) {
    MODEM_LOGD("%s: %s %s", __FUNCTION__, MODEM_RADIO_TYPE, "not_find");
    return 0;
  }

  if (0 != pthread_create(&t1, NULL, detect_sipc_modem, g_modem)) {
    MODEM_LOGE("detect_sipc_modem  create error!\n");
  }

  if (0 != pthread_create(&t2, NULL, parm_modem_monitor, NULL)) {
    MODEM_LOGE("pmic_monitor create error!\n");
  }
