...
int read_nv_partition(char *path, char *Bak_path, char *path_out) {
  ...
  int32 ret, ret1, ret2, size, magic;
  int32 status = 0;
  int32 Bak_size = 0;
  uint8 *buf = NULL;
  uint8 * Bak_buf = NULL;
  ...
  do {
    handle =
        open(path, O_RDWR, S_IRUSR | S_IWUSR /* S_IRWXU | S_IRWXG | S_IRWXO */);
    if (handle <= 0)
      return 0;
  ...
  modem_ctl_enable_busmonitor(0);
  modem_ctl_enable_dmc_mpu(0);
  switch (status) {
    case 0:
  ...
  modem_ctl_enable_busmonitor(1);
  modem_ctl_enable_dmc_mpu(1);
  close(handle);
  close(Bak_Handle);
  close(out_handle);
  free(buf);
