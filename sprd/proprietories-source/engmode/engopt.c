...
int eng_bind(int fd) { return 0; }

int eng_connect(int fd) { return 0; }
int eng_listen(int fd) { return 0; }
...
int eng_close(int fd) { return close(fd); }

int eng_read(int fd, void* buf, size_t len) { return read(fd, buf, len); }

int eng_write(int fd, const void* buf, size_t len) {
  return write(fd, buf, len);
}
int eng_lseek(int fd, int pos, int where) { return lseek(fd, pos, where); }

int eng_unlink(const char* path) { return unlink(path); }

int eng_creat(const char* path, int mode) { return 0; }
