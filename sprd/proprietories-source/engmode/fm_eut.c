...
typedef struct _FM_SIGNAL_PARAM_T
{
    unsigned char  nOperInd; 	// 	Tune: 	0: tune successful;
                                        //			1: tune failure
                                                //	Seek: 	0: seek
out valid channel successful
                            //			1: seek out valid channel
failure
...
void btfmTuneTestCallback(int status, FM_SIGNAL_PARAM_T *chnl_info) {
  ...
  ALOGI("Tune callback, status: %d, freq: %d rssi: %d", status,
        mFmSignalParm.nFreqValue, mFmSignalParm.nRssi);
}
...
void btfmSeekTestCallback(int status, FM_SIGNAL_PARAM_T *chnl_info) {
  ...
  ALOGI("seek callback, status: %d, freq: %d rssi: %d", status,
        mFmSignalParm.nFreqValue, mFmSignalParm.nRssi);
}

void btfmAudioModeCallback(int status, int audioMode) {
  ALOGI("Audio mode change callback, status: %d, audioMode: %d", status,
        audioMode);
}

void btfmAudioPathCallback(int status, int audioPath) {
  ALOGI("Audio path change callback, status: %d, audioPath: %d", status,
        audioPath);
}
...
static btfm_test_callbacks_t fmtestCallback = {
    ...
    btfmSeekTestCallback,          btfmTuneTestCallback,
};
...
int fm_write_reg(unsigned int addr, int value) {
  ...
  if ((NULL != sFmInterface) &&
    ...
    // needd return in callback.
    while (counter++ < 3 && FM_STATE_ENABLED != sFmWriteRegStatus &&
           FM_STATE_ERR != sFmWriteRegStatus)
      sleep(1);
...
static int btHalLoad(void) {
  ...
  if (err == 0) {
    err = s_hwModule->methods->open(module, BT_HARDWARE_MODULE_ID,
                                    (hw_device_t **)&s_hwDev);
    if (err == 0) {
...
int fmOpen(void) {
  ...
  if (FM_STATE_ENABLED == sFmStatus || FM_STATE_PLAYING == sFmStatus)
    return 0;  // fm has been opened
  if (btHalLoad() < 0) {
    return -1;
  } else {
