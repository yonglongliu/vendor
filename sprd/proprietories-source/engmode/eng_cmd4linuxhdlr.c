...
static int eng_linuxcmd_cam0id(char* req, char* rsp);
static int eng_linuxcmd_cam1id(char* req, char* rsp);
static int eng_linuxcmd_cam0size(char* req, char* rsp);
static int eng_linuxcmd_cam1size(char* req, char* rsp);
static int eng_linuxcmd_lcdid(char* req, char* rsp);
// device info end
...
{CMD_EMMCDDRSIZE,        CMD_TO_AP,     "AT+EMMCDDRSIZE",      eng_linuxcmd_get_emmcddrsize},
    {CMD_CAM0ID,          CMD_TO_AP,     "AT+CAM0ID",         eng_linuxcmd_cam0id},
    {CMD_CAM1ID,          CMD_TO_AP,     "AT+CAM1ID",         eng_linuxcmd_cam1id},
    {CMD_CAM0SIZE,          CMD_TO_AP,     "AT+CAM0SIZE",         eng_linuxcmd_cam0size},
    {CMD_CAM1SIZE,          CMD_TO_AP,     "AT+CAM1SIZE",         eng_linuxcmd_cam1size},
    {CMD_LCDID,          CMD_TO_AP,     "AT+LCDID",         eng_linuxcmd_lcdid},
...
void *thread_fastsleep(void *para) {
  ...
  int count;
  int sleep = 0;
  int trials = 3;  //尝试次数可调整，如果还不行可再增大
  int i;
...
int connect_tcp_server(int protocol_family, const char *address,
                       unsigned short port) {
  int fd = -1;
  ...
  if (-1 == fd) {
    ENG_LOG("can't create client socket");
    return fd;
  }
  ...
  if (inet_pton(protocol_family, address, &addr.sin_addr) <= 0) {
    close(fd);
    fd = -1;
    ENG_LOG("fail to convert %s to network address", address);
  } else {
...
static int notice_slogmodem(char *cmd)
{
  ...
  */
  int fd = connect_tcp_server(AF_INET, LOCAL_HOST, SLOG_MODEM_SERVER_PORT);
...
// device info
int eng_linuxcmd_cam0id(char *req, char *rsp)
{
    int fd, ret = 1, len = 0;
    int voltage;
    float vol;
	...
	struct cam_info_s cam_info[2] = {0};

    fd = open("/data/misc/media/sensorinfo.file", O_RDONLY);
    if(fd < 0){
        ENG_LOG("%s: open %s fail [%s]",__FUNCTION__, "/data/misc/media/sensorinfo.file", strerror(errno));
        ret = 0;
    }

    if(ret==1) {
        memset(cam_info, 0, sizeof(cam_info));
        len = read(fd, cam_info, sizeof(cam_info));
        if(len > 0){

            sprintf(rsp, "%s%s", cam_info[0].id, ENG_STREND);
        }else {
            sprintf(rsp, "%s%s", SPRDENG_ERROR, ENG_STREND);
        }
    } else {
...
int eng_linuxcmd_cam1id(char *req, char *rsp)
{
    int fd, ret = 1, len = 0;
    int voltage;
    float vol;
    ...
    if(ret==1) {
        ...
        if(len > 0){
            ...
            sprintf(rsp, "%s%s", cam_info[1].id, ENG_STREND);
        }else {
            sprintf(rsp, "%s%s", SPRDENG_ERROR, ENG_STREND);
        }
    } else {
 ...
 int eng_linuxcmd_cam0size(char *req, char *rsp)
{
    int fd, ret = 1, len = 0;
    int voltage;
    float vol;
    ...
    if(ret==1) {
        ...
        if(len > 0){
            ...
            sprintf(rsp, "%sW%s", cam_info[0].size, ENG_STREND);
        }else {
            sprintf(rsp, "%s%s", SPRDENG_ERROR, ENG_STREND);
        }
    } else {
 ...
 int eng_linuxcmd_cam1size(char *req, char *rsp)
{
    int fd, ret = 1, len = 0;
    int voltage;
    float vol;
    ...
    if(ret==1) {
        ...
        if(len > 0){
            ...
            sprintf(rsp, "%sW%s", cam_info[1].size, ENG_STREND);
        }else {
            sprintf(rsp, "%s%s", SPRDENG_ERROR, ENG_STREND);
        }
    } else {
 ...
 int eng_linuxcmd_lcdid(char *req, char *rsp)
{
    int fd, ret = 1, len = 0;
    int voltage;
    float vol;
    ...
    if(fd < 0){
        ENG_LOG("%s: open %s fail [%s]",__FUNCTION__, "/proc/cmdline", strerror(errno));
        ret = 0;
    }
	...
	if(p)
	{
		...
		while(*p != ' ')
			lcd_id[i++] = *p++;
		sprintf(rsp, "%s%s%s%s", lcd_id, ENG_STREND, SPRDENG_OK, ENG_STREND);
	}
	else
		sprintf(rsp, "%s%s", SPRDENG_ERROR, ENG_STREND);
        }else {
