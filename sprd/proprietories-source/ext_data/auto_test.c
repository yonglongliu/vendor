...
static int check_usb_name(void)
{
    ...
    int ret = 0;
    ifc_init();
    for (i = 0;i<sizeof(usb_name)/sizeof(char*);i++) {
...
static void start_autotest_v6(struct command *c) {
    ...
    usbname_id = check_usb_name();
    exec_cmd("ip -6 rule add iif %s lookup %d pref %d", usb_name[usbname_id],
    ...
    exec_cmd("ip -6 route add default dev %s table %d", usb_name[usbname_id],
