        ...
        "allmodules")
            if [ "NONE" = $SECURE_BOOT ]; then
                ...
                if (echo $TARGETBOARD | grep -E 'whale|sp9001|sp9850s|sp9860g|sp9833|sp9850ka|sc9850j|9850kh|sp9820e') 1>/dev/null 2>&1 ; then
                    ...
                    doImgHeaderInsert $SPL $FDL1 $SECVM $MVCFG $MOB  $FDL2  $UBOOT $BOOT $RECOVERY $UBOOTAUTO
                    doAESencrypt  $SPLSIGN  $SECVMSIGN $MVCFGSIGN $MOBSIGN $UBOOTSIGN $BOOTSIGN $RECOVERYSIGN
                    doSignImage $SPLSIGN $FDL1SIGN $SECVMSIGN $MVCFGSIGN $MOBSIGN $FDL2SIGN $UBOOTSIGN  $BOOTSIGN $RECOVERYSIGN $UBOOTAUTOSIGN
                else
                    doImgHeaderInsert $SPL $UBOOT $FDL1 $FDL2 $BOOT $RECOVERY $UBOOTAUTO
                    doSignImage $SPLSIGN $UBOOTSIGN $FDL1SIGN $FDL2SIGN $BOOTSIGN $RECOVERYSIGN $UBOOTAUTOSIGN
                fi
