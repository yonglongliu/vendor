...
#define WCN_DOWNLOAD_CMD_REBOOT_WCN \
  "rebootwcn"  // to do a reset of wcn, need to bootup the cp2 again
#define WCN_DOWNLOAD_CMD_DUMP_WCN "dumpwcn"  // to dump wcn mem only
#define WCN_DOWNLOAD_CMD_START_WCN \
  "startwcn"  // to bootup the cp2, use in power on CP2
#define WCN_DOWNLOAD_CMD_STOP_WCN "stopwcn"  // used in power off CP2

#define WCN_DOWNLOAD_RESP_REBOOT_WCN \
  "rebootwcn-ok"  // the response of the reset cmd
...
static int connect_download(void) {
  ...
  client_fd = socket_local_client(
      WCN_DOWNLOAD_SOCKET_NAME, ANDROID_SOCKET_NAMESPACE_ABSTRACT, SOCK_STREAM);

  while (client_fd < 0 && retry_count > 0) {
    retry_count--;
    WCND_LOGD("%s: Unable bind server %s, waiting (error:%s)...\n", __func__,
              WCN_DOWNLOAD_SOCKET_NAME, strerror(errno));
    usleep(100 * 1000);
    client_fd =
        socket_local_client(WCN_DOWNLOAD_SOCKET_NAME,
...
static int do_download_cmd(int cmd_id) {
  ...
  if (ret < 0) {
    WCND_LOGE("write %s bytes to client_fd:%d fail (error:%s)", cmd_str,
              download_fd, strerror(errno));
...
int wcnd_before_reset(WcndManager *pWcndManger) {
  ...
  if (!pWcndManger->is_in_userdebug &&
      pWcndManger->dumpmem_on) {  // in user mode, and dump mem is on
    ...
    WCND_LOGD(
        "%s: in userdebug or reset disabled, do not do reset process!"
        "but for MARLIN need to do reset for dump mem!!",
        __FUNCTION__);
...
/**
* write image file pointed by the src_fd to the destination pointed by the
* dst_fd
* return -1 for fail
*/
static int write_image(int src_fd, int src_offset, int dst_fd, int dst_offset,
                       int size) {
...
* 2. download image: cat /dev/block/platform/sprd-sdhci.3/by-name/wcnmodem >
* /proc/cpwcn/modem
...
static inline int stop_cp2(WcndManager *pWcndManger) {
  ...
  WCND_LOGD("%s: open stop interface: %s, fd = %d", __func__,
            pWcndManger->wcn_stop_iface_name, stop_fd);
  if (stop_fd < 0) {
    WCND_LOGE("open %s failed, error: %s", pWcndManger->wcn_stop_iface_name,
              strerror(errno));
    return -1;
  }
  ...
  if (len != 1) {
    WCND_LOGE("write 1 to %s to stop CP2 failed!!",
              pWcndManger->wcn_stop_iface_name);
...
static inline int start_cp2(WcndManager *pWcndManger) {
  ...
  WCND_LOGD("%s: open start interface: %s, fd = %d", __func__,
            pWcndManger->wcn_start_iface_name, start_fd);
  if (start_fd < 0) {
    WCND_LOGE("open %s failed, error: %s", pWcndManger->wcn_start_iface_name,
              strerror(errno));
    return -1;
  }
...
// download image: cat /dev/block/platform/sprd-sdhci.3/by-name/wcnmodem >
// /proc/cpwcn/modem
static inline int download_image_to_cp2(WcndManager *pWcndManger) {
  ...
  WCND_LOGD("%s: open download interface: %s, fd = %d", __func__,
            pWcndManger->wcn_download_iface_name, download_fd);
  if (download_fd < 0) {
    WCND_LOGE("open %s failed, error: %s", pWcndManger->wcn_download_iface_name,
              strerror(errno));
    return -1;
  }
  ...
  WCND_LOGD("Loading %s in bank %s:%d %d", image_file,
            pWcndManger->wcn_download_iface_name, 0,
            pWcndManger->wcn_image_file_size);

  // start downloading  image
  if (write_image(image_fd, 0, download_fd, 0,
                  pWcndManger->wcn_image_file_size) < 0) {
...
* stop CP2
*/
int wcnd_stop_cp2(WcndManager *pWcndManger) { return stop_cp2(pWcndManger); }
