...
/**
* Note:
* For the connection that will only be used to send a commond, when handle the
* command from this
* connection, its client type will be set to WCND_CLIENT_TYPE_CMD. With this
* type, after the command
* result is send back, its client type will be set to WCND_CLIENT_TYPE_SLEEP,
* then when there is an event
* to notify, these connections will be ignored.
*/

static int wcn_process_btwificmd(int client_fd, char *cmd_str,
...
static int before_send_atcmd(int client_fd, char *atcmd_str,
  ...
  if (pWcndManger->state != WCND_STATE_CP2_STARTED) {
    if (to_get_cp2_version) {
      ...
      WCND_LOGD("%s: CP2 is not in normal state (%d), at cmd FAIL", __func__,
                pWcndManger->state);
...
try_again:
  if (try_counts++ > 5) {
    ...
    if ((len <= 0)) {
      ...
      if (strstr(buffer, "fail") || strstr(buffer, "FAIL")) {
        ...
        if (WCND_ATCMD_CMDID_GET_CHIP == atcmd_id) {
  ...
  if (client_fd <= 0) {
    WCND_LOGE("Write '%s' to Invalid client_fd, ret_value: %d", buffer,
              ret_value);
    return ret_value;
  }
...
int wcnd_runcommand(int client_fd, int argc, char *argv[]) {
  ...
  if (!strcmp(argv[0], "reset")) {
  ...
  } else if (strstr(argv[0], "BT") || strstr(argv[0], "WIFI") ||
             strstr(argv[0], "FM")) {  // bt/wifi/fm cmd
    int i = 0;
   ...
   if (pWcndManger->marlin_type == MARLIN_CHIP_TYPE_2) {
      pWcndManger->is_engineer_poweron = 1;
    ...
    //special handle for marlin2
    if (pWcndManger->marlin_type == MARLIN_CHIP_TYPE_2) {
      pWcndManger->is_engineer_poweron = 0;
  ...
  }else if(!strcmp(argv[0], WCND_CMD_EVENT_GE2_ERROR))
   {
      WCND_LOGD("%s",WCND_CMD_EVENT_GE2_ERROR);
        if((pWcndManger->marlin_type != MARLIN_CHIP_TYPE_2 ) || (pWcndManger->is_ge2_error == 1)) return   -1;
        ...
        if(pWcndManger->is_ge2_version) {
           if(pWcndManger->is_in_userdebug) {
               ...
               handle_ge2_error(pWcndManger,argv[1]);
          }
        }
  } else {
