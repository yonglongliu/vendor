...
#define WAIT_ONE_TIME \
  (200) /* wait for 200ms at a time when polling for property values */
...
void wcnd_wait_for_supplicant_stopped(void) {
  ...
  while (maxnaps-- > 0) {
    usleep(200 * 1000);
    if (property_get("init.svc.p2p_supplicant", value1, "stopped") &&
        property_get("init.svc.wpa_supplicant", value2, "stopped")) {
      if ((strcmp(value1, "stopped") == 0) &&
          (strcmp(value2, "stopped") == 0)) {
        return;
      }
    }
  }
