...
static int on_marlin2_poweron(WcndManager *pWcndManger) {
  ...
  if (pWcndManger->marlin_type != MARLIN_CHIP_TYPE_2) return 0;
  ...
  if(pWcndManger->is_in_userdebug) {
    ...
    wcnd_process_atcmd(-1, WCND_ATCMD_CP2_EXIT_USER, pWcndManger);
  }else {
...
static void wcn_state_cp2_stopped(WcndManager *pWcndManger,
  ...
  if (pWcndManger->btwifi_state)
    WCND_LOGE("WARNING: btwifi_state: 0x%x in state: WCND_STATE_CP2_STOPPED",
  ...
  switch (pMessage->event) {
    ...
    case WCND_EVENT_PENGING_EVENT:
      ...
      if (pWcndManger->btwifi_state ||
          (pWcndManger->pending_events & WCND_EVENT_CP2POWERON_REQ)) {
        // transact to next state
        pWcndManger->state = WCND_STATE_CP2_STARTING;

        WCND_LOGD(
            "Enter WCND_STATE_CP2_STARTING from WCND_STATE_CP2_STOPPED for "
            "PENDING CP2 POWER ON EVENT");
        wcnd_send_selfcmd(pWcndManger, "wcn " WCND_SELF_CMD_START_CP2);
      }
      break;
    ...
    case WCND_EVENT_CP2_OK:
      ...
      pWcndManger->notify_enabled = 1;
      WCND_LOGD(
          "Warning: Enter WCND_STATE_CP2_STARTED from WCND_STATE_CP2_STOPPED");
      break;
...
static void wcn_state_cp2_starting(WcndManager *pWcndManger,
                                   WcndMessage *pMessage) {
  if (!pWcndManger || !pMessage) return;
  ...
  if (!pWcndManger->btwifi_state)
    WCND_LOGE("WARNING: btwifi_state: 0x%x in state: WCND_STATE_CP2_STARTING",
              pWcndManger->btwifi_state);
  ...
  switch (pMessage->event) {
    ...
    case WCND_EVENT_CP2_DOWN:
      ...
      // transact to next state
      pWcndManger->state = WCND_STATE_CP2_STOPPED;
      wcnd_send_notify_to_client(pWcndManger, WCND_CMD_RESPONSE_STRING " FAIL",
                                 WCND_CLIENT_TYPE_CMD);
      wcnd_send_notify_to_client(pWcndManger, WCND_CMD_RESPONSE_STRING " FAIL",
                                 WCND_CLIENT_TYPE_CMD_PENDING);
    ...
    case WCND_EVENT_CP2POWEROFF_REQ:
      WCND_LOGD(
          "Warning: Receive WCND_EVENT_CP2POWEROFF_REQ at "
          "WCND_STATE_CP2_STARTING, btwifi_state: 0x%x",
          pWcndManger->btwifi_state);
...
static void wcn_state_cp2_started(WcndManager *pWcndManger,
  ...
  if (!pWcndManger->btwifi_state)
    WCND_LOGE("WARNING: btwifi_state: 0x%x in state: WCND_STATE_CP2_STARTED",
              pWcndManger->btwifi_state);
  ...
  switch (pMessage->event) {
    ...
    case WCND_EVENT_FM_CLOSE:
      ...
      if (pWcndManger->saved_btwifi_state) {
        ...
        if (!pWcndManger->btwifi_state) {
          ...
          WCND_LOGD(
              "Enter WCND_STATE_CP2_STOPPING from WCND_STATE_CP2_STARTED");

          wcnd_send_selfcmd(pWcndManger, "wcn " WCND_SELF_CMD_STOP_CP2);

        } else {
          WCND_LOGD(
              "one of BT/WIFI/FM is still opened(0x%x), do not shutdow CP2",
              pWcndManger->btwifi_state);
          wcnd_send_notify_to_client(pWcndManger,
                                     WCND_CMD_RESPONSE_STRING " OK",
                                     WCND_CLIENT_TYPE_CMD);
        }
      }
    ...
    case WCND_EVENT_CP2_ASSERT:
      WCND_LOGD("CP2 Assert Happens, current bi/wifi state: 0x%x",
                pWcndManger->btwifi_state);
    ...
    case WCND_EVENT_CP2_DOWN:
      ...
      WCND_LOGD(
          "Warning: Enter WCND_STATE_CP2_STOPPED from WCND_STATE_CP2_STOPPING");
...
static void wcn_state_cp2_assert(WcndManager *pWcndManger,
  ...
  if (!pWcndManger->btwifi_state)
    WCND_LOGE("WARNING: btwifi_state: 0x%x in state: WCND_STATE_CP2_ASSERT",
              pWcndManger->btwifi_state);
  ...
  switch (pMessage->event) {
    ...
    case WCND_EVENT_FM_CLOSE:
      WCND_LOGD(
          "WARNING: saved_btwifi_state: 0x%x in state: WCND_STATE_CP2_ASSERT",
          pWcndManger->saved_btwifi_state);
      if (pWcndManger->saved_btwifi_state) {
        if (pMessage->event == WCND_EVENT_BT_CLOSE)
          pWcndManger->saved_btwifi_state &= (~WCND_BTWIFI_STATE_BT_ON);
        else if (pMessage->event == WCND_EVENT_WIFI_CLOSE)
      ...
      wcnd_send_notify_to_client(pWcndManger, WCND_CMD_RESPONSE_STRING " OK",
                                 WCND_CLIENT_TYPE_CMD_SUBTYPE_CLOSE);
      break;

    case WCND_EVENT_BT_OPEN:
    case WCND_EVENT_WIFI_OPEN:
    case WCND_EVENT_FM_OPEN:
      WCND_LOGD("WARNING: btwifi_state: 0x%x in state: WCND_STATE_CP2_ASSERT",
                pWcndManger->btwifi_state);
    ...
    case WCND_EVENT_CP2_DOWN:
      if (pWcndManger->marlin_type == MARLIN_CHIP_TYPE_2) {
          if(!pWcndManger->is_in_userdebug && !pWcndManger->dumpmem_on && check_if_reset_enable()) {
              ...
              //if(pWcndManger->is_engineer_poweron) {
                  ...
                  //wcnd_process_atcmd(-1,"startwcn",pWcndManger);
              //}
      ...
      WCND_LOGE("ERROR:CP2 ASSERT RESET Failed, btwifi_state: 0x%x",
                pWcndManger->btwifi_state);
    ...
    case WCND_EVENT_CP2POWEROFF_REQ:
      WCND_LOGE(
          "WARNING: Receive WCND POWER ON/OFF EVENT at WCND_STATE_CP2_ASSERT, "
          "btwifi_state: 0x%x",
          pWcndManger->btwifi_state);
      if (!pWcndManger->doing_reset && (pMessage->replyto_fd > 0)) {
        WCND_LOGE(
            "WARNING: Receive WCND POWER ON/OFF EVENT at "
            "WCND_STATE_CP2_ASSERT, btwifi_state: 0x%x, and Does not doing "
            "reset",
            pWcndManger->btwifi_state);

        wcnd_notify_client(pWcndManger, WCND_CMD_RESPONSE_STRING " FAIL",
                           pMessage->replyto_fd, WCND_CLIENT_TYPE_CMD);
      }
      break;
...
static void wcn_state_cp2_stopping(WcndManager *pWcndManger,
                                   WcndMessage *pMessage) {
  int i = 0;
  ...
  if (pWcndManger->btwifi_state)
    WCND_LOGE("WARNING: btwifi_state: 0x%x in state: WCND_STATE_CP2_STOPPING",
              pWcndManger->btwifi_state);
...
int wcnd_sm_step(WcndManager *pWcndManger, WcndMessage *pMessage) {
  ...
  switch (pWcndManger->state) {
    ...
    default:
      ...
      WCND_LOGE("ERROR:Unknown CP2 STATE (%d) btwifi_state: 0x%x",
                pWcndManger->state, pWcndManger->btwifi_state);
      break;
  }
...
int wcnd_sm_init(WcndManager *pWcndManger) {
  ...
  if (!pWcndManger->is_wcn_modem_enabled) {
    ...
    pWcndManger->btwifi_state |=
        (WCND_BTWIFI_STATE_BT_ON | WCND_BTWIFI_STATE_WIFI_ON |
         WCND_BTWIFI_STATE_FM_ON);
  } else {
    pWcndManger->state = WCND_STATE_CP2_STOPPED;
    pWcndManger->notify_enabled = 0;
    // pWcndManger->btwifi_state |= (WCND_BTWIFI_STATE_BT_ON
    // |WCND_BTWIFI_STATE_WIFI_ON);
  }
