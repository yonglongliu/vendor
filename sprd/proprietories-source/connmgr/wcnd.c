...
void generate_wifi_mac() {
  ...
  // force replace configuration file if vaild mac is in factory configuration
  // file
...
WcndManager *wcnd_get_default_manager(void) { return &default_wcn_manager; }
...
*/
static void dispatch_command(WcndManager *pWcndManger, int client_fd,
...
static int process_active_client_fd(WcndManager *pWcndManger, int fd) {
  ...
  len = TEMP_FAILURE_RETRY(
      read(fd, buffer,
           sizeof(buffer) - 1));  // reserve last byte for null character.
  if (len < 0) {
...
static int is_cp2_alive_ok(WcndManager *pWcndManger, int is_loopcheck) {
  ...
  select_retry:
    if ((rc = select(max + 1, &read_fds, NULL, NULL, &timeout)) < 0) {
      if (errno == EINTR) goto select_retry;
    ...
    if (!(FD_ISSET(loop_fd, &read_fds))) {
      WCND_LOGD("select loop_fd(%d) return > 0, but loop_fd is not set!",
                loop_fd);
      close(loop_fd);
      return -1;
    }
...
polling_test:
  ...
  if (loop_test_count++ > MAX_LOOP_TEST_COUNT) {
    WCND_LOGE("%s: write test for %d counts, still failed return!!!!!",
              __func__, MAX_LOOP_TEST_COUNT);
    return -1;
  }
  ...
  pWcndManger->loopcheck_checked = 1;
  return 0;
}
...
do_cp2reset:
  ...
  if (reset_count++ > RESET_FAIL_RETRY_COUNT) {
    WCND_LOGE("%s Reset CP2 for %d times, still failed return!!!!!", __func__,
              RESET_FAIL_RETRY_COUNT);
  ...
  wcnd_send_notify_to_client(pWcndManger, WCND_CP2_RESET_START_STRING,
  ...
  wcnd_send_notify_to_client(pWcndManger, WCND_CP2_RESET_END_STRING,
                             WCND_CLIENT_TYPE_NOTIFY);
  ...
  if (is_alive_ok) {
    wcnd_send_notify_to_client(pWcndManger, WCND_CP2_ALIVE_STRING,
                               WCND_CLIENT_TYPE_NOTIFY);
  } else {
...
int wcnd_dump_cp2(WcndManager *pWcndManger) {
  ...
  //for BUG 643723 beacuse dump will hold CP2.so can't clear
  // clear
  //pWcndManger->is_cp2_error = 0;
...
// kill fm??
  ...
  // if(pWcndManger && pWcndManger->wait_wifi_driver_unloaded) //wait for wifi
  // driver to be unloaded, otherwise reset may fail
  if (pWcndManger->marlin_type != MARLIN_CHIP_TYPE_2) {
  ...
  // to do reset
  // move to handle WCND_EVENT_CP2_ASSERT in state machine
...
static int handle_cp2_assert(WcndManager *pWcndManger, int assert_fd) {
  ...
  if (pWcndManger->doing_reset) return 0;
...
* 3. send notify to client the cp2 reset completed
*/
static int handle_cp2_watchdog_exception(WcndManager *pWcndManger,
                                         int watchdog_fd) {
  if (!pWcndManger) {
    WCND_LOGE("%s: UNEXCPET NULL WcndManager", __FUNCTION__);
    return -1;
  ...
  if (len <= 0) {
    WCND_LOGE("read %d return %d, errno = %s", watchdog_fd, len,
              strerror(errno));
    // return -1;
  }
...
get_watchdogfd:
  ...
  WCND_LOGD("%s: open watchdog dev: %s, fd = %d", __func__,
            pWcndManger->wcn_watchdog_iface_name, watchdog_fd);
  if (watchdog_fd < 0) {
    WCND_LOGD("open %s failed, error: %s", pWcndManger->wcn_watchdog_iface_name,
              strerror(errno));
    sleep(2);
    if (count-- > 0) goto get_watchdogfd;
  }
  ...
  while (1) {
    ...
    if (watchdog_fd > 0) {
      ...
      if (watchdog_fd > max) max = watchdog_fd;
    }
    ...
    if(max < 0) {
      WCND_LOGE(
          "ERROR: have not fd to Select On (assert_fd = %d, watchdog_fd = %d, "
          "max=%d)",
          assert_fd, watchdog_fd, max);
      return NULL;
    }
    ...
    if ((rc = select(max + 1, &read_fds, NULL, NULL, NULL)) < 0) {
      ...
      WCND_LOGD("select failed assert_fd = %d, watchdog_fd = %d, max=%d",
                assert_fd, watchdog_fd, max);
      sleep(1);
      continue;
    } else if (!rc) {
...
static int create_listen_tcp_socket(unsigned short port) {
    int enable = 1;
    struct sockaddr_in addr;
    int sk = socket(AF_INET, SOCK_STREAM, 0);
    if (-1 == sk) {
        WCND_LOGE("socket failed  %s\n", strerror(errno));
        return -1;
    }
    if (setsockopt(sk, SOL_SOCKET, SO_REUSEADDR, &enable, sizeof(int)) < 0){
        WCND_LOGE("setsockopt error  errno = %s", strerror(errno));
        close(sk);
        sk = -1;
    }
    ...
    int err = bind(sk, (struct sockaddr*)(&addr),sizeof(addr));
    if (-1 == err) {
        WCND_LOGE("bind error is %d, errno = %s",err, strerror(errno));
        close(sk);
        return -1;
    }
    ...
    err = listen(sk, 5);
    if (-1 == err) {
        WCND_LOGE("listen error is %d, errno = %s",err, strerror(errno));
        close(sk);
        sk = -1;
    }
    return sk;
...
static int init(WcndManager *pWcndManger, int is_eng_only) {
  ...
  if (pWcndManger->is_eng_mode_only) {
    ...
    if(pWcndManger->is_in_userdebug) {
        pWcndManger->listen_fd = create_listen_tcp_socket(TCP_SOCKET_PORT);
        WCND_LOGE("pWcndManger->listen_fd is %d",pWcndManger->listen_fd);
    } else{
  ...
  snprintf(pWcndManger->wcn_assert_iface_name, WCND_MAX_IFACE_NAME_SIZE, "%s",
           WCN_ASSERT_IFACE);
  snprintf(pWcndManger->wcn_watchdog_iface_name, WCND_MAX_IFACE_NAME_SIZE, "%s",
           WCN_WATCHDOG_IFACE);
  ...
  snprintf(pWcndManger->wcn_download_iface_name, WCND_MAX_IFACE_NAME_SIZE, "%s",
           WCN_DOWNLOAD_IFACE);
  snprintf(pWcndManger->wcn_image_file_name, WCND_MAX_IFACE_NAME_SIZE, "%s",
           WCN_IMAGE_NAME);
  ...
  WCND_LOGD(
      ...
      pWcndManger->wcn_assert_iface_name, pWcndManger->wcn_watchdog_iface_name,
  ...
  // To check if "persist.sys.sprd.wcnreset" is set or not. if not set it to be
  // default "1"
...
#ifdef USE_MARLIN
  // check marlin type
  if (1 == wcnd_find("/sys/devices/", MARLIN2_IDNENTITY)||1 == wcnd_find("/sys/devices/platform/", MARLIN2_IDNENTITY)) {
    pWcndManger->marlin_type = MARLIN_CHIP_TYPE_2;
  } else if(1 == wcnd_find("/sys/devices/", MARLIN_IDNENTITY)||1 == wcnd_find("/sys/devices/platform/", MARLIN_IDNENTITY)) {
...
void handle_ge2_error(WcndManager *pWcndManger,char *info_str) {
  ...
  if (pWcndManger->doing_reset) return;
...
static void *cp2_loop_check_thread(void *arg) {
  ...
  // First wait for 20 seconds for CP2 to be ready, and then start doing LOOP
  // CHECK
  sleep(20);
  ...
  while (1) {
    usleep(LOOP_CHECK_INTERVAL_MSECS * 1000);
    //SPRD:bug 615920
    //Add a tag to make sure that cp2 would not be checked twice in 5 seconds
    if ((!pWcndManger->notify_enabled ||
        (pWcndManger->state != WCND_STATE_CP2_STARTED))||(pWcndManger->loopcheck_checked==1)) {
      //if cp2 has been checked, we will check it after 5 seconds
      WCND_LOGD("%s: cp2 has been checked , we will check it after 5 seconds!!",
                __FUNCTION__);
      pWcndManger->loopcheck_checked = 0;
      continue;
    }
    // cp2 exception happens just continue for next poll
    if (pWcndManger->is_cp2_error) {
      WCND_LOGD("%s: CP2 exception happened and not reset success!!",
                __FUNCTION__);
    ...
    while (count-- > 0) {
      ...
      if (is_cp2_alive_ok(pWcndManger, 0) <
          0) {  // marlin cannot support select, so use non-block mode
#else
      if (is_cp2_alive_ok(pWcndManger, 1) < 0) {
        ...
        is_loopcheck_fail = 1;
        pWcndManger->btwifi_state |= WCND_BTWIFI_STATE_LOOPCHECK_FAIL;
        if (count > 0) sleep(3);//first time is fail, wait 3s for next check
      } else {
        is_loopcheck_fail = 0;
        pWcndManger->btwifi_state &= ~WCND_BTWIFI_STATE_LOOPCHECK_FAIL;
...
void wcnd_set_armlog(WcndManager *pWcndManger) {

    char value[PROPERTY_VALUE_MAX] = {'\0'};

    property_get(WCND_CP2_LOG_KEY, value, "0");
    int is_enabled = atoi(value);
    if(is_enabled) {
        wcnd_send_selfcmd(pWcndManger, "wcn at+armlog=1");
	}
}
/**
...
static int store_cp2_version_info(WcndManager *pWcndManger) {
  ...
  if(pWcndManger->is_ge2_version == 0) {
	 wcnd_process_atcmd(-1, WCND_ATCMD_CP2_GET_CHIP, pWcndManger);
  }
...
int main(int argc, char *argv[]) {
  ...
  if (!pWcndManger->eng_autotest && !is_recovery) {
  ...
  if (!pWcndManger->is_eng_mode_only &&
    ...
    if (need_config_cp2) {
      ...
      WCND_LOGE("loop_check Fail when startup: (pWcndManger->state:%d) !!!",
                pWcndManger->state);
    }
  }
