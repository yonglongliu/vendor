...
static void *lsensor_thread()
{
	...
	if(strlen(SENSOR_DEV_NAME[SENSOR_PXY])){
		fd_pro = get_sensor_name(SENSOR_DEV_NAME[SENSOR_PXY]);
		if(fd_pro < 0) {
			ui_set_color(CL_RED);
			cur_row = ui_show_text(cur_row, 0, TEXT_SENSOR_OPEN_FAIL);
		}
	}else{
		LOGD("mmitest get pxy sensor %s failed",SENSOR_DEV_NAME[SENSOR_PXY]);
	}

	if(strlen(SENSOR_DEV_NAME[SENSOR_LUX])){
		fd_lux = get_sensor_name(SENSOR_DEV_NAME[SENSOR_LUX]);
		if(fd_lux < 0) {
			ui_set_color(CL_RED);
			cur_row = ui_show_text(cur_row, 0, TEXT_SENSOR_OPEN_FAIL);
		}
	}else{
		LOGD("mmitest get lux sensor %s failed",SENSOR_DEV_NAME[SENSOR_LUX]);
	}
...
//			}
			...
			switch(data.type){
				...
				case SENSOR_TYPE_PROXIMITY:
					...
					if(flush_value == 2 && proximity_value == 0 && data.distance > 0)
					{
						proximity_value = data.distance;
						proximity_pass = 1;
						lsensor_show();
					}
					else if(flush_value == 2)
					{
						proximity_value = data.distance;
						lsensor_show();
					}
		...
		if(light_pass == 1 && proximity_pass == 1){
			ui_push_result(RL_PASS);
			ui_set_color(CL_GREEN);
			ui_show_text(cur_row+2, 0, TEXT_TEST_PASS);
			gr_flip();
