...
void setSocketName(int tag, int socket0, int socket1)
{
    ...
    if (socket0 >= 0) {
	if (tag == 1){//input bug 726410
            snprintf(sockName, SOCKET_NAME_LEN - 1, "input:%s-%s-f%d", pidName, tidName, socket0);
	}else if (tag == 0){//bittube
	    snprintf(sockName, SOCKET_NAME_LEN - 1, "bittube:%s-%s-f%d", pidName, tidName, socket0);
	}else{
    ...
    if (socket1 >= 0) {
        if (tag == 1){//input bug 726410
            snprintf(sockName, SOCKET_NAME_LEN - 1, "input:%s-%s-f%d", pidName, tidName, socket1);
        }else if (tag == 0){//bittube
            snprintf(sockName, SOCKET_NAME_LEN - 1, "bittube:%s-%s-f%d", pidName, tidName, socket1);
        }else{
