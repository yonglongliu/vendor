...
int CpStorage::get_cur_top_dir(LogString& dir) const {
  int ret{-1};
  MediaStorage* cur_ms = m_stor_mgr.get_media_stor();

  if (cur_ms) {
    if (!cur_ms->check_top_dir(dir)) {
      ret = 0;
    }
  }
...
void CpStorage::on_file_size_update(int err) {
  ...
  if (m_cur_file->exists()) {
    ...
    if (LogFile::FIO_DISK_FULL == err) {  // Disk full
      if (m->overwrite()) {
        m->process_disk_full(cp_dir);
      } else {
        // Disk is full and log overwrite is disabled, so discard log.
        m_log_scheduler.discard_queue();
      }
    } else {
      if (m->check_quota()) {
