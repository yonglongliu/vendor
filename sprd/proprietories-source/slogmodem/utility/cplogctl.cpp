...
static int control_modem_log_output(char** argv, int argc, bool output) {
  if (1 != argc) {
    err_log("open_log/close_log command allows for one argument");
    fprintf(stderr, "open_log/close_log command allows for one argument\n");
    return 1;
  }

  if (strcmp(argv[0], "5mode")) {
    err_log("unsupported subsystem %s", argv[0]);
  ...
  if (output) {
    res = sendCmd(0, "AT+ARMLOG=1\r");
  } else {
    res = sendCmd(0, "AT+ARMLOG=0\r");
  }

  if (!strstr(res, "OK")) {
    err_log("control MODEM log output failed: %s", res);
    fprintf(stderr, "control MODEM log output failed: %s\n", res);
  }
...
int main(int argc, char** argv) {
  ...
  } else if (!strcmp(argv[1], "close_log")) {
    control_modem_log_output(argv + 2, argc - 2, false);
  ...
  } else if (!strcmp(argv[1], "open_log")) {
    control_modem_log_output(argv + 2, argc - 2, true);
