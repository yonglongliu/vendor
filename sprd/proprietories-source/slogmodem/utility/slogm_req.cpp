...
int SlogmRequest::connect_server() {
  int ret = -1;

  m_fd = socket(AF_INET, SOCK_STREAM, 0);
  if (-1 == m_fd) {
    err_log("can't create slogmodem client socket");
    fprintf(stderr, "create slogmodem client failed, errno = %d.\n", errno);
    return ret;
  }
  ...
  if (inet_pton(AF_INET, LOCAL_HOST, &addr.sin_addr) <= 0) {
    err_log("fail to convert to network address");
    fprintf(stderr, "Fail to convert to network adress, errno = %d.\n", errno);
  } else {
    if (connect(m_fd, (const struct sockaddr *)&addr, sizeof addr)) {
      err_log("can't connect to server");
      fprintf(stderr, "Connect to server refused, errno = %d.\n", errno);
    } else {
      ret = 0;
    }
  }
