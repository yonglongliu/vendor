...
class MediaStorage {
 public:
  ...
  void set_root_dir(const LogString& root_dir_path) {
    m_root_dir = root_dir_path;
  }
  const LogString& get_root_dir() const { return m_root_dir; }

  void set_parent_dir(const LogString& parent_dir) {
    m_parent_dir = parent_dir;
    m_log_dir = parent_dir + "/modem_log";
  }
  const LogString& get_top_dir() const { return m_log_dir; }

  /*  check_top_dir - check the top directory and return its path.
   *  @tdir: if the top directory is present or created, return the path
   *         in this parameter.
   *
   */
  int check_top_dir(LogString& tdir);
 ...
 private:
  ...
  // The parent directory of the modem_log directory
  ...
  // if no more space for saving, true
  bool m_store_full;
};
