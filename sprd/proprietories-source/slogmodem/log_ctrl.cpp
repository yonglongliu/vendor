...
int LogController::init(LogConfig* config) {
  ...
  if (m_cli_mgr->init(LOCAL_HOST, SLOG_MODEM_SERVER_PORT) < 0) {
    delete m_cli_mgr;
    m_cli_mgr = nullptr;
  }
  ...
  m_wcn_state = init_wcn_state_handler(WCN_STAT_SOCK_NAME,
                                       WCN_ASSERT_MESSAGE);
#endif
...
WcndStateHandler* LogController::init_wcn_state_handler(const char* sock_name,
                                                        const char* assert_msg) {
  WcndStateHandler* p =
      new WcndStateHandler(this, &m_multiplexer, sock_name, assert_msg);

  int err = p->init();
  if (err < 0) {
    delete p;
    p = nullptr;
    err_log("connect to %s error", sock_name);
  }

  return p;
}

WcndStateHandler* LogController::init_wcn_state_handler(const char* addr,
                                                        unsigned short port,
                                                        const char* assert_msg) {
  WcndStateHandler* p =
      new WcndStateHandler(this, &m_multiplexer, addr, port, assert_msg);

  int err = p->init();
  if (err < 0) {
    delete p;
    p = nullptr;
    err_log("connect to %s:%hu error", addr, port);
  }
...
void LogController::process_cp_evt(CpType type, CpStateHandler::CpEvent evt) {
  ...
  if (it == m_log_pipes.end()) {
    err_log("unknown MODEM type %d asserts, system MODEM %d",
            static_cast<int>(type),
            static_cast<int>(LogConfig::get_wan_modem_type()));
    return;
  }
...
int LogController::save_sipc_dump(CpStorage* stor, const struct tm& lt) {
  ...
  // /d/sipc/smsg
  snprintf(fn, sizeof fn, "smsg_%04d-%02d-%02d_%02d-%02d-%02d.log",
  ...
  // /d/sipc/sbuf
  snprintf(fn, sizeof fn, "sbuf_%04d-%02d-%02d_%02d-%02d-%02d.log",
           year, mon, lt.tm_mday, lt.tm_hour, lt.tm_min, lt.tm_sec);
  ...
  // /d/sipc/sblock
  snprintf(fn, sizeof fn, "sblock_%04d-%02d-%02d_%02d-%02d-%02d.log",
           year, mon, lt.tm_mday, lt.tm_hour, lt.tm_min, lt.tm_sec);
  f = stor->create_file(LogString(fn), LogFile::LT_SIPC);
  if (f) {
  ...
  // /d/sipc/mbox
  snprintf(fn, sizeof fn, "mailbox_%04d-%02d-%02d_%02d-%02d-%02d.log",
           year, mon, lt.tm_mday, lt.tm_hour, lt.tm_min, lt.tm_sec);
  f = stor->create_file(LogString(fn), LogFile::LT_SIPC);
  if (f) {
...
int LogController::save_etb(CpStorage* stor, const struct tm& lt) {
  ...
  snprintf(fn, sizeof fn, "etb_%04d-%02d-%02d_%02d-%02d-%02d.log",
           year, mon, lt.tm_mday, lt.tm_hour, lt.tm_min, lt.tm_sec);

  ret = -1;
  LogFile* f = stor->create_file(LogString(fn), LogFile::LT_UNKNOWN);
  if (f) {
