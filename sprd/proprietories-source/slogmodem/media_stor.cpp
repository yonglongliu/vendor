...
int MediaStorage::init(FileWatcher* fw) {
  ...
  if ((m_stor_mgr->is_modem_log_in_ap_log()) &&
      access(ls2cstring(m_parent_dir), F_OK)) {
    int err = mkdir(ls2cstring(m_parent_dir),
                    S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
    if (-1 == err && EEXIST != errno) {
      err_log("create top dir %s error", ls2cstring(m_parent_dir));
      return -1;
    }
  }

  int err = mkdir(ls2cstring(m_log_dir),
                  S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);

  if (-1 == err && EEXIST != errno) {
    err_log("create log dir %s error", ls2cstring(m_log_dir));
    return -1;
  }

  DIR* pd = opendir(ls2cstring(m_log_dir));
  if (!pd) {
    err_log("opendir %s error", ls2cstring(m_log_dir));
    return -1;
  }
...
int MediaStorage::check_top_dir(LogString& tdir) {
  int ret{-1};
  int err = mkdir(ls2cstring(m_log_dir),
                  S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
...
bool MediaStorage::is_media_full() {
  ...
  if (!statvfs(ls2cstring(m_root_dir), &vstat)) {
    if (vstat.f_bsize / vstat.f_frsize * vstat.f_bfree
        <= vstat.f_blocks / 100 * 5) {
      is_full = true;
    }
  }
...
int MediaStorage::check_quota() {
  ...
  if (m_overwrite) {
    ...
    if (is_media_full()) {
      m_store_full = true;
      trim_size = m_file_limit;
      info_log("media full: trim %lu",
               static_cast<unsigned long>(trim_size));
    } else if (m_limit && m_size > m_limit) {
      trim_size = m_size - m_limit;
    } else {  // Not full, no limit
      return 0;
    }
  } else if (!m_limit || m_size < m_limit) {  // More quota
    return 0;
  } else {  // No more quota
    return -1;
  }

  // Some size need to be trimmed.
  if (trim(trim_size)) {
    m_store_full = false;
    return 0;
  } else {
...
LogFile* MediaStorage::create_file(const LogString& cp_name,
  ...
  if (prepare_cur_set()) {
    if (LogFile::LT_LOG == t) {
      f = m_cur_set->recreate_log_file(cp_name, new_cp_dir, flags);
    } else {
      f = m_cur_set->create_file(cp_name, fname, t, flags);
    }
  }
...
CpSetDirectory* MediaStorage::prepare_cur_set() {
  // If modem log is located in ap log, check if ap log accessible
  if ((m_stor_mgr->is_modem_log_in_ap_log()) &&
      access(ls2cstring(m_parent_dir), F_OK)) {
    int err =
    ...
    if (err && EEXIST != errno) {
      err_log("create parent dir (%s) error", ls2cstring(m_parent_dir));
      return 0;
    }
  }
