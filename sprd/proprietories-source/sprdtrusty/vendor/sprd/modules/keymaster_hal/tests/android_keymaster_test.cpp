...
#include <include/keymaster/soft_keymaster_device.h>
#endif
...
#if (TRUSTY_KEYMASTER_VERSION == 2)
/**
 * Test instance creator that builds a pure trusty keymaster2 implementation.
 */
class TrustyKeymasterTestInstanceCreator : public Keymaster2TestInstanceCreator {
  public:
    keymaster2_device_t* CreateDevice() const override {
        //std::cerr << "Creating trusty V2.0 device" << std::endl;
        const hw_module_t *mod;
        hw_get_module_by_class(KEYSTORE_HARDWARE_MODULE_ID, NULL, &mod);
        TrustyKeymasterDevice* device = new TrustyKeymasterDevice(mod);
        ...
        device->device()->configure(device->device(), &version_info);
    ...
    string name() const override { return "Trusty Keymaster2"; }
};
...
/**
 * Test instance creator that builds a TrustyKeymasterDevice which wraps a fake hardware keymaster1
...
class Keymaster1TestInstanceCreator : public Keymaster2TestInstanceCreator {
    ...
    hw_get_module_by_class(KEYSTORE_HARDWARE_MODULE_ID, NULL, &mod);
    printf("Found keymaster1 module %s, version %x\n", mod->name, mod->module_api_version);
    mod->methods->open(mod,KEYSTORE_KEYMASTER, (struct hw_device_t**)&km1_device);
        context_ = new TestKeymasterContext;
        soft_keymaster = new SoftKeymasterDevice(context_);
        soft_keymaster->SetHardwareDevice(km1_device);
        dev = soft_keymaster->keymaster2_device();
        AuthorizationSet version_info(AuthorizationSetBuilder()
                                          .Authorization(TAG_OS_VERSION, kOsVersion)
                                          .Authorization(TAG_OS_PATCHLEVEL, kOsPatchLevel));
        dev->configure(dev, &version_info);
        return dev;
    }

    bool algorithm_in_km0_hardware(keymaster_algorithm_t ) const override{ return false; }
    KeymasterContext* keymaster_context() { return context_; }
    int keymaster0_calls() const override { return 0; }
    int minimal_digest_set() const override { return false; }
    bool is_keymaster1_hw() const override { return true; }
    string name() const override { return "Trusty Keymaster1"; }
  private:
    mutable TestKeymasterContext* context_;
    mutable SoftKeymasterDevice *soft_keymaster ;
};
#endif
static auto test_params = testing::Values(
#if (TRUSTY_KEYMASTER_VERSION == 2)
    InstanceCreatorPtr(new TrustyKeymasterTestInstanceCreator)
#else
...
class NewKeyGeneration : public Keymaster2Test {
  protected:
    void CheckBaseParams() {
        ...
        AuthorizationSet tee_auths = hw_enforced();
        EXPECT_GT(auths.SerializedSize(), 12U);

        EXPECT_TRUE(contains(tee_auths, TAG_PURPOSE, KM_PURPOSE_SIGN));
        EXPECT_TRUE(contains(tee_auths, TAG_PURPOSE, KM_PURPOSE_VERIFY));
        EXPECT_TRUE(contains(auths, TAG_USER_ID, 7));
        EXPECT_TRUE(contains(tee_auths, TAG_USER_AUTH_TYPE, HW_AUTH_PASSWORD));
        EXPECT_TRUE(contains(tee_auths, TAG_AUTH_TIMEOUT, 300));
...
static string read_file(const string& file_name) {
    string path="/data/cer/" + file_name;
    ifstream file_stream(path, std::ios::binary);
...
TEST_P(ImportKeyTest, EcdsaSuccess) {
    ...
    if (!GetParam()->algorithm_in_km0_hardware(KM_ALGORITHM_EC))
        ...
        EXPECT_TRUE(contains(sw_enforced(), TAG_ORIGIN, KM_ORIGIN_UNKNOWN));
    EXPECT_TRUE(contains(sw_enforced(), KM_TAG_CREATION_DATETIME));
...
static bool verify_chain(const keymaster_cert_chain_t& chain) {
    if(chain.entry_count == 0) return false;
...
static bool verify_attestation_record(const string& challenge,
    ...
    EXPECT_NE(expected_keymaster_version, att_keymaster_version);
#else
    ...
    if (expected_sw_enforced.GetTagValue(TAG_ATTESTATION_APPLICATION_ID,&attestation_application_id))
        att_sw_enforced.push_back(TAG_ATTESTATION_APPLICATION_ID, attestation_application_id);
...
TEST_P(AttestationTest, RsaAttestation) {
    ...
    if(cert_chain.entry_count ==0)return;
