...
TEST_P(StorageServiceTest, Write512) {
    int rc;
    file_handle_t handle;
    size_t blk = 512;
    size_t file_size = 512;
    const char *fname = "test_persistent_512_file";
...
TEST_P(StorageServiceTest, Read512) {
    int rc;
    file_handle_t handle;
    const char *fname = "test_persistent_512_file";
...
TEST_P(StorageServiceTest, Delete512) {
    int rc;
    const char *fname = "test_persistent_512_file";
    rc = storage_delete_file(session_, fname, STORAGE_OP_COMPLETE);
    rc = (rc == -ENOENT) ? 0 : rc;
...
TEST_P(StorageServiceTest, Write2048) {
    int rc;
    file_handle_t handle;
    size_t blk = 2048;
    size_t file_size = 2048;
    const char *fname = "test_persistent_2048_file";
...
TEST_P(StorageServiceTest, Read2048) {
    int rc;
    file_handle_t handle;
    const char *fname = "test_persistent_2048_file";
    ...
    ReadPattern(handle, 0, 2048, 2048);
    ASSERT_FALSE(HasFatalFailure());
...
TEST_P(StorageServiceTest, Delete2048) {
    int rc;
    const char *fname = "test_persistent_2048_file";
    rc = storage_delete_file(session_, fname, STORAGE_OP_COMPLETE);
    rc = (rc == -ENOENT) ? 0 : rc;
...
TEST_P(StorageServiceTest, WriteReadSequential) {
    ...
    WritePattern(handle, 0, blk, blk, true);
    ASSERT_FALSE(HasFatalFailure());
...
// Persistent 128k
TEST_P(StorageServiceTest, CreatePersistent128K_4040) {
    int rc;
    file_handle_t handle;
    size_t file_size = 128 * 1024;
    const char *fname = "test_persistent_128K_file";
...
TEST_P(StorageServiceTest, CreatePersistent128K_2032) {
    int rc;
    file_handle_t handle;
    size_t file_size = 128 * 1024;
    const char *fname = "test_persistent_128K_file";
...
TEST_P(StorageServiceTest, CreatePersistent128K_496) {
    int rc;
    file_handle_t handle;
    size_t file_size = 128 * 1024;
    const char *fname = "test_persistent_128K_file";
...
TEST_P(StorageServiceTest, CreatePersistent128K_240) {
    int rc;
    file_handle_t handle;
    size_t file_size = 128 * 1024;
    const char *fname = "test_persistent_128K_file";
...
TEST_P(StorageServiceTest, ReadPersistent128K_4040) {
    int rc;
    file_handle_t handle;
    size_t exp_len = 128 * 1024;
    const char *fname = "test_persistent_128K_file";
...
TEST_P(StorageServiceTest, ReadPersistent128K_2032) {
    int rc;
    file_handle_t handle;
    size_t exp_len = 128 * 1024;
    const char *fname = "test_persistent_128K_file";
...
TEST_P(StorageServiceTest, ReadPersistent128K_496) {
    int rc;
    file_handle_t handle;
    size_t exp_len = 128 * 1024;
    const char *fname = "test_persistent_128K_file";
...
TEST_P(StorageServiceTest, ReadPersistent128K_240) {
    int rc;
    file_handle_t handle;
    size_t exp_len = 128 * 1024;
    const char *fname = "test_persistent_128K_file";
...
TEST_P(StorageServiceTest, CleanUpPersistent128K) {
    int rc;
    const char *fname = "test_persistent_128K_file";
    rc = storage_delete_file(session_, fname, STORAGE_OP_COMPLETE);
    rc = (rc == -ENOENT) ? 0 : rc;
...
TEST_P(StorageServiceTest, TransactDiscardWriteMany) {
    ...
    file_handle_t handle2;
    size_t blk = 1024;
    size_t exp_len1 = 8 * 512;
    size_t exp_len2 = 7 * 512;
    storage_off_t fsize = (storage_off_t)(-1);
...
TEST_P(StorageServiceTest, TransactRewriteExistingTruncate) {
    ...
    for (uint i = 1; i < 8; i++) {
        ...
        // write data (with commit)
        WritePattern(handle, 0, (8 - i) * blk, blk, true);
        ASSERT_FALSE(HasFatalFailure());
...
TEST_P(StorageServiceTest, TransactRewriteExistingSetSize) {
    ...
    for (uint i = 1; i < 8; i++) {
        ...
        // write data (with commit)
        WritePattern(handle, 0, (8 - i) * blk, blk, false);
        ASSERT_FALSE(HasFatalFailure());

        // update size (with commit)
        rc = storage_set_file_size(handle, (8 - i) * blk, STORAGE_OP_COMPLETE);
        ASSERT_EQ(0, rc);
