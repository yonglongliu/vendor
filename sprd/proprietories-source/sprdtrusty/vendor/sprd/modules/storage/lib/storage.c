...
{
    struct storage_msg msg = { .cmd = STORAGE_FILE_RENAME, .flags = _to_msg_flags(opflags)};
    struct storage_file_rename_req req;
    struct iovec tx[2] = {{&msg, sizeof(msg)}, {&req, sizeof(req)}};
    struct iovec rx[1] = {{&msg, sizeof(msg)}};
    int old_len = 0, new_len = 0;

    old_len = strlen(old_file_name);
    new_len = strlen(new_file_name);
    if (old_len > FILE_NAME_MAX || new_len > FILE_NAME_MAX) {
        ALOGE("invalid file name lenth (max:%d).\n", FILE_NAME_MAX);
        return -EINVAL;
    }

    memcpy(req.old_file_name, old_file_name, old_len);
    req.old_file_name[old_len] = '\0';

    memcpy(req.new_file_name, new_file_name, new_len);
    req.old_file_name[new_len] = '\0';
                      ...
                      const void *buf, size_t size, uint32_t opflags)
{
    ...
    printf("storage_write(%d) use %ld us\n", chunk,
