...
void ipc_disconnect(void)
{
    ...
    ALOGE("ipc_disconnect(%d) to disconnect storage server\n", tipc_fd);
    tipc_close(tipc_fd);
    tipc_fd = -1;
}
...
int ipc_respond(struct storage_msg *msg, void *out, size_t out_size)
{
    ...
    printf("%s: writv(0x%x, msg->cmd 0x%d) in %ld us\n", __func__, tipc_fd, msg->cmd,
...
int ipc_send_message(struct storage_msg *msg)
{
    int rc;
   ...
   // struct iovec rx[1] = {{msg, sizeof(*msg)}};
    ...
    rc = writev(tipc_fd, tx, 1);
    if (rc < 0) {
        ALOGE("error sending message 0x%x: %s\n",
              msg->cmd, strerror(errno));
        return -1;
    }
...
#if PRINT_TIME
    gettimeofday(&end, NULL);
    printf("%s: ipc_send_msg(0x%x, msg->cmd 0x%d) in %ld us\n", __func__, tipc_fd, msg->cmd,
        (1000000 *  end.tv_sec + end.tv_usec));
#endif
