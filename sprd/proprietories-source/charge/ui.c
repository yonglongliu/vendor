...
static void res_init(void)
{
	...
	for(i = 0; i<=6; i++){
		sprintf(&gIndex[i], "%s%d", gIndeterminate,i);
		strncat(&gIndex[i], temp, 30);
		LOGD("picture is %s\n",gIndex[i]);
	}
	for(j = 0;j<10;j++){
		sprintf(&gNoIndex[j], "%s_%d", gNo,j);
		strncat(&gNoIndex[j], temp, 30);
		LOGD("number is %s\n",gNoIndex[j]);
	}
	strncat(gPer, temp, 30);
}
