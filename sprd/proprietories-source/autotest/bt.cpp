...
void btDeviceFoundCallback(int num_properties, bt_property_t *properties)
{
	...
	DBGMSG("nameLen=%d",nameLen);
	memset(sRemoteDev[sRemoteDevNum].name, 0, sizeof(sRemoteDev[sRemoteDevNum].name));
...
int btClose(void)
{
    ...
    if(NULL == sBtInterface){
        ERRMSG("sBtInterface is NULL!\n");
