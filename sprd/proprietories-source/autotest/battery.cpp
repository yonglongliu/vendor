...
#define TEMP_STATE "/sys/class/power_supply/battery/temp"
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
int  batGetTempureValue( char *bat_temp)
{
    FUN_ENTER;
    int fd = open(TEMP_STATE, O_RDONLY);
    DBGMSG("BatteryTempureValue  fd= %d\n", fd);
    if( fd < 0 ) {
        ERRMSG("open '%s' error: %s\n", TEMP_STATE, strerror(errno));
        return -1;
    }
    read(fd, bat_temp, 4);
    DBGMSG(" BatteryTempureValue  bat_temp=%s \n", bat_temp);
     close(fd);
    FUN_EXIT;
    return 0;
}
