...
static inline void mali_freq_init(struct device *dev)
{
	...
	gpu_dfs_ctx.freq_list_len = of_property_count_elems_of_size(dev->of_node,"sprd,dfs-lists",3*sizeof(u32));
	...
	for(i=0; i<gpu_dfs_ctx.freq_list_len; i++)
	{
		...
		of_property_read_u32_index(dev->of_node, "sprd,dfs-lists", 3*i+1, &clk);
		...
		of_property_read_u32_index(dev->of_node, "sprd,dfs-lists", 3*i,   &gpu_dfs_ctx.freq_list[i].freq);
		of_property_read_u32_index(dev->of_node, "sprd,dfs-lists", 3*i+2, &gpu_dfs_ctx.freq_list[i].div);
...
static inline void mali_power_on_late(struct kbase_device *kbdev)
{
	//mali power security key
	kbase_reg_write(kbdev, GPU_CONTROL_REG(PWR_KEY), 0x2968a819, NULL);

	//mali power cfg override
	kbase_reg_write(kbdev, GPU_CONTROL_REG(PWR_OVERRIDE1), 0xf0cb032c, NULL);
...
struct kbase_platform_funcs_conf platform_whale2_funcs = {
...
struct kbase_pm_callback_conf pm_whale2_callbacks = {
