...
static inline void mali_freq_init(struct device *dev)
{
	...
	gpu_dfs_ctx.gpu_top_reg = ioremap_nocache(pmu_phy_base+REG_PMU_APB_PD_GPU_TOP_PWR_CFG1, 4);
	of_node_put(pmu_np);
...
struct kbase_platform_funcs_conf platform_isharkl2_funcs = {
...
struct kbase_pm_callback_conf pm_isharkl2_callbacks = {
