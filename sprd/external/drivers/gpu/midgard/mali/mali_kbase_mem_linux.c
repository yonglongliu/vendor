...
//	    (reg->flags & (KBASE_REG_CPU_WR|KBASE_REG_CPU_RD))) {
	...
	if (is_wc) {
		...
		if (nr_pages > 0)
		{
			struct page **array_wc_pages;
			int j;
			array_wc_pages = drm_malloc_ab(sizeof(struct page *), nr_pages);
			if (NULL == array_wc_pages)
			{
				KBASE_DEBUG_PRINT(2, "malloc for x86 page_size=%ld, nr_pages=%ld\n", sizeof(struct page *), nr_pages);
				WARN_ON(1);
				err = -ENOMEM;
				kfree(map);
				goto out;
			...
			for (i = 0, j = 0; i < nr_pages; i++) {
				...
				if (get_page_memtype(pg) !=  _PAGE_CACHE_MODE_WC)
					array_wc_pages[j++] = pg;
			}
			set_pages_array_wc(array_wc_pages, j);
			drm_free_large(array_wc_pages);
		}
#endif
