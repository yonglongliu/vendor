...
int kbasep_pm_metrics_init(struct kbase_device *kbdev)
{
    ...
    kbdev->pm.backend.metrics.hotplug_time_period_start = ktime_get();
	kbdev->pm.backend.metrics.hotplug_time_busy = 0;
	kbdev->pm.backend.metrics.hotplug_time_idle = 0;
#endif
 ...
 * function
 */
static void kbase_pm_get_hotplug_utilisation_calc(struct kbase_device *kbdev,
								ktime_t now)
{
	ktime_t diff;
	...
	diff = ktime_sub(now, kbdev->pm.backend.metrics.hotplug_time_period_start);
	if (ktime_to_ns(diff) < 0)
		return;
	...
	if (kbdev->pm.backend.metrics.gpu_active) {
		...
		kbdev->pm.backend.metrics.hotplug_time_busy += ns_time;
	} else {
		kbdev->pm.backend.metrics.hotplug_time_idle += (u32) (ktime_to_ns(diff)
							>> KBASE_PM_TIME_SHIFT);
	}

	kbdev->pm.backend.metrics.hotplug_time_period_start = now;
}

int kbase_pm_get_hotplug_utilisation(struct kbase_device *kbdev)
{
	int utilisation = 0;
	u32 busy_time = 0, idle_time = 0;
	unsigned long flags;
	...
	//get busy/idle time
	kbase_pm_get_hotplug_utilisation_calc(kbdev, now);
	busy_time = kbdev->pm.backend.metrics.hotplug_time_busy;
	idle_time = kbdev->pm.backend.metrics.hotplug_time_idle;

	//get utilisation
	if (0 < (busy_time + idle_time))
	{
		utilisation = (100 * busy_time) / (busy_time + idle_time);
	}

	//reset hotplug busy/idle time
	kbdev->pm.backend.metrics.hotplug_time_busy = 0;
	kbdev->pm.backend.metrics.hotplug_time_idle = 0;
...
KBASE_EXPORT_TEST_API(kbase_pm_get_hotplug_utilisation);
...
#ifdef CONFIG_MALI_HOTPLUG
	kbase_pm_get_hotplug_utilisation_calc(kbdev, *timestamp);
#endif
