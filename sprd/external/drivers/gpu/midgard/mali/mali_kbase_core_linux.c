...
copy_failed:
	...
	case KBASE_FUNC_SET_BOOST:
		{
			struct kbase_uk_set_boost *gpu_boost = args;

			kctx->jctx.level = gpu_boost->level;
		}
		break;
#endif
...
#if KERNEL_VERSION(4, 4, 49) > LINUX_VERSION_CODE
/*
...
#ifdef CONFIG_MALI_HOTPLUG
/** Show callback for the @c limit_cores sysfs file.
 *
 * This function is called to get the contents of the @c limit_cores sysfs
 * file.
 *
 * @param dev	The device this sysfs file is for
 * @param attr	The attributes of the sysfs file
 ...
 * @return The number of bytes output to @c buf.
 */
static ssize_t show_limit_cores(struct device *dev, struct device_attribute *attr, char * const buf)
{
	struct kbase_device *kbdev;
	ssize_t ret = 0;
	...
	ret = scnprintf(buf, PAGE_SIZE,
			"Current limit max cores : %d\n",
			kbase_num_limit_cores(kbdev));

	return ret;
}

/** Store callback for the @c limit_cores sysfs file.
 *
 * This function is called when the @c limit_cores sysfs file is written to.
 *
 * @param dev	The device with sysfs file is for
 * @param attr	The attributes of the sysfs file
 * @param buf	The value written to the sysfs file
 ...
 * @return @c count if the function succeeded. An error code on failure.
 */
static ssize_t set_limit_cores(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct kbase_device *kbdev;
	int ret, limit_cores;
	...
	ret = kstrtoint(buf, 0, &limit_cores);
	if (ret || limit_cores <= 0) {
		dev_err(kbdev->dev, "Couldn't process limit_cores write operation.\n");
		return -EINVAL;
	}

	kbase_set_max_cores(kbdev, limit_cores);

	return count;
}

/** The sysfs file @c limit_cores.
 *
 * This is used to limiit shader core availability for debugging purposes.
 * Reading it will show the current max cores.
 * Writing to it will set the current max cores.
 */
static DEVICE_ATTR(limit_cores, S_IRUGO | S_IWUSR, show_limit_cores, set_limit_cores);
#endif
...
#ifdef CONFIG_MALI_HOTPLUG
	if (kbdev->inited_subsys & inited_hotplug) {
		kbase_hotplug_term(kbdev);
		kbdev->inited_subsys &= ~inited_hotplug;
	}
#endif /* CONFIG_MALI_HOTPLUG */
...
#ifdef CONFIG_MALI_HOTPLUG
	err = kbase_hotplug_init(kbdev);
	if (err) {
		dev_err(kbdev->dev, "Couldn't initialize hotplug\n");
		kbase_platform_device_remove(pdev);
		return -EINVAL;
	}
	kbdev->inited_subsys |= inited_hotplug;
