		  ...
		  size_t length, loff_t *offset)
{
	...
	dev_dbg(tee->dev, "%s > ctx %p\n",__func__, ctx);

	rpc = tee->rpc;

	if (atomic_read(&rpc->used) == 0) {
		dev_err(tee->dev, "%s: rpc->used %p, ERROR Supplicant application NOT ready\n"
				, __func__, &rpc->used);
		ret = -EPERM;
		goto out;
	}

	//printk(KERN_ERR "wait for datatouser\n");
	if (down_interruptible(&rpc->datatouser))
		return -ERESTARTSYS;
	//printk(KERN_ERR "got datatouser!");

	dev_dbg(tee->dev, "> %s tgid:[%d]\n",__func__, task->tgid);
...
int tee_supp_init(struct tee *tee)
{
	...
	dev_dbg(tee->dev, "%s rpc->used %p is set zero\n",__func__, &rpc->used);
	tee->rpc = rpc;
	return 0;
}
