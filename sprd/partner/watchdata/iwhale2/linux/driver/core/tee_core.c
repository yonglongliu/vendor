...
static char *_tee_supp_app_name = "wdtee_listener";

/* Store the class misc reference */
static struct class *wd_misc_class;
...
struct tee *tee_get_tee(const char *devname)
{
	...
	if (!devname)
		return NULL;
	device = class_find_device(wd_misc_class, NULL, devname, device_match);
	if (!device) {
...
static int tee_supp_open(struct tee *tee)
{
	...
	if (strncmp(_tee_supp_app_name, current->comm,
		...
		dev_dbg(_DEV(tee), "%s: tee->rpc->used %p\n", __func__, &tee->rpc->used);
	}
...
static void tee_supp_release(struct tee *tee)
{
	...
	if ((atomic_read(&tee->rpc->used) == 1) &&
			(strncmp(_tee_supp_app_name, current->comm,
					strlen(_tee_supp_app_name)) == 0))
		atomic_sub(1, &tee->rpc->used);
	dev_dbg(_DEV(tee), "%s > : rpc->used %p sub 1\n", __func__, &tee->rpc->used);
}
...
extern void* g_shm_vaddr;
extern uint32_t reg[];
extern unsigned long g_shm_paddr;
ssize_t tee_intel_read(struct file *filp, char __user *buffer,
		  size_t length, loff_t *offset){
	struct tee *tee;

	tee = filp->private_data;
	BUG_ON(!tee);

	//printk(KERN_ERR "[%s] : wait for intel_datatouser\n",__func__);
	if (down_interruptible(&tee->intel_datatouser))
		return -ERESTARTSYS;
	//printk(KERN_ERR "[%s] : got intel_datatouser\n",__func__);
	if(copy_to_user(buffer, reg, 32)){
		dev_err(_DEV(tee), "%s: copy_to_user failed\n", __func__);
		return -1;
	}
	return 32;
}

ssize_t tee_intel_write(struct file *filp, const char __user *buffer,
		   size_t length, loff_t *offset){
	struct tee *tee;
	...
	//printk(KERN_ERR "[%s] : get data to sys\n",__func__);

	if (copy_from_user(reg, buffer, 32)) {
		dev_err(_DEV(tee), "%s: copy_from_user failed\n", __func__);
		return -1;
	}
	up(&tee->intel_datatosys);

	return 0;
}

static int tee_intel_open(struct inode *inode, struct file *filp){
	struct tee *tee;

	tee = container_of(filp->private_data, struct tee, intel_miscdev);

	BUG_ON(!tee);
	BUG_ON(tee->intel_miscdev.minor != iminor(inode));

	dev_dbg(_DEV(tee), "%s: > name=\"%s\"\n", __func__, tee->intel_name);

	mutex_lock(&tee->intel_lock);

	filp->private_data = tee;

	return 0;
}

static int tee_intel_release(struct inode *inode, struct file *filp){
	struct tee *tee;
	...
	mutex_unlock(&tee->intel_lock);
	
	return 0;
}
#if 0
static int tee_intel_mmap(struct file *filp, struct vm_area_struct *vma)
{
	struct tee *tee;
	...
	if (g_shm_vaddr != NULL) {
		printk(KERN_ERR "[%s] 0x%x => 0x%x (+%zu)\n", __func__,
				(unsigned int)g_shm_paddr, (unsigned int)vma->vm_start, size);
#if 1
		...
		if (remap_pfn_range(vma, vma->vm_start, g_shm_paddr >> PAGE_SHIFT, size, prot))
			return -EAGAIN;
		BUG_ON(vma->vm_private_data != NULL);
		vma->vm_private_data = (void*)g_shm_paddr;
	}else{
		printk(KERN_ERR "[%s] : share memory not ready", __func__);
	}
...
const struct file_operations tee_intel_fops = {
	.owner = THIS_MODULE,
	.read = tee_intel_read,
	.write = tee_intel_write,
	.open = tee_intel_open,
	/*.mmap = tee_intel_mmap,*/
	.release = tee_intel_release,
};
//add end
			   ...
			   const struct tee_ops *ops, size_t len)
{
	...
	snprintf(tee->name, sizeof(tee->name), "wdtee%s%02d", name, tee->id);
	...
	//add by tzy on 2016-8-26
	snprintf(tee->intel_name, sizeof(tee->intel_name), "wdtee_%s%02d", "intel", tee->id);

	tee->intel_miscdev.parent = dev;
	tee->intel_miscdev.minor = MISC_DYNAMIC_MINOR;
	tee->intel_miscdev.name = tee->intel_name;
	tee->intel_miscdev.fops = &tee_intel_fops;
	mutex_init(&tee->intel_lock);
	tee->intel_datatouser = (struct semaphore)
	    __SEMAPHORE_INITIALIZER(tee->intel_datatouser, 0);

	tee->intel_datatosys= (struct semaphore)
	    __SEMAPHORE_INITIALIZER(tee->intel_datatosys, 0);

	tee->intel_debugfs = (struct semaphore)
	    __SEMAPHORE_INITIALIZER(tee->intel_debugfs, 0);
	tee->intel_debugfs_1 = (struct semaphore)
	    __SEMAPHORE_INITIALIZER(tee->intel_debugfs_1, 0);
	//add end

	pr_info("TEE core: Alloc the misc device \"%s\" (id=%d)\n", tee->intel_name, tee->id);
...
int tee_core_add(struct tee *tee)
{
    ...
    if(tee->miscdev.this_device->class == NULL){
        printk(KERN_ERR "-------------------------%s %s %s %d :class is : %p-------------------------\n",__DATE__,__TIME__,__func__,__LINE__,tee->miscdev.this_device->class);
        return -EINVAL;
    }
    ...
    if (wd_misc_class)
        BUG_ON(wd_misc_class != tee->miscdev.this_device->class);
    else
        wd_misc_class = tee->miscdev.this_device->class;
    ...
    //add by tzy on 2016-8-26
    rc = misc_register(&tee->intel_miscdev);
    if (rc != 0) {
        pr_err("TEE Core: misc_register() failed name=\"%s\"\n",
                tee->intel_name);
        return rc;
    ...
    dev_set_drvdata(tee->intel_miscdev.this_device, tee);

    /* Register a static reference on the class misc
     * to allow finding device by class */

    pr_info("TEE Core: Register the misc device \"%s\" (id=%d,minor=%d)\n",
            dev_name(tee->intel_miscdev.this_device), tee->id,
            tee->intel_miscdev.minor);
    //add end
...
int tee_core_del(struct tee *tee)
{
    if (tee) {
        ...
        //add by tzy on 2016-8-26
        if (tee->intel_miscdev.minor != MISC_DYNAMIC_MINOR) {
            pr_info("TEE Core: Deregister the misc device \"%s\" (id=%d)\n",
                    dev_name(tee->intel_miscdev.this_device), tee->id);
            misc_deregister(&tee->intel_miscdev);
        }
        //add end
    }
