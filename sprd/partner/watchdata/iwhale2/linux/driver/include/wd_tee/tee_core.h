...
struct tee {
	...
	struct miscdevice intel_miscdev;
	struct mutex intel_lock;
	struct semaphore intel_datatouser;
	struct semaphore intel_datatosys;
	struct semaphore intel_debugfs;
	struct semaphore intel_debugfs_1;
	char intel_name[TEE_MAX_TEE_DEV_NAME];
	//add end
	struct tee_rpc *rpc;
