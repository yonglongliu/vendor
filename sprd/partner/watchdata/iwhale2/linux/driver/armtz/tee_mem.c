				     ...
				     void *shm_vaddr, unsigned long shm_paddr)
{
	...
	//cl_size = 64;
	dev_dbg(dev, "wdtee cache line size %d\n", cl_size);
...
size_t tee_shm_pool_check(struct device *dev, struct shm_pool *pool)
{
	...
	dev_dbg(dev, "> %s pool totle size %zu, used %zu\n", __func__, pool->size, pool->used);

	size = pool->size - pool->used;

	dev_dbg(dev, "< %s pool remainder %zu\n", __func__, size);
