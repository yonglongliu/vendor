...
/* START [J16031100] - Abnormal T4T Read Patch*/
static bool         sFixPreRfInterface = false;
/* END[J16031100] - Abnormal T4T Read Patch*/
...
{
    ALOGD ("%s: enter, isWriteOk:%d", __FUNCTION__, isWriteOk);
    ...
    ALOGD ("%s: exit, sWriteOk:%d", __FUNCTION__, sWriteOk);
}
...
static jint nativeNfcTag_doConnect (JNIEnv*, jobject, jint targetHandle)
{
    ...
    ALOGD ("%s: sCurrentConnectedTargetType: 0x%02X, sCurrentConnectedTargetProtocol: 0x%02X ", __FUNCTION__,sCurrentConnectedTargetType, sCurrentConnectedTargetProtocol);
...
/* START [J16031100] - Abnormal T4T Read Patch*/
    if(sFixPreRfInterface)
    {
        retCode = NFCSTATUS_SUCCESS;
        ALOGD ("%s() Skip RF interface chaning because of initial operation.", __FUNCTION__);
        goto TheEnd;
    }
/* End [J16031100] - Abnormal T4T Read Patch*/
...
/* START [J16031100] - Abnormal T4T Read Patch*/
            ALOGD ("%s: ndef detection timeout; (sFixPreRfInterface=%d)", __FUNCTION__, sFixPreRfInterface);
            if( (sCurrentRfInterface != NFA_INTERFACE_FRAME) ||
                (sCurrentConnectedTargetProtocol != NFA_PROTOCOL_ISO_DEP) )
                break;
            ALOGD ("%s: Do RF switch for checking Tag presence.", __FUNCTION__);
/* END [J16031100] - Abnormal T4T Read Patch*/
        }
...
static jint nativeNfcTag_doReconnect (JNIEnv*, jobject)
{
    ...
    if (sCurrentConnectedTargetProtocol == NFA_PROTOCOL_ISO_DEP)
		/* START [J16031100] - Abnormal T4T Read Patch*/
    {
        if (natTag.isMifareDESFire ())
            retCode = reSelect(NFA_INTERFACE_ISO_DEP, false);
        else if( (sCurrentRfInterface == NFA_INTERFACE_FRAME) &&
                 natTag.isNdefDetectionTimedOut() &&
                 (natTag.getPresenceCheckAlgorithm() != NFA_RW_PRES_CHK_RB_CH0))
        {
            ALOGD ("%s: Do RF switch for checking Tag presence.", __FUNCTION__);
            retCode = reSelect(NFA_INTERFACE_ISO_DEP, false);
        }
...
void nativeNfcTag_doTransceiveStatus (tNFA_STATUS status, uint8_t* buf, uint32_t bufLen)
{
    ...
    if (sCurrentConnectedTargetProtocol == NFA_PROTOCOL_MIFARE)
    {
        ALOGD ("%s: Mifare", __FUNCTION__);
        if (EXTNS_GetCallBackFlag () == FALSE)
        {
            ALOGD ("%s: false", __FUNCTION__);
...
/* START [J16031100] - Abnormal T4T Read Patch*/
    if( (sCheckNdefStatus == NFC_STATUS_CMD_NOT_CMPLTD) &&
        (sCurrentConnectedTargetProtocol == NFA_PROTOCOL_ISO_DEP) )
    {
        ALOGD ("%s: Fix RF Intface for abnormal T4T tag", __FUNCTION__);
        sFixPreRfInterface = true;
    }
/* END [J16031100] - Abnormal T4T Read Patch*/
...
#if 1 // Check it whether it uses or not
...
#if 1 //we need to check whether it uses of nor (AOSP uses)
    if (sCurrentConnectedTargetProtocol == NFA_PROTOCOL_ISO_DEP)
    {
        int retCode = NFCSTATUS_SUCCESS;
