...
clean_and_return:
    ...
    {
        ALOGE("CRIT: Memory Allocation failed for MFC!");
...
STATIC void Mfc_CheckNdef_Completion_Routine(void *NdefCtxt, NFCSTATUS status)
{
    ...
    if(NFCSTATUS_SUCCESS == status)
    {
        ...
        {
            ALOGD("Mfc_CheckNdef_Completion_Routine : READ_ONLY_CARD");
...
NFCSTATUS Mfc_PresenceCheck(void)
{
    ...
    if (gAuthCmdBuf.auth_status == TRUE)
    {
        ...
        if (status != NFCSTATUS_PENDING)
        {
            ALOGD ("%s: enter auth_sent will set false", __FUNCTION__);
            gAuthCmdBuf.auth_sent = FALSE;
            status = NFCSTATUS_FAILED;
        }
        else
        {
            ALOGD ("%s: enter auth_sent will true", __FUNCTION__);
    ...
    ALOGD ("%s: exit, status = 0x%02x", __FUNCTION__, status);
    return status;
}
/*******************************************************************************
**
...
NFCSTATUS Mfc_WriteNdef(uint8_t *p_data, uint32_t len)
{
    ...
    {
        ALOGE("MFC Error: Invalid Parameters to Ndef Write");
        status = NFCSTATUS_FAILED;
        goto Mfc_WrNdefEnd;
    }
    ...
    else
    {
        ...
        int i = 0;
        for(i=0; i<len; i++)
            ALOGD ("%s: pdata[%d]: 0x%02X", __FUNCTION__,i,p_data[i]);
...
NFCSTATUS Mfc_Transceive(uint8_t *p_data, uint32_t len)
{
    ...
    ALOGD("%s: enter, p_data[0]: 0x%02X",__FUNCTION__,p_data[0]);
    ...
    else
    {
        ...
        ALOGD("%s: enter, NdefMap->Cmd:0x%02X, phNfc_emifareRaw",__FUNCTION__,NdefMap->Cmd);
    ...
    {
        ALOGE("ERROR: Mfc_Transceive = 0x%x", status);
    }
    ALOGD("%s: exit",__FUNCTION__);
                       ...
                       )
{
    ...
    else
    {
        ...
        else
        {
            ...
            ALOGD ("%s: RecvdExtnRspId", __FUNCTION__);
            ...
            switch(RecvdExtnRspId)
            {
                ...
                default:
                {
                    ALOGD ("%s: [NFC] no data case", __FUNCTION__);
                    status = NFCSTATUS_FAILED;
                }
                break;
            }
...
{
    ALOGD("%s: enter",__FUNCTION__);
    //START-KHJ added to send write cmd
    uint8_t bBuffIdx = 0x00;
    //END-KHJ added to send write cmd
        ...
        (0 != (pTransceiveInfo->sSendData.length)))
    {
        ...
        pMappedTranscvIf->tSendData.wLen              = bBuffIdx + pTransceiveInfo->sSendData.length;//add bBuff
...
NFCSTATUS Mfc_RecvPacket(uint8_t *buff, uint8_t buffSz)
{
    ...
    ALOGD("%s: NFC status: 0x%02X", __FUNCTION__, status);
    ...
    {
        ALOGD("%s: gphNxpExtns_Context.writecmdFlag", __FUNCTION__);
        ...
        if ( NFCSTATUS_PENDING != status )
        {
            ALOGE("ERROR : Mfc_RecvPacket: 0x%x", status);
        }
        else
        {
            status = NFCSTATUS_SUCCESS;
    ...
    {
        ALOGD("%s: gphNxpExtns_Context.incrdecflag", __FUNCTION__);
        pcmd_buff = (uint8_t *)malloc((uint32_t)MAX_BUFF_SIZE);
        if( NULL == pcmd_buff )
        {
            return NFCSTATUS_FAILED;
    ...
    {
        ALOGD("%s: gphNxpExtns_Context.default", __FUNCTION__);
                             ...
                             uint8_t *buff, uint16_t *buffSz)
{
    ...
    //buff[i++] = phNciNfc_e_MfRawDataXchgHdr;//FW dependency code
...
//    pphNciNfc_RemoteDevInformation_t  pActivDev = NULL;           /*commented to eliminate unused variable warning*/
    ...
    {
        //bKey = bKey | PHNCINFC_ENABLE_KEY_B;// FW dependency code
        ALOGD("%s:AUTH_KEYB:0x61, pbuff[2]: 0x%02X", __FUNCTION__, tTranscvInfo.tSendData.pBuff[2]);
    }
    ...
    //buff[0] = phNciNfc_e_MfcAuthReq;// FW dependency code
    ALOGD("%s:tSendData.pbuff[0]:0x%02X", __FUNCTION__, tTranscvInfo.tSendData.pBuff[0]);
    buff[0] = tTranscvInfo.tSendData.pBuff[0];
    buff[1] = bBlockAddr;
    buff[2] = bKey;

    if(bKey == 0)
    {
        ALOGD("%s: Mifare madkey", __FUNCTION__);
        memcpy(&buff[2], aMfc_keys[bKey], PHLIBNFC_MFC_AUTHKEYLEN);
    }
    else if(bKey == 1)
    {
        ALOGD("%s: Mifare Ndefkey", __FUNCTION__);
        memcpy(&buff[2], aMfc_keys[bKey], PHLIBNFC_MFC_AUTHKEYLEN);
    }
    else if(bKey == 2)
    {
        ALOGD("%s: Mifare Rawkey", __FUNCTION__);
        memcpy(&buff[2], aMfc_keys[bKey], PHLIBNFC_MFC_AUTHKEYLEN);
    }
    else
    {
        ALOGD("%s: Mifare key", __FUNCTION__);
        memcpy(&buff[2], aMfc_keys[3], PHLIBNFC_MFC_AUTHKEYLEN);
    }
...
STATIC NFCSTATUS phLibNfc_GetKeyNumberMFC(uint8_t *buffer,uint8_t *bKey)
{
       ...
       NULL != buffer )
    {
       ...
       for(bIndex = 0; bIndex < bNoOfKeys; bIndex++)
       {
           ...
           if(!sdwStat)
           {
               ALOGD("Mifare : phLibNfc_GetKeyNumberMFC Key found");
               *bKey = bIndex;
               wStatus = NFCSTATUS_SUCCESS;
               break;
           }
        }
       ALOGD("Mifare : phLibNfc_GetKeyNumberMFC returning = 0x%x Key = 0x%x", wStatus, *bKey);
    }
    else
    {
        wStatus = NFCSTATUS_FAILED;
        ALOGD("Mifare : phLibNfc_GetKeyNumberMFC returning = 0x%x", wStatus);
    }
                                    ...
                                    pphNciNfc_TransceiveInfo_t   pMappedTranscvIf)
{
    ...
    switch(pTransceiveInfo->cmd.MfCmd)
    {
        ...
        {
            ALOGD("%s: Authen A|B",__FUNCTION__);
            if( (NULL != pTransceiveInfo->sSendData.buffer) &&
                (0 != (pTransceiveInfo->sSendData.length)) &&
                ...
                )
            {
                ...
                if(NFCSTATUS_FAILED != status )
                {
                    ...
                    //phLibNfc_CalSectorAddress(&bSectorNumber);// to deal with block address
        ...
        {
            ALOGD("%s: MifareRaw: do nothing",__FUNCTION__);
                                   ...
                                   uint16_t *SendRecvLength)
{
    ...
    ALOGD("%s: enter, pTans->cmd:0x%02X,length:%d",__FUNCTION__,pTransceiveInfo->cmd,SendLength);

    if( (Cmd.MfCmd == phNfc_eMifareAuthentA) ||
        (Cmd.MfCmd == phNfc_eMifareAuthentB) )
    {
        ALOGD("%s: Cmd.MfCmd=0x%02X",__FUNCTION__,Cmd.MfCmd);
    ...
    if (NFCSTATUS_SUCCESS == status )
    {
        ...
        if (NFCSTATUS_PENDING != status)
        {
            ALOGE("ERROR : phNciNfc_SendMfReq()");
        }
    }
    else
    {
        ALOGE (" ERROR : Sending phNciNfc_SendMfReq");
    }
    if( buff != NULL )
    {
        free(buff);
