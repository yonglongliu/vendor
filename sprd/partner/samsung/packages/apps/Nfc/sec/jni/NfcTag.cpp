...
/* START [J14111102] - KOVIO BARCODE */
            ...
            if (TimeDiff(mLastKovioTime, now) < 2000)
...
void NfcTag::discoverTechnologies (tNFA_ACTIVATED& activationData)
{
    ...
    ALOGD ("%s: rfDetail protocol: 0x%02x", fn,rfDetail.protocol);
    mNumTechList = 0;
...
void NfcTag::discoverTechnologies (tNFA_DISC_RESULT& discoveryData)
{
    ...
    mNumTechList++;
/*START [16060100J] - Handle more value in RF DISCOVER_NTF to fix AOSP  bug */
    if (discovery_ntf.more != NCI_DISCOVER_NTF_MORE)
/*END [16060100J] - Handle more value in RF DISCOVER_NTF to fix AOSP  bug */		
    {
        for (int i=0; i < mNumTechList; i++)
        {
            ALOGD ("%s: index=%d; tech=%d; handle=%d; nfc type=%d", fn,
