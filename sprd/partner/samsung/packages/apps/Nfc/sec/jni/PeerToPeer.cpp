...
#define SEGMENTED_DATA_WAIT_TIMEOUT     3000    /* [J14111300] */
...
/* START [J14111300] - Do not wait Segmented Message infinitely(workaround) */
    extern bool lessFwVersion(int m1, int m2, int b1, int b2);
/* END [J14111300] Do not wait Segmented Message infinitely(workaround) */
}
...
/* START [J14111300] - Do not wait Segmented Message infinitely(workaround) */
            if (android::lessFwVersion(1, 1, 0, 15))
            {
                if(false == pConn->mReadEvent.wait(SEGMENTED_DATA_WAIT_TIMEOUT))
                {
                    ALOGE ("%s: time over for 3000ms ", fn);
                    break;
                }
            }
            else
...
/* START [P1605170001] - Add debugging message for monitoring listen tech of CE */
    ALOGD ("%s: enter isEnable: %u  mIsP2pListening: %u mP2pListenTechMask : 0x%X", fn, isEnable, mIsP2pListening, mP2pListenTechMask);
