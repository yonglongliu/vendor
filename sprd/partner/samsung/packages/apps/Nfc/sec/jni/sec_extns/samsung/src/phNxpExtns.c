...
static NFCSTATUS phNxpExtns_ProcessSysMessage (phLibNfc_Message_t *msg)
{
    ...
    {
        ALOGD ("%s: EXTNS_STATUS_CLOSE", __FUNCTION__);
        return NFCSTATUS_FAILED;
    }
    ...
    switch (msg->eMsgType)
    {
        case PH_NXPEXTNS_RX_DATA:
            ...
            ALOGD ("%s: status: 0x%02X", __FUNCTION__, status);
            break;
        ...
        case PH_NXPEXTNS_MIFARE_READ_ONLY:
            ALOGD ("%s: enter, MIFARE_READ_ONLY", __FUNCTION__);
...
NFCSTATUS EXTNS_GetPresenceCheckStatus (void)
{
    ...
    if (sem_timedwait (&gAuthCmdBuf.semPresenceCheck, &ts))
    {
        ...
        gAuthCmdBuf.auth_sent = FALSE;// [KHJ] set false becasue of presence check 
        return NFCSTATUS_FAILED;
    }
...
{
    ALOGE ("%s: enter , auth_sent will set false", __FUNCTION__);
