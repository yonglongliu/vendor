...
void data_trace(char *head, int len, uint8_t *p_data);
...
int device_open()
{
    ...
    ret = get_config_string(cfg_name_table[CFG_POWER_DRIVER], pw_driver_name, sizeof(pw_driver_name));
    if (ret == 0)
        return -EPERM;

    ret = get_config_string(cfg_name_table[CFG_TRANS_DRIVER], tr_driver_name, sizeof(tr_driver_name));
    if (ret == 0)
        return -EPERM;
    ...
    if (pw_driver < 0)
    {
        OSI_loge("Failed to open device driver: %s, pw_driver : 0x%x, errno = %d", pw_driver_name, pw_driver, errno);
        return pw_driver;
    }
...
int device_write(uint8_t *data, size_t len)
{
    ...
    while (len != 0)
    {
        ...
        if (ret < 0 )
        {
            OSI_loge("write error ret = %d, errno = %d, retry = %d", ret, errno, retry);
...
void data_trace(char *head, int len, uint8_t *p_data)
{
    ...
    {
        if (i < header)
            snprintf(trace_buffer+(i*3), 6, "%02x   ", p_data[i]);
        else
            snprintf(trace_buffer+(i*3+2), 4, "%02x ", p_data[i]);
        i++;
    }

    OSI_logd(" %s(%3d) %s", head, i, trace_buffer);
}
