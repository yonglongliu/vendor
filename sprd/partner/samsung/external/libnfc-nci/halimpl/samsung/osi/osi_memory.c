...
tOSI_MEM_HANDLER OSI_mem_get (size_t size)
{
    ...
    if (size > OSI_MEM_POOL_SIZE)
    {
        OSI_loge("%s : memory getting failed. Max size=%d, Requested size=%d",
                            __func__, OSI_MEM_POOL_SIZE, size);
        return NULL;
    }
...
#ifdef OSI_USE_DYNAMIC_BUF
        /* get a new buffer */
        free_mem = osi_info.mem[index] = (tOSI_MEM_HANDLER)malloc(OSI_MEM_POOL_SIZE);
        if (osi_info.mem[index] != NULL)
        {
            osi_info.mem[index]->state = OSI_FREE;
            osi_info.mem_max_cnt++;
            OSI_loge("%s : get a new buffer (max: %d)", __func__, osi_info.mem_max_cnt);
        }
        else
#endif
        if (--err_cnt > 0)
...
tOSI_QUEUE_HANDLER OSI_queue_allocate (char *que_name)
{
    ...
    for (index = 0; index < osi_info.queue_max_cnt; index++)
    {
        ...
        {
            if (osi_info.queue[index].name == NULL)
                continue;

            if (strcmp((char const*)osi_info.queue[index].name, (char const*)que_name) == 0)
            {
                OSI_loge("%s : %s queue is already allocated [%d]",
                        __func__, que_name, index);
                free_que = NULL;
                break;
            }
        }
    ...
    if (free_que == NULL)
    {
        OSI_loge("%s : Failed to find free queue(max: %d)", __func__, OSI_MAX_QUEUE);
    }
    else
    {
...
tOSI_QUEUE_HANDLER OSI_queue_get_handler (char *name)
{
    ...
    for (index = 0; index < OSI_MAX_QUEUE; index++)
    {
        ...
        if (strcmp((char const*)osi_info.queue[index].name, (char const*)name)  == 0)
        {
