...
** Public functions
*************************************************************************/
tOSI_TIMER_HANDLER OSI_timer_allocate (char *timer_name)
{
    ...
    for (index = 0; index < OSI_MAX_TIMER; index++)
    {
        ...
        {
            if ((char const*)osi_info.timer[index].name == NULL)
                continue;

            if (strcmp((char const *)osi_info.timer[index].name, (char const *)timer_name) == 0)
            {
                OSI_loge("%s : %s timer is already allocated [%d]",
                        __func__, timer_name, index);
                free_timer = NULL;
                break;
            }
        }
    ...
    if (free_timer == NULL)
    {
        OSI_loge("%s : Failed to find free timer(max: %d)", __func__, OSI_MAX_TIMER);
    }
    else
    {
...
//[END] S.LSI - To increase usingTimer prior to timer_thread
                ...
                if(ret_th != 0)
                    OSI_loge("%s : Error pthread_attr_init! ,erron: %d", __func__,ret_th);

                ret_th = pthread_create( &osi_info.timer_thread,
                            &attr,
                            (void *)timer_thread,
                            NULL);
                if(ret_th != 0)
                    OSI_loge("%s : Error to create timer_thread! ,erron: %d", __func__,ret_th);

                ret_th = pthread_attr_destroy(&attr);
                if(ret_th != 0)
                    OSI_loge("%s : Error pthread_arrt_destroy ,erron: %d", __func__,ret_th);
            }
            else 
                osi_info.usingTimer++;
            }
...
tOSI_TIMER_HANDLER OSI_timer_get_handler (char *name)
{
    ...
    for (index = 0; index < OSI_MAX_TIMER; index++)
    {
        ...
        if (strcmp((char const *)osi_info.timer[index].name, (char const *)name) == 0)
        {
...
int32_t OSI_timer_get_current_time()
{
    ...
    return (((now->tm_hour*3600)+(now->tm_min*60)+(now->tm_sec))*1000)+(sec.tv_usec/1000);
}
/************************************************************************
** Internal function
*************************************************************************/
void OSI_timer_update(int32_t tick)
{
    ...
    for (index = 0; index < OSI_MAX_TIMER; index++)
    {
        if (osi_info.timer[index].state == OSI_RUN)
        {
            ...
            {
                if( (OSI_timer_get_current_time()-osi_info.timer[index].exact_time) > osi_info.timer[index].init_timeout )
                {
                    osi_info.timer[index].state = OSI_STOP;
                    osi_info.usingTimer--;

                    if (osi_info.timer[index].callback != NULL)
                        osi_info.timer[index].callback( osi_info.timer[index].callback_param);
                }
                else
                {
                    osi_info.timer[index].timeout = osi_info.timer[index].init_timeout
...
void timer_thread(void)
{
    ...
    while(osi_info.usingTimer > 0)
    {
        ...
        do
        {
            ...
            if(err < 0)
                OSI_loge("%s:Fail nanosleep",__func__);
