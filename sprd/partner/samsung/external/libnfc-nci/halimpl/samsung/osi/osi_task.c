...
tOSI_TASK_HANDLER OSI_task_allocate (char *task_name, tOSI_TASK_ENTRY task_entry)
{
    ...
    for (index = 0; index < OSI_MAX_TASK; index++)
    {
        ...
        {
            if (osi_info.task[index].name == NULL)
                continue;

            /* User can't not make same name of task */
            if (strcmp((char const *)osi_info.task[index].name, (char const *)task_name) == 0)
            {
                OSI_loge("%s : %s task is already allocated [%d]",
                        __func__, task_name, index);
                free_task = NULL;
                break;
            }
        }
...
OSI_STATE OSI_task_run (tOSI_TASK_HANDLER task_handler)
{
    ...
    else
    {
        ...
        if ( !pthread_create( &(task_handler->task), &attr,
                    (void *)osi_task_entry, (void *)(task_handler->task_entry)) )
        {
        ...
        {
            OSI_loge("%s : pthread_create failed(%d), %s", __func__, ret, task_handler->name);
        }
...
OSI_STATE OSI_task_isRun(tOSI_TASK_HANDLER task_handler)
{
    ...
    osi_lock();
    if (task_handler && task_handler->state == OSI_RUN)
        ret = OSI_RUN;
    osi_unlock();
    return ret;
}
...
tOSI_TASK_HANDLER OSI_task_get_handler (char *name)
{
    ...
    for (index = 0; index < OSI_MAX_TASK; index++)
    {
        if ((char const*)osi_info.task[index].name == NULL)
            continue;

        if (strcmp((char const *)osi_info.task[index].name, (char const *)name) == 0)
        {
