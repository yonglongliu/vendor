...
void OSI_deinit()
{
    ...
    {
        if (osi_info.mem[index])
            free(osi_info.mem[index]);
    }
#endif
...
inline void osi_lock()
{
    pthread_mutex_lock(&osi_info.mutex);
}

inline void osi_unlock()
{
    pthread_mutex_unlock(&osi_info.mutex);
}
