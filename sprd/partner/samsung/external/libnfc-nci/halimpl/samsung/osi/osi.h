 ...
 *              NULL if any problem.
 */
tOSI_TASK_HANDLER OSI_task_allocate (
        char            *task_name,
 ...
 */
OSI_STATE OSI_task_run (
        tOSI_TASK_HANDLER task_handler
);
 ...
 *              NULL, on error.
 */
tOSI_TASK_HANDLER OSI_task_get_handler (
        char        *name
);
 ...
 *              NULL if any problem.
 */
tOSI_QUEUE_HANDLER OSI_queue_allocate (
        char    *que_name
);
 ...
 *              NULL if any problem.
 */
void *OSI_queue_get_wait (
        tOSI_QUEUE_HANDLER    target
);
 ...
 *              NULL, on error.
 */
tOSI_QUEUE_HANDLER OSI_queue_get_handler (
        char        *name
);
 ...
 */
tOSI_TIMER_HANDLER OSI_timer_allocate (
        char    *timer_name
);
 ...
 *              NULL, on error.
 */
tOSI_TIMER_HANDLER OSI_timer_get_handler (
        char        *name
);
...
#define OSI_logt(format, ...)   do{if (osi_debug_level >= 2) __osi_log(LOG_INFO, "%s: " format, __func__, ##__VA_ARGS__);} while(0)
#define OSI_logd(format, ...)   do{if (osi_debug_level >= 1) __osi_log(LOG_DEBUG, "%s: " format, __func__, ##__VA_ARGS__);} while(0)
#define OSI_loge(format, ...)   do{if (osi_debug_level >= 0) __osi_log(LOG_ERROR, "%s: " format, __func__, ##__VA_ARGS__);} while(0)
#else
#define OSI_logt(...)
#define OSI_logd(...)
#define OSI_loge(...)
