...
/* START [P1604040001] - Support Dual-SIM solution */
#if(NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
int nfa_hci_setPreferredSimSlot(UINT16 sim_slot)
{
    NFA_TRACE_DEBUG1 ("nfa_hci_setPreferredSimSlot () : SIM Slot = 0x%x", sim_slot);
...
BOOLEAN nfa_hci_is_valid_cfg (void)
{
    ...
    for (xx = 0; xx < NFA_HCI_MAX_GATE_CB; xx++)
    {
        if (nfa_hci_cb.cfg.dyn_gates[xx].gate_id != 0)
        {
            ...
            {
                NFA_TRACE_EVENT1 ("[1] nfa_hci_is_valid_cfg  Invalid Gate owner: %u", nfa_hci_cb.cfg.dyn_gates[xx].gate_owner);
                return FALSE;
            }
/* START [P1604040001] - Support Dual-SIM solution */
#if(NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
...
/* END [P1604040001] - Support Dual-SIM solution */
            {
                ...
                if (zz == app_count)
                {
                    NFA_TRACE_EVENT1 ("[2] nfa_hci_is_valid_cfg  Invalid Gate owner: %u", nfa_hci_cb.cfg.dyn_gates[xx].gate_owner);
                    return FALSE;
                }
            }
...
/* START [P1604040001] - Support Dual-SIM solution */
#if(NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
    nfa_nv_co_read2((UINT8 *)&nfa_hci_cb.cfg, sizeof (nfa_hci_cb.cfg),DH_NV_BLOCK, (UINT8)nPreferredSimSlot);
#else
...
void nfa_hci_handle_nv_read (UINT8 block, tNFA_STATUS status)
{
    ...
    if (block == DH_NV_BLOCK)
    {
        ...
        {
            NFA_TRACE_DEBUG0("nfa_hci_handle_nv_read (): HCI network is reseted.");
        ...
        {
            NFA_TRACE_DEBUG0("nfa_hci_handle_nv_read (): HCI network is reused.");
...
#if(NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
        BOOLEAN ret = TRUE;
        ret = nfa_nv_co_write2 ((UINT8 *)&nfa_hci_cb.cfg, sizeof (nfa_hci_cb.cfg),DH_NV_BLOCK, (UINT8)nPreferredSimSlot);

        /* Does need to copy nv_storage fiile. */
        if(nPreferredSimSlot == 0x02)
        {
             if( (nfa_hci_cb.dh_is_occurred_clear_all_pipe) || (open ("/data/nfc/nfaStorage.bin1", O_RDONLY) < 0) )
             {
                 nfa_hci_cb.dh_is_occurred_clear_all_pipe = FALSE;
                 ret = nfa_nv_co_write2 ((UINT8 *)&nfa_hci_cb.cfg, sizeof (nfa_hci_cb.cfg),DH_NV_BLOCK, (UINT8)0x01);
             }
        }
        else
        {
            if( (nfa_hci_cb.dh_is_occurred_clear_all_pipe) || (open ("/data/nfc/nfaStorage2.bin1", O_RDONLY) < 0) )
            {
                nfa_hci_cb.dh_is_occurred_clear_all_pipe = FALSE;
                ret = nfa_nv_co_write2 ((UINT8 *)&nfa_hci_cb.cfg, sizeof (nfa_hci_cb.cfg),DH_NV_BLOCK, (UINT8)0x02);
            }
        }
#if 0       // Dual_SIM Debugging
            int xx;
            tNFA_HCI_DYN_PIPE *pg;
            for (xx = 0, pg = nfa_hci_cb.cfg.dyn_pipes; xx < NFA_HCI_MAX_PIPE_CB; xx++, pg++)
            {
                NFA_TRACE_DEBUG2("nfa_hci_sys_enable () : [%d] dyn_pipe_id(0x%x)", xx, pg->pipe_id);
                NFA_TRACE_DEBUG2("nfa_hci_sys_enable () : [%d] local_gate(0x%x)", xx,  pg->local_gate);
                NFA_TRACE_DEBUG2("nfa_hci_sys_enable () : [%d] dest_host(0x%x)", xx, pg->dest_host);
                NFA_TRACE_DEBUG2("nfa_hci_sys_enable () : [%d] dest_gate(0x%x)", xx, pg->dest_gate);
                NFA_TRACE_DEBUG2("nfa_hci_sys_enable () : [%d] pipe_owner(0x%x)", xx, nfa_hciu_get_pipe_owner(pg->pipe_id));
            }
#endif
#endif
/* END [P1604040001] - Support Dual-SIM solution */
