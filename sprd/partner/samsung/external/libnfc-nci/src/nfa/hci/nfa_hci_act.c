...
#if 0
        NFA_TRACE_DEBUG2 ("nfa_hci_api_get_gate_pipe_list (%d) ==> gate_owner(0x%x)", xx, pg->gate_owner);
        NFA_TRACE_DEBUG2 ("nfa_hci_api_get_gate_pipe_list (%d) ==> dyn_pipes(0x%x)", xx, pg->gate_id);
#endif
// START [20160829001] Debugging Code for HCI Network.
...
void nfa_hci_handle_admin_gate_cmd (UINT8 *p_data)
{
    ...
    case NFA_HCI_ADM_NOTIFY_PIPE_CREATED:
        ...
        else
        {
            ...
            {
/* START [160812001] Patch for Connectivity Gate Issue (by KJY)*/
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
                if (nfa_hciu_check_pipe_between_gates (dest_gate, source_host, source_gate)) {

                    if((dest_gate == NFA_HCI_CONNECTIVITY_GATE && dest_gate == source_gate && source_host == 0x03) ||
                    (dest_gate == 0xF0 && dest_gate == source_gate && source_host == 0x03)) {
                        tNFA_HCI_DYN_PIPE   *pp;
                        int                 xx;
                        int                 remove_pipe = -1;

                        NFA_TRACE_DEBUG1 ("HCI Patch - Start [0x%02X] pipe re-assign.", dest_gate);
                        ...
                        for (xx = 0, pp = nfa_hci_cb.cfg.dyn_pipes; xx < NFA_HCI_MAX_PIPE_CB; xx++, pp++)
                        {
                            ...
                            {
                                NFA_TRACE_DEBUG2 ("HCI Patch - Pipe re-assigned. removed - 0x%02X, re-assigned - 0x%02X", pp->pipe_id, pipe);
...
#endif
/* END [160812001] Patch for Connectivity Gate Issue (by KJY)*/
                ...
                else if ((response = nfa_hciu_add_pipe_to_gate (pipe, dest_gate, source_host, source_gate)) == NFA_HCI_ANY_OK)
                {
                    ...
                    NFA_TRACE_DEBUG1 ("HCI Patch - pgate->gate_owner(0x%02X)", pgate->gate_owner);
