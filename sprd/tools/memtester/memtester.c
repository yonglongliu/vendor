...
extern int set_cpu_affinity(unsigned int cpu);
...
void volatile* malloc_lock_mem(size_t wantbytes, size_t wantbytes_orig, ptrdiff_t pagesizemask,
	...
	while (!done_mem) {
		...
		*bufsize = wantbytes;
		...
		if (*do_mlock) {
			...
			fprintf(fileno(stdout), ", trying mlock ...");
			fsync(fileno(stdout));
			fflush(stdout);
	...
	if (!(*do_mlock))
		fprintf(fileno(stdout), "Continuing with unlocked memory; testing "
...
void test_refresh(ptrdiff_t pagesizemask, size_t pagesize)
{
	...
	if (aligned == NULL) {
		fprintf(fileno(stdout), "failed to malloc_lock_mem\n");
		exit(EXIT_FAIL_NONSTARTER);
	}
...
int main(int argc, char **argv) {
    ...
    size_t pagesize, wantraw, wantmb, wantbytes, wantbytes_orig, bufsize,
         halflen, count;
    char *memsuffix , *addrsuffix, *loopsuffix, *modesuffix, *coreIndexsuffix, *reftimesuffix, *refsizesuffix = NULL;
    ...
    if (env_testmask = getenv("MEMTESTER_TEST_MASK")) {
        ...
        if (errno) {
            fprintf(fileno(stdout), "error parsing MEMTESTER_TEST_MASK %s: %s\n",
        ...
        switch (opt) {
            ...
            case 'd':
                if (stat(optarg,&statbuf)) {
                    fprintf(fileno(stdout), "can not use %s as device: %s\n", optarg,
                            strerror(errno));
                    usage(argv[0]); /* doesn't return */
                } else {
                    if (!S_ISCHR(statbuf.st_mode)) {
                        fprintf(fileno(stdout), "can not mmap non-char device %s\n",
                                optarg);
                        usage(argv[0]); /* doesn't return */
                    } else {
                        device_name = optarg;
    ...
    if (optind >= argc) {
        fprintf(fileno(stdout), "need memory argument, in MB\n");
        usage(argv[0]); /* doesn't return */
    }
    ...
    if (errno != 0) {
        fprintf(fileno(stdout), "failed to parse memory argument");
        usage(argv[0]); /* doesn't return */
    }
    switch (*memsuffix) {
        case 'G':
    ...
    optind++;
    if (wantmb > maxmb) {
        fprintf(fileno(stdout), "This system can only address %llu MB.\n", (ull) maxmb);
        exit(EXIT_FAIL_NONSTARTER);
    }
    if (wantbytes < pagesize) {
        fprintf(fileno(stdout), "bytes %ld < pagesize %ld -- memory argument too large?\n",
                wantbytes, pagesize);
        exit(EXIT_FAIL_NONSTARTER);
    }
    ...
    if (optind >= argc) {
        ...
        if (errno != 0) {
            fprintf(fileno(stdout), "failed to parse number of loops");
            usage(argv[0]); /* doesn't return */
        }
        if (*loopsuffix != '\0') {
            fprintf(fileno(stdout), "loop suffix %c\n", *loopsuffix);
            usage(argv[0]); /* doesn't return */
        }
    }
    fprintf(fileno(stdout), "loops = %d\n", loops);
    ...
    if (optind >= argc) {
        ...
        if (errno != 0) {
            fprintf(fileno(stdout), "failed to parse number of mode");
            usage(argv[0]); /* doesn't return */
        }
        if (*modesuffix != '\0') {
            fprintf(fileno(stdout), "mode suffix %c\n", *modesuffix);
            usage(argv[0]); /* doesn't return */
        }
    }

    compare_20_times = mode;
    fprintf(fileno(stdout), "mode = %d\n", mode);
    ...
    if (optind >= argc) {
        ...
        if (errno != 0) {
            fprintf(fileno(stdout), "failed to parse number of coreIndex");
            usage(argv[0]); /* doesn't return */
        }
        if (*coreIndexsuffix != '\0') {
            fprintf(fileno(stdout), "coreIndex suffix %c\n", *coreIndexsuffix);
            usage(argv[0]); /* doesn't return */
        }
    }
    fprintf(fileno(stdout), "coreIndex = %d\n", coreIndex);
    ...
    if (optind >= argc) {
        ...
        if (errno != 0) {
            fprintf(fileno(stdout), "failed to parse number of refsizesuffix");
            usage(argv[0]); /* doesn't return */
        }
        if (*refsizesuffix != 'K') {
            fprintf(fileno(stdout), "refreshsize suffix %c\n", *refsizesuffix);
            usage(argv[0]); /* doesn't return */
        }
    }
    fprintf(fileno(stdout), "refsize = %d\n", refsize);
    ...
    if (optind >= argc) {
        ...
        if (errno != 0) {
            fprintf(fileno(stdout), "failed to parse number of reftime");
            usage(argv[0]); /* doesn't return */
        }
        if (*reftimesuffix != '\0') {
            fprintf(fileno(stdout), "reftime suffix %c\n", *reftimesuffix);
            usage(argv[0]); /* doesn't return */
        }
    }
    fprintf(fileno(stdout), "reftime = %d\n", reftime);
    ...
    if (use_phys) {
        ...
        if (memfd == -1) {
            fprintf(fileno(stdout), "failed to open %s for physical memory: %s\n",
        ...
        buf = (void volatile *) mmap(0, wantbytes, PROT_READ | PROT_WRITE,
                                     ...
                                     physaddrbase);
        if (buf == MAP_FAILED) {
            fprintf(fileno(stdout), "failed to mmap %s for physical memory: %s\n",
                    device_name, strerror(errno));
            exit(EXIT_FAIL_NONSTARTER);
        }

        if (mlock((void *) buf, wantbytes) < 0) {
            fprintf(fileno(stdout), "failed to mlock mmap'ed space\n");
            do_mlock = 0;
        }
